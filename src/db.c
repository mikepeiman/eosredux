...
void load_traps args((FILE * fp,
		      OBJ_INDEX_DATA * pObj,
		      ROOM_INDEX_DATA * pRoom, EXIT_DATA * pExit));
	...
	for (;;)
		...
		case 'm':
			...
			if ((iMob = get_mob_index(value)) == NULL) {
				bug("Load_mobprogs: vnum %d doesnt exist",
				    value);
				exit(EXIT_FAILURE);
			}
...
void mprog_read_programs(FILE * fp, MOB_INDEX_DATA * pMobIndex)
{
	...
	if ((letter = fread_letter(fp)) != '>') {
		bug("Mprog_read_programs: vnum %d MOBPROG char",
		    pMobIndex->vnum);
		exit(EXIT_FAILURE);
	}
		...
		EXIT_DATA * pExit)
{
	...
	while (!done) {
		pTrap->type =
		    flag_value((pObj ? oprog_types
				: (pRoom ? rprog_types : eprog_types)),
			       fread_word(fp));
...
void load_recall(FILE * fp)
{
	...
	if (pArea->recall < 1) {
		sprintf(buf, "Load_recall:  %s invalid recall point",
			pArea->name);
		bug(buf, 0);
		pArea->recall = ROOM_VNUM_TEMPLE;
	}
				...
				if (!IS_NPC(pch)
					...
					if (pArea->reset_sound == NULL)
						send_to_char(AT_GREEN,
							     "You shiver as a cold breeze blows through the room.\n\r",
							     pch);
					else if (str_cmp
						 (pArea->reset_sound, "none")) {
						send_to_char(AT_GREEN,
							     pArea->reset_sound,
							     pch);
						send_to_char(AT_GREEN, "\n\r",
...
void reset_room(ROOM_INDEX_DATA * pRoom)
{
	...
	for (pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next) {
		...
		default:
			sprintf(log_buf,
				"Reset_room: bad command %c in room %d.",
		...
		case 'M':
			if (!(pMobIndex = get_mob_index(pReset->arg1))) {
				sprintf(log_buf,
					"Reset_room: 'M': bad vnum %d in room %d.",
		...
		case 'O':
			if (!(pObjIndex = get_obj_index(pReset->arg1))) {
				sprintf(log_buf,
					"Reset_room: 'O': bad vnum %d in room %d.",
					pReset->arg1, pRoom->vnum);
				bug(log_buf, 0);
				continue;
			}
		...
		case 'P':
			if (!(pObjIndex = get_obj_index(pReset->arg1))) {
				sprintf(log_buf,
					"Reset_room: 'P': bad vnum %d in room %d.",
					pReset->arg1, pRoom->vnum);
				bug(log_buf, 0);
				continue;
			}
		...
		case 'E':
			if (!(pObjIndex = get_obj_index(pReset->arg1))) {
				sprintf(log_buf,
					"Reset_room: 'E' or 'G': bad vnum %d in room %d.",
					pReset->arg1, pRoom->vnum);
				bug(log_buf, 0);
				continue;
			}
			...
			if (!LastMob) {
				sprintf(log_buf,
					"Reset_room: 'E' or 'G': null mob for vnum %d in room %d.",
					pReset->arg1, pRoom->vnum);
				bug(log_buf, 0);
				last = FALSE;
				break;
...
/* OLC 1.1b */
			if (!(pRoomIndex = get_room_index(pReset->arg1))) {
				sprintf(log_buf,
					"Reset_room: 'R': bad vnum %d in room %d.",
					pReset->arg1, pRoom->vnum);
				bug(log_buf, 0);
				continue;
			}
		...
		if (parent->extra_descr) {
			...
			for (ed = parent->extra_descr; ed; ed = ed->next) {
				clone->extra_descr->keyword =
				    str_dup(parent->extra_descr->keyword);
				clone->extra_descr->description =
...
void *new_mem(int sMem, int isperm)
{
	...
	if (!(pMem = calloc(1, sMem)) ||
		...
		if (!pMem) {
			...
			fprintf(stderr,
				"New_mem: NULL warning memory reboot.\n");
		...
		if (pMem && !Already) {
			...
			fprintf(stderr, "Memory used: %d\n", sAllocPerm);
			exit(EXIT_FAILURE);	/*No mem for anything. Straight reboot */
		}
		Already = TRUE;
	}
	...
	for (pArea = area_first; pArea; pArea = pArea->next) {
		...
		if (!IS_SET(pArea->area_flags, AREA_PROTOTYPE) &&
			...
			sprintf(buf, "&W%-8.8s &G%-18.18s &B%-10.10s",
				pArea->name, &pArea->name[9], &pArea->name[28]);
			strcat(buf1, buf);
			if (++col % 2 == 0)
				strcat(buf1, "\n\r");
		}
...
void do_memory(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->level >= LEVEL_IMMORTAL) {
		...
		sprintf(buf,
			"Perms   %5d blocks of %7d bytes.  Max %7d bytes.\n\r",
		...
		sprintf(buf,
			"MemAllocated: %d MemFreed: %d Memory being used: %d\n\r",
			MemAllocated, MemFreed, MemAllocated - MemFreed);
		send_to_char(C_DEFAULT, buf, ch);
		sprintf(buf, "%3d%% of %d.\n\r",
			(((MemAllocated - MemFreed) * 10) / (1024 * 1024)),
			(10 * 1024 * 1024));
		send_to_char(C_DEFAULT, buf, ch);
		return;
	}
...
void challenge(const char *str, int param1, int param2)
{
	...
	for (d = descriptor_list; d; d = d->next) {
		...
		if (d->connected == CON_PLAYING
		    && !IS_SET(ch->deaf, CHANNEL_CHALLENGE)) {
			send_to_char(AT_WHITE, buf, d->character);
			send_to_char(C_DEFAULT, "\n\r", d->character);
		}
	}
...
void logch(char *l_str, int l_type, int lvl)
{
	...
	for (d = descriptor_list; d; d = d->next) {
		if (d->connected != CON_PLAYING
		    || IS_SET(d->character->deaf, l_type)
...
void parse_ban(char *argument, BAN_DATA * banned)
{
	...
	memset(user, 0, MAX_INPUT_LENGTH);
...
void load_player_list(void)
{
	...
	for (;;) {
		...
		sprintf(buf, "%s%c/%s.gz", PLAYER_DIR,
			LOWER(pPlayerList->name[0]),
			capitalize(pPlayerList->name));
		if (stat(buf, &statis)) {
			sprintf(buf, "%s%c/%s", PLAYER_DIR,
				LOWER(pPlayerList->name[0]),
				capitalize(pPlayerList->name));
			if (stat(buf, &statis))
				continue;
...
void race_sort(RACE_DATA * pRace)
{
	...
	for (fRace = first_race; fRace; fRace = fRace->next) {
		if (pRace->vnum == fRace->vnum
			...
			&& (!fRace->next || pRace->vnum < fRace->next->vnum)
		    )
		    ) {
