...
void do_globedarkness(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		send_to_char(AT_DGREY,
			     "Syntax: globe <create|dissipate|locate>\n\r", ch);
		return;
	}
	if (!str_cmp(argument, "locate")) {
		for (pd = ch->powered; pd; pd = pd->next) {
			if (pd->type == gsn_globedark) {
				sprintf(buf,
					"Globe of Darkness&w, &W%s&w; &cCost&w: &R%d&w.\n\r",
		...
		if (!found)
			send_to_char(AT_CYAN,
				     "You are not sustaining any &zGlobes&w.\n\r",
				     ch);
		return;
	}
	if (!str_cmp(argument, "dissipate")) {
		if (!is_raffected(ch->in_room, gsn_globedark)) {
			send_to_char(AT_CYAN,
				     "There is no &zGlobe &cin this room&w.\n\r",
				     ch);
			return;
		}
		for (pd = ch->powered; pd; pd = pd->next) {
			...
			if (pd->type == gsn_globedark) {
				...
				if (pd->room == ch->in_room) {
					send_to_char(AT_DGREY,
						     "You wave your hand and the globe dissipates.\n\r",
						     ch);
					act(AT_DGREY,
					    "The globe of darkness dissipates.",
					    ch, NULL, NULL, TO_ROOM);
					raffect_remove(ch->in_room, ch,
						       pd->raf);
					return;
				}
			}
		...
		send_to_char(AT_CYAN,
			     "You are not powering the &zGlobe&c in this room.\n\r",
			     ch);
		return;
	}
	if (!str_cmp(argument, "create")) {
		if (is_raffected(ch->in_room, gsn_globedark)) {
			send_to_char(AT_GREY,
				     "A globe of darkness already sits in this room.\n\r",
				     ch);
			return;
		}
		...
		globe->description =
		    str_dup("A &zGlobe of Darkness &Xsits upon the room.");
		...
		send_to_char(AT_DGREY,
			     "You raise your hand and place of globe of darkness upon the area!\n\r",
			     ch);
		act(AT_DGREY,
		    "$n raises his hand and everything goes pitch black!", ch,
		    NULL, NULL, TO_ROOM);
		return;
	}
	do_globedarkness(ch, "");
