		     ...
		     OBJ_DATA * obj, void *vo, CHAR_DATA * rndm)
{
	...
	*argpt = '\0';
	...
	if (!str_cmp(buf, "isnpc")) {
		...
		case 'r':
			if (rndm)
				...
				return -1;
		default:
			sprintf(log_buf, "Mob: %d bad argument to 'isnpc' (%s)",
				mob->pIndexData->vnum, arg);
			bug(log_buf, 0);
			return -1;
		}
	...
	if (!str_cmp(buf, "isalign")) {
		...
		default:
			sprintf(log_buf,
				"Mob: %d bad argument to 'isalignment' (%s)",
				mob->pIndexData->vnum, arg);
			bug(log_buf, 0);
			return -1;
		}
	...
	if (!str_cmp(buf, "isgood")) {
		...
		default:
			sprintf(log_buf,
				"Mob: %d bad argument to 'isgood' (%s)",
				mob->pIndexData->vnum, arg);
			bug(log_buf, 0);
			return -1;
		}
	...
	if (!str_cmp(buf, "isevil")) {
		...
		default:
			sprintf(log_buf,
				"Mob: %d bad argument to 'isevil' (%s)",
				mob->pIndexData->vnum, arg);
			bug(log_buf, 0);
			return -1;
		}
	...
	if (!str_cmp(buf, "isneutral")) {
		...
		default:
			sprintf(log_buf,
				"Mob: %d bad argument to 'isneutral' (%s)",
				mob->pIndexData->vnum, arg);
			bug(log_buf, 0);
			return -1;
		}
	...
	if (!str_cmp(buf, "sex")) {
		if (is_number(val))
			...
			bug("Mob: %d bad sex for argument to 'sex'",
			    mob->pIndexData->vnum);
			return -1;
		}
	...
	if (!str_cmp(buf, "position")) {
		if (is_number(val))
			...
			bug("Mob: %d bad position argument sent to 'position'",
			    mob->pIndexData->vnum);
			return -1;
		}
		       ...
		       CHAR_DATA * rndm)
{
	...
	while (loopdone == FALSE) {	/*scan over any existing or statements */
		...
		while (*cmnd == ' ')
			cmnd++;
		if (*cmnd == '\0') {
			bug("Mob: %d no commands after IF/OR",
			    mob->pIndexData->vnum);
			return null;
		}
	...
	if (flag)
		for (;;) {	/*ifcheck was true, do commands but ignore else to endif */
			if (!str_cmp(buf, "if")) {
				com_list =
				    mprog_process_if(morebuf, com_list, mob,
						     actor, obj, vo, rndm);
				while (*cmnd == ' ')
					cmnd++;
				if (*com_list == '\0')
			...
			while (*cmnd == ' ')
				cmnd++;
			if (*cmnd == '\0') {
				bug("Mob: %d missing else or endif",
				    mob->pIndexData->vnum);
				return null;
			}
			morebuf = one_argument(cmnd, buf);
...
/*       &&  vch->level < MAX_LEVEL-3 */
	...
	while (*cmnd != '\0') {
		...
		if (!str_cmp(buf, "if"))
			command_list =
			    mprog_process_if(morebuf, command_list, mob, actor,
					     obj, vo, rndm);
		else
			mprog_process_cmnd(cmnd, mob, actor, obj, vo, rndm);
		cmnd = command_list;
	...
	if (IS_NPC(mob) && (mob->pIndexData->progtypes & BRIBE_PROG)) {
		...
		for (mprg = mob->pIndexData->mobprogs; mprg != NULL;
			...
			if ((mprg->type & BRIBE_PROG)
			    &&
			    ((amount->gold +
			      (amount->silver / SILVER_PER_GOLD) +
			      (amount->copper / COPPER_PER_GOLD)) >=
			     atoi(mprg->arglist))) {
...
void mprog_speech_trigger(char *txt, CHAR_DATA * mob)
{
	...
	for (vmob = mob->in_room->people; vmob != NULL;
	     vmob = vmob->next_in_room)
		if (IS_NPC(vmob) && (vmob->pIndexData->progtypes & SPEECH_PROG)) {
			mprog_wordlist_check(txt, vmob, mob, NULL, NULL,
					     SPEECH_PROG);
		}
