...
void advance_level(CHAR_DATA * ch)
{
	...
	add_hp =
	    ...
	    number_range(class_table[prime_class(ch)].hp_min,
	...
	if (IS_NPC(ch)) {
		...
		case POS_SLEEPING:
			gain += (ch->pcdata->learned[gsn_fastheal] > 0
				 && ch->pcdata->learned[gsn_fastheal] >
				 number_percent())? get_curr_con(ch) *
			    2 : get_curr_con(ch);
			break;
		case POS_RESTING:
			gain += (ch->pcdata->learned[gsn_fastheal] > 0
		...
		default:
			gain += (ch->pcdata->learned[gsn_fastheal] > 0
				 && ch->pcdata->learned[gsn_fastheal] >
				 number_percent())? get_curr_con(ch) : 0;
		}
...
/* Ward of Healing */
	if (is_raffected(ch->in_room, gsn_ward_heal)) {
		send_to_char(AT_WHITE,
			     "The wards of healing soothe your wounds.\n\r",
			     ch);
		gain += 100;
	}
...
static void mobile_update(void)
{
	...
	for (ch = char_list; ch; ch = ch->next) {
		...
		if (!IS_NPC(ch)
		    || !ch->in_room || IS_AFFECTED(ch, AFF_STUN)
		...
		if (IS_SET(ch->act, ACT_SCAVENGER)
		    && ch->in_room->contents && number_bits(2) == 0) {
			OBJ_DATA *obj;
			OBJ_DATA *obj_best;
			int max;
			...
			if (obj_best) {
				...
				act(AT_GREY, "$n gets $p.", ch, obj_best, NULL,
				    TO_ROOM);
			}
		}
		...
		if (!IS_SET(ch->act, ACT_SENTINEL)
		    ...
		    && !ch->hunting && (!IS_SET(ch->act, ACT_STAY_AREA)
					|| pexit->to_room->area ==
					ch->in_room->area)) {
...
static void weather_update(void)
{
	...
	case 11:
		...
		strcat(buf,
		       "&YThe sun sits high in its heavenly cradle.&w\n\r");
		break;
	...
	case 18:
		...
		strcat(buf,
		       "&cThe sun begins its slow descent in the west.&w\n\r");
		break;

	case 19:
		weather_info.sunlight = SUN_DARK;
		strcat(buf,
		       "&cDarkness spreads its embrace about the land.&w\n\r");
		break;
	...
	for (ch = char_list; ch; ch = ch->next) {
		...
		if (!IS_NPC(ch) && (ch->level < LEVEL_IMMORTAL
			...
			if ((obj = get_eq_char(ch, WEAR_LIGHT))
				...
				if (--obj->value[2] == 0 && ch->in_room) {
					--ch->in_room->light;
					act(C_DEFAULT, "$p goes out.", ch, obj,
					    NULL, TO_ROOM);
					act(C_DEFAULT, "$p goes out.", ch, obj,
					    NULL, TO_CHAR);
					extract_obj(obj);
		...
		if (ch->gspell && --ch->gspell->timer <= 0) {
			send_to_char(AT_BLUE,
				     "You slowly lose your concentration.\n\r",
				     ch);
			end_gspell(ch);
		}
...
static void obj_update(void)
{
	...
	for (obj = object_list; obj; obj = obj_next) {
		...
		if (--obj->timer == 0) {
			...
			if (obj->carried_by) {
				act(C_DEFAULT, message, obj->carried_by, obj,
				    NULL, TO_CHAR);
			} else if (obj->in_room && (rch = obj->in_room->people)) {
				act(C_DEFAULT, message, rch, obj, NULL,
				    TO_ROOM);
				act(C_DEFAULT, message, rch, obj, NULL,
				    TO_CHAR);
			}
		...
		for (mch = ch->in_room->people; mch; mch = mch->next_in_room) {
			...
			if (IS_NPC(mch) && mch->mpactnum > 0
				...
				for (tmp_act = mch->mpact; tmp_act != NULL;
					...
					mprog_wordlist_check(tmp_act->buf, mch,
							     tmp_act->ch,
							     tmp_act->obj,
							     tmp_act->vo,
							     ACT_PROG);
			...
			count = 0;
			victim = NULL;
			for (vch = mch->in_room->people; vch;
...
static void arena_update()
{
	...
	case 3:
		if (arena.och) {
			...
			sprintf(buf,
				"&C%s &coffering &C%s &ca challenge for &W%d &ccoins.",
	...
	if (arena.och)
		...
		challenge
		    ("&C%s&c's challenge not accepted.  Opening arena for new challenger.",
		     (int)(arena.cch->name), 0);
	send_to_char(C_DEFAULT,
		     "Your challenge was not accepted.  Refunding "
		     "award money.\n\r", arena.cch);
...
static void auc_update()
{
...
#else
		sprintf(buf, "%s for %d gold coins (going ONCE).",
			auc_obj->short_descr, auc_cost);
#endif
		auc_channel(buf);
		sprintf(buf, "%s auctioning %s.", auc_held->name,
			auc_obj->name);
	...
	case 2:
		...
		sprintf(buf, "%s for %s (going TWICE)", auc_obj->short_descr,
			money_string(&auc_cost));
#else
		sprintf(buf, "%s for %d gold coins (going TWICE).",
			auc_obj->short_descr, auc_cost);
#endif
		auc_channel(buf);
		sprintf(buf, "%s auctioning %s.", auc_held->name,
	...
	case 3:
		...
		sprintf(buf, "%s for %s (going THRICE)", auc_obj->short_descr,
			money_string(&auc_cost));
#else
		sprintf(buf, "%s for %d gold coins (going THRICE).",
			auc_obj->short_descr, auc_cost);
#endif
		auc_channel(buf);
		sprintf(buf, "%s auctioning %s.", auc_held->name,
	...
	if (auc_bid
	    &&
	    (((auc_bid->money.gold * C_PER_G) +
	      (auc_bid->money.silver * S_PER_G) + (auc_bid->money.copper)) >
	     ((auc_cost.gold * C_PER_G) + (auc_cost.silver * S_PER_G) +
	      (auc_cost.copper)))) {
		sprintf(buf, "%s SOLD! to %s for %s", auc_obj->short_descr,
			auc_bid->name, money_string(&auc_cost));
		add_money(&auc_held->money, &auc_cost);
		spend_money(&auc_bid->money, &auc_cost);

#else
	if (auc_bid && auc_bid->gold >= auc_cost) {
		sprintf(buf, "%s for %d gold coins SOLD! to %s.",
			auc_obj->short_descr, auc_cost, auc_bid->name);
		auc_bid->gold -= auc_cost;
		auc_held->gold += auc_cost;
#endif
		obj_to_char(auc_obj, auc_bid);
		act(AT_DGREEN, "$p appears in your hands.", auc_bid, auc_obj,
		    NULL, TO_CHAR);
		act(AT_DGREEN, "$p appears in the hands of $n.", auc_bid,
		    auc_obj, NULL, TO_ROOM);
	} else if (auc_bid) {
#ifdef NEW_MONEY
		sprintf(buf, "Amount not carried for %s, ending auction.",
			auc_obj->short_descr);
#else
		sprintf(buf,
			"%d gold coins not carried for %s, ending auction.",
			auc_cost, auc_obj->short_descr);
#endif
		obj_to_char(auc_obj, auc_held);
		act(AT_DGREEN, "$p appears in your hands.", auc_held, auc_obj,
		    NULL, TO_CHAR);
		act(AT_DGREEN, "$p appears in the hands of $n.", auc_held,
		    auc_obj, NULL, TO_ROOM);
	} else {
		sprintf(buf, "%s not sold, ending auction.",
			auc_obj->short_descr);
		obj_to_char(auc_obj, auc_held);
		act(AT_DGREEN, "$p appears in your hands.", auc_held, auc_obj,
		    NULL, TO_CHAR);
