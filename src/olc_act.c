...
bool redit_proglist(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		send_to_char(C_DEFAULT,
			     "Syntax:  proglist <all/obj/mob/room>\n\r", ch);
		return FALSE;
	}
	...
	if (!found) {
		if (!str_cmp(arg, "all")) {
			send_to_char(C_DEFAULT,
				     "Programs not found in this area.\n\r",
				     ch);
			return FALSE;
		} else {
			sprintf(buf2,
				"Programs on %ss not found in this area.\n\r",
				arg);
...
bool redit_olist(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		send_to_char(C_DEFAULT,
			     "Syntax:  olist <all/name/item_type/object level>\n\r",
			     ch);
		return FALSE;
	}
...
bool aedit_show(CHAR_DATA * ch, char *argument)
{
	...
	EDIT_AREA(ch, pArea);
	sprintf(buf,
		"                  &W01234567 &B0123456789012345678 &G0123456789\n\r");
	send_to_char(C_DEFAULT, buf, ch);
	...
	if (!stat(buf, &statis)) {
		sprintf(buf, "&WLast modifed on Build: &C%s\r",
			(char *)ctime(&statis.st_mtime));
		send_to_char(C_DEFAULT, buf, ch);
	} else
		send_to_char(C_DEFAULT,
			     "&WLast modifed on Build: &C(No such area)\n\r",
			     ch);

	sprintf(buf, "../../EOS/area/%s", pArea->filename);
	if (!stat(buf, &statis)) {
		sprintf(buf, "&WLast modifed on Storm: &C%s\r",
			(char *)ctime(&statis.st_mtime));
		send_to_char(C_DEFAULT, buf, ch);
	} else
		send_to_char(C_DEFAULT,
			     "&WLast modifed on Storm: &C(No such area)\n\r",
			     ch);
...
bool aedit_llevel(CHAR_DATA * ch, char *argument)
{
	...
	if (!is_number(llevel) || llevel[0] == '\0') {
		send_to_char(C_DEFAULT,
			     "Syntax:  llevel [#lowest sugested level]\n\r",
			     ch);
		return FALSE;
	}
...
bool aedit_ulevel(CHAR_DATA * ch, char *argument)
{
	...
	if (!is_number(ulevel) || ulevel[0] == '\0') {
		send_to_char(C_DEFAULT,
			     "Syntax:  ulevel [#upper most sugested level]\n\r",
			     ch);
		return FALSE;
	}
...
bool aedit_clan_hq(CHAR_DATA * ch, char *argument)
{
	...
	if (get_trust(ch) >= L_CON || !str_cmp(ch->guild->name, "CREATION")) {
		...
		send_to_char(C_DEFAULT,
			     "You are &B*&wtoo&B*&w low of trust to do this.\n\r",
			     ch);
		return FALSE;
	}
	return TRUE;
...
bool aedit_color(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		send_to_char(C_DEFAULT, "Syntax:  color[color number] \n\r",
			     ch);
		send_to_char(C_DEFAULT,
			     "&WType &Rhelp AT_COLOR&W for list of colors.\n\r",
			     ch);
		return FALSE;
	}
	...
	if (!(is_number(argument))) {
		send_to_char(C_DEFAULT,
			     "Color choices must be numeric (1-15).\n\r", ch);
		send_to_char(C_DEFAULT,
			     "&WType &Rhelp AT_COLOR&W for list of colors.\n\r",
			     ch);
		return FALSE;
...
bool aedit_vnum(CHAR_DATA * ch, char *argument)
{
	...
	/* OLC 1.1b */
	if (ilower <= 0 || ilower >= INT_MAX || iupper <= 0
	    || iupper >= INT_MAX) {
...
bool aedit_lvnum(CHAR_DATA * ch, char *argument)
{
	...
	if ((ilower = atoi(lower)) > (iupper = pArea->uvnum)) {
		send_to_char(C_DEFAULT,
			     "AEdit:  Value must be less than the uvnum.\n\r",
			     ch);
		return FALSE;
	}
...
bool redit_show(CHAR_DATA * ch, char *argument)
{
	...
	for (door = 0; door < MAX_DIR; door++) {
		...
		if ((pexit = pRoom->exit[door])) {
			...
			sprintf(buf,
				"&W-%-5s to &z[&W%5d&z] &WKey: &z[&W%5d&z]",
...
bool redit_rdamage(CHAR_DATA * ch, char *argument)
{
	...
	if (!is_number(arg)) {
		send_to_char(AT_WHITE,
			     "Amount must be the number of hp the room is to damage the CH per update.\n\r",
			     ch);
		return FALSE;
	}
...
bool change_exit(CHAR_DATA * ch, char *argument, int door)
{
	...
	if (!str_cmp(command, "delete")) {
		if (!pRoom->exit[door] || pRoom->exit[door]->to_room == 0) {
			send_to_char(C_DEFAULT,
				     "REdit:  Bad Room number 0.. don't delete it :)",
				     ch);
			return FALSE;
		}
		if (!pRoom->exit[door]) {
			send_to_char(C_DEFAULT,
				     "REdit:  Exit does not exist.\n\r", ch);
			return FALSE;
		}
	...
	if (!str_cmp(command, "link")) {
		...
		if (pLinkRoom->exit[rev]) {
			send_to_char(C_DEFAULT,
				     "REdit:  Remote side's exit exists.\n\r",
				     ch);
			return FALSE;
		}
 ...
 */
		if (arg[0] == '\0') {
			...
			pArea = ch->in_room->area;
			for (value = pArea->lvnum; pArea->uvnum >= value;
			     value++) {
				if (!get_room_index(value))
					break;
			}
			if (value > pArea->uvnum) {
				send_to_char(C_DEFAULT,
					     "REdit:  No free room vnums in this area.\n\r",
					     ch);
				return FALSE;
			}
			sprintf(arg, "%d", value);
	...
	if (!str_cmp(command, "key")) {
		...
		if (!(pObjIndex = get_obj_index(atoi(arg)))) {
			send_to_char(C_DEFAULT,
				     "REdit:  Item does not exist.\n\r", ch);
			return FALSE;
		}

		if (pObjIndex->item_type != ITEM_KEY) {
			send_to_char(C_DEFAULT,
				     "REdit:  Item is not a key.\n\r", ch);
			return FALSE;
		}
...
/*     one_argument( argument, keyword );   */
	...
	if (!str_cmp(command, "format")) {
		...
		if (strlen(ed->description) >= (MAX_STRING_LENGTH - 4)) {
			send_to_char(C_DEFAULT,
				     "String too long to be formatted.\n\r",
				     ch);
			return FALSE;
		}
...
bool redit_delet(CHAR_DATA * ch, char *argument)
{
	send_to_char(AT_GREY,
		     "If you want to DELETE the room, spell it out.\n\r", ch);
	return FALSE;
}
...
bool redit_delete(CHAR_DATA * ch, char *argument)
{
	...
	if (pRoom == prev) {
		...
		if (!prev) {
			bug("redit_delete: room %d not found.", pRoom->vnum);
		}
	}
...
/*	newobj = create_object( pObjIndex, number_fuzzy( olevel ) );  */
	...
	if (argument[0] == '\0'
		...
		newobj = create_object(pObjIndex, number_fuzzy(olevel));
#ifdef NEW_MONEY
		newobj->cost.gold = newobj->cost.silver = newobj->cost.copper =
		    0;
#else
		newobj->cost = 0;
#endif
	...
	if ((to_mob = get_char_room(ch, arg2)) != NULL) {
		...
		for (pMob = ch->in_room->reset_first; pMob; pMob = pMob->next) {
			++reset_loc;
			if (pMob->arg1 == to_mob->pIndexData->vnum
			    && ++counter == mob_num)
				break;
		}
		if (!pMob) {
			send_to_char(C_DEFAULT,
				     "Mobile not reset in this room.\n\r", ch);
			return FALSE;
		}
...
void show_obj_values(CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
	...
	case ITEM_LIGHT:
		if (obj->value[2] == -1)
			sprintf(buf,
				"&z[&Wv2&z] &cLight&w: &WInfinite&z[&R-1&z]\n\r");
		else
	...
	case ITEM_NOTEBOARD:
		sprintf(buf,
			...
			obj->value[0], get_obj_index(obj->value[0])
			? get_obj_index(obj->value[0])->short_descr
			: "none", obj->value[1], obj->value[2]);
		send_to_char(C_DEFAULT, buf, ch);
		break;
...
bool set_ac_vnum(CHAR_DATA * ch, char *argument)
{
	...
	case 0:
		send_to_char(AT_GREY,
			     "Obj ac_type does not require a vnum.\n\r", ch);
		return FALSE;
	case 1:
...
bool set_ac_v2(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0' || !is_number(argument)) {
		send_to_char(AT_WHITE,
			     "Syntax:  ac_v2 [ max charges (-1 unlimited) ]\n\r",
			     ch);
		return FALSE;
	}
	value = atoi(argument);
...
bool set_ac_setspell(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0'
	    || spn == -1 || (*skill_table[spn].spell_fun) == (*spell_null)) {
		send_to_char(AT_WHITE,
			     "Syntax:  ac_setspell [ valid spell name ]\n\r",
			     ch);
		return FALSE;
	}
		    ...
		    char *argument)
{
	...
	case ITEM_STAFF:
		...
		case 1:
			send_to_char(C_DEFAULT,
				     "TOTAL NUMBER OF CHARGES SET.\n\r\n\r",
				     ch);
			pObj->value[1] = atoi(argument);
			break;
		case 2:
			send_to_char(C_DEFAULT,
				     "CURRENT NUMBER OF CHARGES SET.\n\r\n\r",
				     ch);
			pObj->value[2] = atoi(argument);
			break;
		case 3:
	...
	case ITEM_NOTEBOARD:
		...
		case 1:
			if (atoi(argument) > get_trust(ch)) {
				send_to_char(C_DEFAULT,
					     "Limited by your trust.\n\r", ch);
				return FALSE;
			}
			send_to_char(C_DEFAULT,
				     "MINIMUM READ LEVEL SET.\n\r\n\r", ch);
			pObj->value[1] = atoi(argument);
			break;
		case 2:
			if (atoi(argument) > get_trust(ch)) {
	...
	case ITEM_BLOOD:
		...
		case 0:
			send_to_char(C_DEFAULT,
				     "MAXIMUM AMOUT OF BLOOD SET.\n\r\n\r", ch);
			pObj->value[0] = atoi(argument);
			break;
		case 1:
			send_to_char(C_DEFAULT,
				     "CURRENT AMOUNT OF BLOOD SET.\n\r\n\r",
				     ch);
			pObj->value[1] = atoi(argument);
			break;
		}
...
bool oedit_show(CHAR_DATA * ch, char *argument)
{
	...
	if (pObj->join) {
		...
		sprintf(buf, "&cJoins to create&w: &z[&R%d&z] "
			"[&W%s&z]\n\r", pObj->join,
			pJoinObj ? pJoinObj->
			short_descr : "&RError: No such object.");
		send_to_char(C_DEFAULT, buf, ch);
	}
	if (pObj->sep_one) {
...
bool oedit_addaffect(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(flag_string(apply_flags, flag_value(apply_flags, loc)),
		     "none")) {
...
bool oedit_create(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		...
		if (value > pArea->uvnum) {
			send_to_char(C_DEFAULT,
				     "OEdit:  No free object vnums in this area.\n\r",
				     ch);
			return FALSE;
		}
	}
	...
	if (get_obj_index(value)) {
		send_to_char(C_DEFAULT,
			     "OEdit:  Object vnum already exists.\n\r", ch);
		return FALSE;
	}
...
{
	send_to_char(AT_GREY,
		     "If you want to DELETE this object, spell it out.\n\r",
		     ch);
	return FALSE;
}
...
bool oedit_delete(CHAR_DATA * ch, char *argument)
{
	...
	if (pObj == prev) {
		...
		if (!prev) {
			bug("oedit_delete: obj %d not found.", pObj->vnum);
		}
	}
...
/*    sprintf( buf,
	...
	if IS_SET
		...
		if ((pObjIndex = get_obj_index(pClan->obj_vnum_1)))
			...
			sprintf(buf,
				"&YLevel 50:&w  [%5d] &RNo such object!!\n\r",
				pClan->obj_vnum_1);
		send_to_char(C_DEFAULT, buf, ch);
		if ((pObjIndex = get_obj_index(pClan->obj_vnum_2)))
			...
			sprintf(buf,
				"&YLevel 75:&w  [%5d] &RNo such object!!\n\r",
				pClan->obj_vnum_1);
		send_to_char(C_DEFAULT, buf, ch);
		if ((pObjIndex = get_obj_index(pClan->obj_vnum_3)))
			...
			sprintf(buf,
				"&YLevel 100:&w [%5d] &RNo such object!!\n\r",
				pClan->obj_vnum_1);
		send_to_char(C_DEFAULT, buf, ch);
	} else {
		if ((pObjIndex = get_obj_index(pClan->obj_vnum_1)))
			...
			sprintf(buf,
				"&YLevel 30:&w  [%5d] &RNo such object!!\n\r",
				pClan->obj_vnum_1);
		send_to_char(C_DEFAULT, buf, ch);
		if ((pObjIndex = get_obj_index(pClan->obj_vnum_2)))
			...
			sprintf(buf,
				"&YLevel 65:&w  [%5d] &RNo such object!!\n\r",
				pClan->obj_vnum_1);
		send_to_char(C_DEFAULT, buf, ch);
		if ((pObjIndex = get_obj_index(pClan->obj_vnum_3)))
			sprintf(buf, "&YLevel 100:&w [%5d] %s\n\r",
...
bool cedit_power(CHAR_DATA * ch, char *argument)
{
	...
	if (number[0] == '\0') {
		...
		send_to_char(AT_WHITE,
			     "To clear a position, in the <name> field type: none",
			     ch);
		return FALSE;
	}
...
bool cedit_induct(CHAR_DATA * ch, char *argument)
{
	...
	if ((argument[0] == '\0') || ((value < 1) || (value > 4))) {
		send_to_char(C_DEFAULT, "Sets which clan position can induct.",
			     ch);
...
bool medit_show(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf,
		...
		"&cSilver&w:&z	       [&R%7d&z]\n\r"
		"&cCopper&w:&z	       [&R%7d&z]\n\r",
		pMob->hitnodice, pMob->money.gold, pMob->money.silver,
		pMob->money.copper);
	send_to_char(C_DEFAULT, buf, ch);
#else
	sprintf(buf,
...
bool medit_create(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		...
		if (value > pArea->uvnum) {
			send_to_char(C_DEFAULT,
				     "MEdit:  No free mob vnums in this area.\n\r",
				     ch);
			return FALSE;
		}
	}
...
{
	send_to_char(AT_GREY,
		     "If you want to DELETE this mob, spell it out.\n\r", ch);
	return FALSE;
}
...
bool medit_delete(CHAR_DATA * ch, char *argument)
{
	...
	if (pMob == prev) {
		...
		if (!prev) {
			bug("medit_delete: mob %d not found.", pMob->vnum);
		}
	}
...
bool medit_shop(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(command, "type")) {
		...
		if ((value = flag_value(type_flags, argument)) == NO_FLAG) {
			send_to_char(C_DEFAULT,
				     "REdit:  That type of item is not known.\n\r",
				     ch);
			return FALSE;
		}
...
bool tedit_show(CHAR_DATA * ch, char *argument)
{
	...
	case ED_RPROG:
		sprintf(buf, "RoomProg type: %s\n\r",
			flag_string(rprog_types, pTrap->type));
		break;
	case ED_EPROG:
		sprintf(buf, "ExitProg type: %s\n\r",
			flag_string(eprog_types, pTrap->type));
		break;
	default:
...
bool mpedit_create(CHAR_DATA * ch, char *argument)
{
	...
	if (!pMob->mobprogs)
		...
		for (pMLast = pMob->mobprogs; pMLast->next;
		     pMLast = pMLast->next) ;
		pMLast->next = pMProg;
	}
...
bool tedit_create(CHAR_DATA * ch, char *argument)
{
	...
	if (!*pFirst)
		*pFirst = pTrap;
		...
		for (pLast = *pFirst; pLast->next_here;
		     pLast = pLast->next_here) ;
...
bool mpedit_arglist(CHAR_DATA * ch, char *argument)
{
	...
	if (pMProg->type == RAND_PROG && prc > 95) {
		send_to_char(C_DEFAULT,
			     "You can't set the percentage that high on a rand_prog.\n\r",
			     ch);
		return FALSE;
	}
...
bool rename_show(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char(AT_WHITE,
		     "REMAKE AN ITEM: Type done when finished.\n\r\n\r", ch);
	sprintf(buf,
		"&Y1) &WKeyword(s):        &zSyntax: &R1 <new keyword>\n\r&B[&G%s&B]\n\r",
		pObj->name);
	send_to_char(C_DEFAULT, buf, ch);
	sprintf(buf,
		"&Y2) &WShort:             &zSyntax: &R2 <Looks in inv or equiped.>\n\r&B[&G%s&B]\n\r",
		pObj->short_descr);
	send_to_char(C_DEFAULT, buf, ch);
	sprintf(buf,
		"&Y3) &WLong:              &zSyntax: &R3 <Looks when on the ground or looked at.>\n\r&B[&G%s&B]\n\r",
		pObj->description);
	send_to_char(C_DEFAULT, buf, ch);
...
bool forge_show(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char(AT_GREY,
		     "Forging an item; type &RDONE &wonly when finished.\n\r",
		     ch);
	...
	sprintf(buf,
		"[&WStat 1&w:       &R+%d&z] [&WDamroll&w:      &R+%d&z] [&WHitroll&w:       &R+%d&z]\n\r",
		max_stat, max_dam, max_hit);
	send_to_char(AT_DGREY, buf, ch);
	sprintf(buf,
		"[&WHit Points&w: &R+%d&z] [&WMana&w:        &R+%d&z] [&WArmor Class&w:  &R%d&z]",
	...
	if (pObj->level >= 40) {
		sprintf(buf,
			"[&WSaving-Spell&w: &R%d&z] [&WSaving-Breath&w: &R%d&z]\n\r",
			max_saves, max_saveb);
		send_to_char(AT_DGREY, buf, ch);
	}
	if (pObj->level >= 45) {
	...
	send_to_char(AT_WHITE, "#&w- &z[&W  affect  &z] [&Wmodifier&z]\n\r",
		     ch);
	for (cnt = 1, paf = pObj->affected; cnt <= pObj->level / 10;
	     cnt++, paf = paf_next) {
		if (cnt == 7)
			break;
		if (paf) {
...
bool forge_addaffect(CHAR_DATA * ch, char *argument)
{
	...
	if (!legal) {
		sprintf(buf,
			"Unknown affect %s, please choose from the list.\n\r",
			loc);
		send_to_char(AT_GREY, buf, ch);
		return FALSE;
	}
	...
	for (pAf = pObj->affected; pAf; pAf = pAf->next) {
		...
		if (cnt >= pObj->level / 10 || cnt >= 6) {
			send_to_char(AT_GREY,
				     "You can no longer add anything to this item.\n\r",
				     ch);
			return FALSE;
		}
		if (pAf->location == flag_value(apply_flags, loc)) {
			sprintf(buf,
				"You have already added %d %s to this item.\n\r",
				pAf->modifier, loc);
			send_to_char(AT_GREY, buf, ch);
			return FALSE;
		}
		if (pAf->location == APPLY_STR
			...
			stat_cnt++;
		if (stat_cnt >= max_statn && (!str_cmp(loc, "strength")
					      || !str_cmp(loc, "dexterity")
					      || !str_cmp(loc, "intelligence")
					      || !str_cmp(loc, "wisdom")
					      || !str_cmp(loc, "constitution"))) {
			send_to_char(AT_GREY,
				     "You have already added the maximum number of stats possible for your experience.\n\r",
				     ch);
			return FALSE;
		}
	}
...
bool forge_type(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		sprintf(buf, "Legal values:\n\r%s %s %s %s %s %s %s\n\r",
			weapon_flags[0].name, weapon_flags[1].name,
			weapon_flags[2].name, weapon_flags[3].name,
			weapon_flags[4].name, weapon_flags[5].name,
			weapon_flags[6].name);
		send_to_char(AT_GREY, buf, ch);
		sprintf(buf, "%s %s %s %s %s %s %s\n\r",
			weapon_flags[7].name, weapon_flags[8].name,
			weapon_flags[9].name, weapon_flags[10].name,
			weapon_flags[11].name, weapon_flags[12].name,
...
void do_opstat(CHAR_DATA * ch, char *argument)
{
	...
	for (pTrap = obj->pIndexData->traps; pTrap != NULL;
	     pTrap = pTrap->next_here) {
		sprintf(buf, ">%s %s\n\r%s\n\r\n\r",
			flag_string(oprog_types, pTrap->type), pTrap->arglist,
			pTrap->comlist);
		send_to_char(C_DEFAULT, buf, ch);
...
bool mreset_show(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char(C_DEFAULT,
		     "&z[ &G#&z] [&RMVnum&z] [&YNum&z] &BMob Short Description\n\r",
		     ch);
	...
	for (pReset = pRoom->reset_first; pReset; pReset = pReset->next) {
		if (pReset->command == 'M') {
			...
			sprintf(buf, "&z[&G%2d&z] [&R%5d&z] [&Y%3d&z] &B%s\n\r",
				...
				pMobIndex ? pMobIndex->
				short_descr : "&RMob does not exist!!");
			send_to_char(C_DEFAULT, buf, ch);
		}
	}
	return FALSE;
...
bool spedit_show(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "Target      [%s]   What the Target is.\n\r",
		"flag_string(pSpell->target)");
	send_to_char(C_DEFAULT, buf, ch);
	sprintf(buf,
		"Position    [%s]   What Position you have to be in at least.\n\r",
		"flag_string(pSpell->minimum_position)");
	send_to_char(C_DEFAULT, buf, ch);
...
bool spedit_dispelable(CHAR_DATA * ch, char *argument)
{
	...
	if (pSpell->dispelable)
		...
		send_to_char(C_DEFAULT, "Spell switched to non dispelable.",
			     ch);
...
bool race_edit_name(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0' || strlen(argument) != 3) {
		send_to_char(C_DEFAULT,
			     "Syntax:  name [string] (string has to be 3 characters long)\n\r",
			     ch);
		return FALSE;
	}
...
bool race_edit_full(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0' || strlen(argument) > 20) {
		send_to_char(C_DEFAULT,
			     "Syntax:  full [string] (String can't be more than 20 characters long.)\n\r",
			     ch);
		return FALSE;
	}
