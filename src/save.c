...
/* 30 */
	...
	for (paf = ch->affected; paf; paf = paf->next) {
		...
		fprintf(fp, "Aff       %3d %3d %3d %3d %10d %3d\n",
			...
			paf->duration,
			paf->modifier,
			paf->location, paf->bitvector, paf->level);
	}
		...
		bool storage)
{
	...
	if (ch->level < obj->level
	    || obj->item_type == ITEM_KEY || obj->deleted)
		return;
	...
	fprintf(fp, "Activates    %d %d %d %d\n",
		obj->ac_type, obj->ac_vnum, obj->ac_charge[0],
		obj->ac_charge[1]);
	...
	for (;;) {
		...
		case 'A':
			...
			if (!str_cmp(word, "Alias")) {
				...
				fMatch = TRUE;
				if (!ch->pcdata) {
					bug("Fread_char: Alias without pcdata",
					    0);
					fread_string(fp);
					fread_string(fp);
					break;
		...
		case 'D':
			...
			KEY("Detract", ch->pcdata->detractments,
			    fread_string(fp));
		...
		case 'T':
			...
			KEY("Trnsfrom", ch->pcdata->transfrom,
			    fread_string(fp));
	...
	*obj = obj_zero;
	...
	for (;;) {
		...
		case 'A':
			...
			KEY("AntiClassF", obj->anti_class_flags,
			    fread_number(fp));
			KEY("AntiRaceF", obj->anti_race_flags,
			    fread_number(fp));
...
static void save_pet(CHAR_DATA * ch, FILE * fp, CHAR_DATA * pet)
{
	...
	fprintf(fp, "%d %d %d %d %d %ld %ld %d %s~ %s~\n",
		pet->pIndexData->vnum, pet->hit, pet->perm_hit, pet->mod_hit,
		pet->act, pet->affected_by, pet->affected_by2, pet->level,
		pet->short_descr, pet->long_descr);
...
void corpse_back(CHAR_DATA * ch, OBJ_DATA * corpse)
{
	...
	for (obj = corpse->contains; obj; obj = obj_next) {
		...
		if (obj->contains) {	/* get stuff in containers */
			for (obj_nest = obj->contains; obj_nest;
			     obj_nest = objn_next) {
...
void save_banlist(BAN_DATA * banlist)
{
	...
	for (ban = ban_list; ban; ban = ban->next) {
		if (ban->user)
			sprintf(buf, "%c %s@%s", ban->type, ban->user,
				ban->name);
		else
