...
static void generate_quest(CHAR_DATA * ch, CHAR_DATA * questman)
{
	...
	for (vsearch = char_list; vsearch; vsearch = vsearch_next) {
		...
		if ((level_diff <= 3 && level_diff > -5)
		    ...
		    &&
		    (!IS_SET
		     (vsearch->in_room->area->area_flags, AREA_PROTOTYPE))
		    &&
...
/*	sprintf(buf, "Look in the general area of %s...\n\r", room->area->name);
	...
	/* Quest to kill a mob */
		...
		case 0:
			sprintf(buf,
				"An enemy of mine, %s, is making vile threats against the Oracle!",
				victim->short_descr);
			do_say(questman, buf);
		...
		case 1:
			sprintf(buf,
				"EOSII's most heinous criminal, %s, has escaped from the dungeon!",
				victim->short_descr);
			do_say(questman, buf);
			sprintf(buf,
...
void quest_update(void)
{
	...
	for (ch = char_list; ch; ch = ch_next) {
		...
		if (ch->nextquest > 0) {
			...
			if (ch->questmob
			    && ch->questmob->deleted && ch->countdown > 1) {
