...
void do_raffect(CHAR_DATA * ch, char *argument)
{
	...
	if (!ch->powered)
		send_to_char(AT_CYAN,
			     "You are not powering any room affects.\n\r", ch);

	for (pd = ch->powered; pd; pd = pd->next) {
		sprintf(buf,
			"%s&w, &cMana Cost&w: &R%d&w, &cRoom&w: &W%s&w.\n\r",
			skill_table[pd->type].name, pd->cost, pd->room->name);
		buf[0] = UPPER(buf[0]);
	...
	for (pd = ch->powered; pd; pd = pd->next) {
		...
		if (pd->type == gsn_raf) {
			...
			if (pd->room == ch->in_room) {
				sprintf(buf, "%s\n\r",
					skill_table[gsn_raf].noun_damage);
				send_to_char(AT_WHITE, buf, ch);
...
void do_ward(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		send_to_char(AT_WHITE,
			     "Syntax: ward <type> <on|off|locate>\n\r", ch);
		return;
	}
	...
	if (type == gsn_ward_heal && is_raffected(ch->in_room, gsn_ward_heal)) {
		send_to_char(AT_WHITE,
			     "This room is already warded to heal.\n\r", ch);
		return;
	}
