...
void close_socket(DESCRIPTOR_DATA * dclose)
{
	...
	if ((ch = dclose->original)) {
		if (!dclose->character) {
			sprintf(log_buf,
				"Close_socket: original without character: %s",
	...
	if ((ch = dclose->character)) {
		sprintf(log_buf, "Closing link to %s.",
			(ch->name ? ch->name : "(unknown)"));
		log_string(log_buf, CHANNEL_LOG, -1);
		if ((dclose->connected == CON_PLAYING
		     || dclose->connected == CON_CHATTING) && ch->in_room) {
			if (ch->name)
				act(AT_GREEN, "$n has lost $s link.", ch, NULL,
				    NULL, TO_ROOM);
			else
				act(AT_GREEN, "Someone has lost their link.",
				    ch, NULL, NULL, TO_ROOM);
			wiznet("Netdeath has claimed $N.", ch, NULL, WIZ_LINKS,
			       0, 0);
	...
	for (str = txt; (str - txt) < length; str++) {
		...
		if (cl < 16) {
			if (d->ansi) {
				...
				sprintf(&ntxt[cn], "\033[0;%d;%dm", (cl > 7),
					(cl & 7) + 30);
				cn += 9;
			}
			continue;
...
void nanny(DESCRIPTOR_DATA * d, char *argument)
{
	...
	case CON_GET_NAME:
		...
		if (check_reconnect(d, argument, FALSE)) {
			...
			if (wizlock && !IS_HERO(ch) && !ch->wizbit) {
				write_to_buffer(d,
						"&cThe game is open for Imms only right now. Come back later..\n\r",
						0);
				close_socket(d);
				return;
			}
			if (ch->level <= numlock && !ch->wizbit && numlock != 0) {
				...
				if (ch->level == 0) {
					...
					write_to_buffer(d,
							...
							0);
					write_to_buffer(d,
							"Please email storm@netway.com to ",
							0);
					write_to_buffer(d,
	...
	case CON_GET_OLD_PASSWORD:
		...
		if (ch->level != L_IMP) {
			...
			wiznet(log_buf, NULL, NULL, WIZ_SITES, 0,
			       get_trust(ch));
		}
		if (IS_HERO(ch))
			do_help(ch, "imotd");
	...
	case CON_CONFIRM_NEW_NAME:
		...
		case 'n':
		case 'N':
			write_to_buffer(d, "&WOk&w, &Wwhat IS it&w, &Wthen&w? ",
					0);
			free_char(d->character);
			d->character = NULL;
			d->connected = CON_GET_NAME;
		...
		default:
			write_to_buffer(d, "&cPlease type &WYes &cor &WNo&w: ",
					0);
			break;
		}
		break;
	...
	case CON_CONFIRM_NEW_PASSWORD:
		...
		write_to_buffer(d, echo_on_str, 0);
		write_to_buffer(d, "&cWhat is your sex &w(&WM&w/&WF&w/&WN&w)? ",
				0);
	...
	case CON_GET_NEW_RACE:
		for (iClass = 0; iClass < MAX_RACE; iClass++) {
			if ((!str_prefix
			     (argument, (get_race_data(iClass))->race_name))
			    ||
			    (!str_cmp
			     (argument, (get_race_data(iClass))->race_full))) {
				ch->race = iClass;
				break;
			}
		}
	...
	case CON_WANT_MULTI:
		...
		case 'N':
			ch->class[1] = -1;
			write_to_buffer(d,
					"&cPress &W'y' &cto enter a description or &WEnter &cto continue&w ",
					0);
...
*/
	...
	case CON_CHECK_AUTHORIZE:
		...
		ch->start_align = 'N';
		sprintf(log_buf, "%s!%s@%s new player.", ch->name, d->user,
			d->host);
		log_string(log_buf, CHANNEL_LOG, -1);
		write_to_buffer(d, "\n\r", 2);
...
/* if authorize is on:  */
		...
		if (!auth_off) {
			...
			sprintf(log_buf,
				"%s!%s@%s a %s %s %s %s needs be to AUTHORIZED.",
				ch->name, d->user, d->host,
...
/* XOR authorize */
	...
	case CON_AUTHORIZE_NAME3:
		...
		sprintf(log_buf,
			...
			ch->sex == SEX_MALE ? "male" : ch->sex ==
			SEX_FEMALE ? "female" : "neuter",
			(get_race_data(ch->race))->race_full, class_short(ch));
		log_string(log_buf, CHANNEL_LOG, -1);
		wiznet(log_buf, ch, NULL, WIZ_GENERAL, 0, 0);
		d->connected++;
...
/* charisma */
			...
			if (!is_class(ch, CLASS_VAMPIRE))
				ch->prompt =
				    str_dup("<&Y%hhp &C%mm &G%vmv&w> ");
			else
				ch->prompt =
				    str_dup("<&Y%hhp &R%mbp &G%vmv&w> ");
			if (ch->start_align == 'E')
				char_to_room(ch,
					     get_room_index
					     (ROOM_VNUM_RW_SCHOOL));
		...
		if (!IS_SET(ch->act, PLR_WIZINVIS)
		    && !IS_SET(ch->act, PLR_CLOAKED))
			info("%s has returned to the storm.", (int)(ch->name),
			     0);
		...
		if (ch->pcdata->corpses >= 2) {
			...
			sprintf(strfng, "%s%c/%s.cps", PLAYER_DIR,
...
bool check_reconnect(DESCRIPTOR_DATA * d, char *name, bool fConn)
{
	...
	for (ch = char_list; ch; ch = ch->next) {
		if (!IS_NPC(ch) && !ch->deleted && (!fConn || !ch->desc)
			...
			if (fConn == FALSE) {
				...
				act(AT_GREEN, "$n has reconnected.", ch, NULL,
				    NULL, TO_ROOM);
				...
				if (!IS_SET(ch->act, PLR_WIZINVIS)
				    && !IS_SET(ch->act, PLR_CLOAKED))
					info("%s has re-established a link to the storm.", (int)(ch->name), 0);
				if (IS_AFFECTED2(ch, AFF_PLOADED))
					REMOVE_BIT(ch->affected_by2,
						   AFF_PLOADED);
				d->connected = CON_PLAYING;
				/*
	...
	for (dold = descriptor_list; dold; dold = dold->next) {
				...
				? dold->original->name : dold->character->name))
		{
			if (dold->connected == CON_PLAYING) {
				free_char(d->character);
				d->character =
				    (dold->original ? dold->original : dold->
				     character);
				if (dold->original)
				...
				if (!IS_SET(d->character->act, PLR_WIZINVIS))
					act(AT_GREEN,
					    "A ghostly aura briefly embodies $n.",
					    d->character, NULL, NULL, TO_ROOM);
				send_to_char(AT_GREEN,
					     "You arise from netdeath and continue"
					     " playing.\n\r", d->character);
				sprintf(log_buf,
					"%s connects, kicking off old link.",
					d->character->name);
				log_string(log_buf, CHANNEL_LOG,
					   d->character->level);
...
void stop_idling(CHAR_DATA * ch)
{
	...
	ch->was_in_room = NULL;
	act(AT_GREEN, "$n has returned from the void.", ch, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void set_char_color(int AType, CHAR_DATA * ch)
{
	...
	if (ch && ch->desc && ch->desc->ansi) {
		if (AType > 15)
			sprintf(buf, "\033[0;%d;5;%dm",
				(((AType - 16) & 8) == 8),
				30 + ((AType - 16) & 7));
		else
			sprintf(buf, "\033[0;%d;%dm", ((AType & 8) == 8),
				30 + (AType & 7));
...
void show_string(DESCRIPTOR_DATA * d, char *input)
{
	...
	if (lines < 0) {
		for (start = d->showstr_point;
		     start > d->showstr_head && lines < 0; start--)
			if (*start == '\r')
				lines++;
	} else
...
/*	    || ( !to->desc && IS_NPC( to ) )*/
		...
		*point++ = '\0';
		...
		if (to->desc && to->desc->ansi) {
			if (AType > 15)
				sprintf(buf2, "\033[0;%d;5;%dm",
					(((AType - 16) & 8) == 8),
					30 + ((AType - 16) & 7));
			else
				sprintf(buf2, "\033[0;%d;%dm",
					((AType & 8) == 8), 30 + (AType & 7));
			strcat(buf2, buf);
		} else
			strcpy(buf2, buf);
		if (to->desc)
	...
	if (argument[0] != '\0') {
		...
		if (auth_off && !str_cmp(arg1, "on")) {
			if (ch->level > L_DIR) {	/* Level 112+ */
				...
				send_to_char(C_DEFAULT,
					     "You are not authorized to issue this command.\n\r",
					     ch);
			return;
		...
		if (mode == 0) {
			if (d->connected >= CON_AUTHORIZE_NAME
				...
				sprintf(buf, "  %s!%s@%s a %s %s %s %s\n\r",
					...
					IS_EVIL(d->
						character) ? "evil" : "neutral",
					d->character->sex ==
					SEX_MALE ? "male" : d->character->sex ==
					SEX_FEMALE ? "female" : "neuter",
					(get_race_data(d->character->race))->
					race_full, class_short(d->character));
			...
			if (mode == 1) {
				...
				send_to_char(C_DEFAULT, "Character denied.\n\r",
					     ch);
				write_to_buffer(d,
						"Please choose a more medieval name.\n\r",
						0);
...
*/
		if ((!str_suffix(pban->name, dnew->host) && !pban->user)
			...
			sprintf(log_buf, "%s!%s@%s being denied access.",
				dnew->character ? dnew->character->
				name : "(unknown)", dnew->user, dnew->host);
			bug(log_buf, 0);
			close(dnew->descriptor);
			if (loggedin) {
				for (dremove = descriptor_list; dremove;
				     dremove = dremove->next)
...
void imc_channel(const char *buf, int len)
{
	...
	for (d = descriptor_list; d; d = d->next) {
		...
		if (d->connected == CON_PLAYING &&
		    IS_IMMORTAL(och) && !IS_SET(och->deaf, CHANNEL_IMC))
...
void do_hotreboot(CHAR_DATA * ch, char *argument)
{
	...
	if (!(fp = fopen(HOTREBOOT_FILE, "w"))) {
		send_to_char(AT_RED, "Can't write to hotreboot file, aborting.",
			     ch);

		sprintf(buf, "Could not write to hotreboot file: %s",
			HOTREBOOT_FILE);
	...
	if (port == 1045)
		execl("../../src/tmp/envy", "Envy", buf, "hotreboot", buf2,
		      (char *)NULL);
	else
...
void hotreboot_recover()
{
	...
	for (;;) {
		...
		if (!fOld) {	/* Player file not found?! */
			...
			do_look(d->character, "");
			act(AT_WHITE, "$n materializes!", d->character, NULL,
			    NULL, TO_ROOM);
			d->connected = CON_PLAYING;
		}
