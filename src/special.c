...
bool spec_cast_adept(CHAR_DATA * ch)
{
	...
	switch (number_bits(3)) {
	case 0:
		act(C_DEFAULT, "$n utters the word 'rodalaht'.", ch, NULL, NULL,
		    TO_ROOM);
		spell_armor(skill_lookup("armor"), ch->level, ch, victim);
		return TRUE;

	case 1:
		act(C_DEFAULT, "$n utters the word 'igna'.", ch, NULL, NULL,
		    TO_ROOM);
		spell_bless(skill_lookup("bless"), ch->level, ch, victim);
		return TRUE;

	case 2:
		act(C_DEFAULT, "$n utters the word 'naeralkced'.", ch, NULL,
		    NULL, TO_ROOM);
	...
	case 3:
		act(C_DEFAULT, "$n utters the word 'frehs'.", ch, NULL, NULL,
		    TO_ROOM);
	...
	case 4:
		act(C_DEFAULT, "$n utters the words 'ralker'.", ch, NULL, NULL,
		    TO_ROOM);
	...
	case 5:
		act(C_DEFAULT, "$n utters the words 'irehx'.", ch, NULL, NULL,
		    TO_ROOM);
...
bool spec_cast_healer(CHAR_DATA * ch)
{
	...
	case 3:
		act(C_DEFAULT, "$n utters the word 'frehs'.", ch, NULL, NULL,
		    TO_ROOM);
		spell_cure_light(skill_lookup("cure serious"), ch->level, ch,
				 victim);
		return TRUE;

	case 4:
		act(C_DEFAULT, "$n utters the words 'obasi'.", ch, NULL, NULL,
		    TO_ROOM);
		spell_cure_poison(skill_lookup("cure poison"), ch->level, ch,
				  victim);
		return TRUE;
	...
	case 6:
		act(C_DEFAULT, "$n utters the words 'ecnanep'.", ch, NULL, NULL,
		    TO_ROOM);
...
bool spec_fido(CHAR_DATA * ch)
{
	...
	for (corpse = ch->in_room->contents; corpse; corpse = corpse_next) {
		...
		act(C_DEFAULT, "$n savagely devours a corpse.", ch, NULL, NULL,
		    TO_ROOM);
...
bool spec_guard(CHAR_DATA * ch)
{
	...
	for (victim = ch->in_room->people; victim;
		...
		if (victim->fighting && (IS_GOOD(ch) || IS_NEUTRAL(ch))
		    && victim->fighting != ch && victim->alignment < max_evil) {
			max_evil = victim->alignment;
			ech = victim;
		}
		if (victim->fighting && IS_EVIL(ch)
		    && victim->fighting != ch && victim->alignment > max_good) {
	...
	if (victim) {
		...
		sprintf(buf,
			"%s is a %s!  JUSTICE WILL PREVAIL!! DEATH TO %s!!!",
...
bool spec_janitor(CHAR_DATA * ch)
{
	...
	for (trash = ch->in_room->contents; trash; trash = trash_next) {
		...
		if (!IS_SET(trash->wear_flags, ITEM_TAKE))
			continue;
		act(C_DEFAULT, "$n picks up some unsightly garbage.", ch, NULL,
		    NULL, TO_ROOM);
...
bool spec_mayor(CHAR_DATA * ch)
{
	...
	case 'W':
		...
		act(C_DEFAULT, "$n awakens and groans loudly.", ch, NULL, NULL,
		    TO_ROOM);
		break;

	case 'S':
		ch->position = POS_SLEEPING;
		act(C_DEFAULT, "$n lies down and falls asleep.", ch, NULL, NULL,
		    TO_ROOM);
		break;

	case 'a':
		act(C_DEFAULT, "$n says 'Hello Honey!'", ch, NULL, NULL,
		    TO_ROOM);
		break;
	...
	case 'd':
		act(C_DEFAULT, "$n says 'Good day, citizens!'", ch, NULL, NULL,
		    TO_ROOM);
		break;

	case 'e':
		act(C_DEFAULT,
		    "$n says 'I hereby declare the city of Bethaven open!'", ch,
		    NULL, NULL, TO_ROOM);
		break;

	case 'E':
		act(C_DEFAULT,
		    "$n says 'I hereby declare the city of Bethaven closed!'",
		    ch, NULL, NULL, TO_ROOM);
		break;
...
bool spec_thief(CHAR_DATA * ch)
{
	...
	for (victim = ch->in_room->people; victim;
	     victim = victim->next_in_room) {
		if (IS_NPC(victim)
		    || victim->level >= LEVEL_IMMORTAL || number_bits(3) != 0 || !can_see(ch, victim))	/* Thx Glop */
			continue;
		...
		if (IS_AWAKE(victim) && number_percent() >= ch->level) {
			...
			amount.gold =
			    (victim->money.gold * number_range(1, 20) / 100);
			if (chance(50)) {
				amount.silver =
				    (victim->money.silver *
				     number_range(1, 20) / 100);
				amount.copper =
				    (victim->money.copper *
				     number_range(1, 20) / 100);
			}
...
bool spec_repairman(CHAR_DATA * ch)
{
	...
	if (IS_SET(pexit->exit_info, EX_BASHED)) {
		...
		act(C_DEFAULT, "You repair the $d.", ch, NULL, pexit->keyword,
		    TO_CHAR);
		act(C_DEFAULT, "$n repairs the $d.", ch, NULL, pexit->keyword,
		    TO_ROOM);
		...
		if ((to_room = pexit->to_room)
			...
			for (rch = to_room->people; rch;
				...
				act(C_DEFAULT,
				    "The $d is set back on its hinges.", rch,
