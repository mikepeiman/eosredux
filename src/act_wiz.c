...
void do_empower(CHAR_DATA * ch, char *argument)
{
	...
	for (argument = one_argument(argument, arg); *arg != '\0';
		...
		if (!*cmd_table[cmd].name)
			...
			has =
			    is_name(NULL, cmd_table[cmd].name,
				    victim->pcdata->empowerments);
			switch (chn) {
			case '+':
				if (has) {
					sprintf(buf,
						"They are already empowered with %s.\n\r",
						cmd_table[cmd].name);
					chn = ' ';
				} else if (is_name
					   (NULL, cmd_table[cmd].name,
					    victim->pcdata->detractments)) {
					sprintf(buf,
						"%s is detracted from them.\n\r",
						cmd_table[cmd].name);
					chn = ' ';
				}
				break;
			...
			case '+':
				...
				sprintf(buf,
					"They are now empowered with %s.\n\r",
					cmd_table[cmd].name);
				break;
			case '-':
				buf[0] = '\0';
				for (pemp =
				     one_argument(victim->pcdata->empowerments,
						  sarg);
				     *sarg != '\0';
				     pemp = one_argument(pemp, sarg))
				...
				if (!*buf)
					...
					victim->pcdata->empowerments =
					    str_dup(buf + 1);
				sprintf(buf,
					"They are no longer empowered with %s.\n\r",
					cmd_table[cmd].name);
				break;
			}
			buf[0] = UPPER(buf[0]);
...
void do_detract(CHAR_DATA * ch, char *argument)
{
	...
	if (!is_name(NULL, "detract", ch->pcdata->empowerments)
		...
		send_to_char(AT_WHITE,
			     "You are note empowered to detract.\n\r", ch);
		return;
	}
	...
	for (argument = one_argument(argument, arg); *arg != '\0';
		...
		if (!*cmd_table[cmd].name)
			...
			case '+':
				if (has) {
					sprintf(buf,
						"%s is already detracted from them.\n\r",
						cmd_table[cmd].name);
					chn = ' ';
				} else if (is_name
					   (NULL, cmd_table[cmd].name,
			...
			case '+':
				...
				sprintf(buf,
					"%s is now detracted from them.\n\r",
					cmd_table[cmd].name);
				break;
			case '-':
				buf[0] = '\0';
				for (pdet =
				     one_argument(victim->pcdata->detractments,
						  sarg);
				     *sarg != '\0';
				     pdet = one_argument(pdet, sarg))
					if (str_cmp(sarg, cmd_table[cmd].name))
						sprintf(buf + strlen(buf),
							" %s", sarg);
				free_string(victim->pcdata->detractments);
				if (!*buf)
					...
					victim->pcdata->detractments =
					    str_dup(buf + 1);
				sprintf(buf,
					"%s is no longer detracted from them.\n\r",
					cmd_table[cmd].name);
				break;
			}
			buf[0] = UPPER(buf[0]);
...
void do_slaymes(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		send_to_char(AT_RED,
			     "  slaymes usee <string>  (What you see.)\n\r",
			     ch);
		send_to_char(AT_RED,
			     "  slaymes room <string>  (What everybody else in the room see.)\n\r",
			     ch);
		send_to_char(AT_RED,
			     "  slaymes vict <string>  (What the victim see.)\n\r",
			     ch);
		return;
	}
...
void do_trmes(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		send_to_char(AT_RED,
			     ...
			     ch);
		send_to_char(AT_RED,
			     "  transmes from <string>  (What the room the victim came from see.)\n\r",
			     ch);
		send_to_char(AT_RED,
			     "  transmes vict <string>  (What the victim see.)\n\r",
			     ch);
		return;
	}
...
void do_bamf(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		send_to_char(AT_RED,
			     ...
			     ch);
		send_to_char(AT_RED,
			     "  bamfmes in   <string>  (What everybody in the room you goto see.)\n\r",
			     ch);
		send_to_char(AT_RED,
			     "  bamfmes see  <string>  (What you see.)\n\r",
			     ch);
		return;
	}
...
void do_disconnect(CHAR_DATA * ch, char *argument)
{
	...
	if (!victim->desc) {
		act(AT_RED, "$N doesn't have a descriptor.", ch, NULL, victim,
		    TO_CHAR);
		return;
	}
...
void do_pardon(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg2, "killer")) {
		if (IS_SET(victim->act, PLR_KILLER)) {
			...
			send_to_char(AT_BLUE,
				     "You are no longer a KILLER.\n\r", victim);
			sprintf(buf, "$N removed %s's killer flag.",
				victim->name);
	...
	if (!str_cmp(arg2, "thief")) {
		if (IS_SET(victim->act, PLR_THIEF)) {
			...
			send_to_char(AT_BLUE, "You are no longer a THIEF.\n\r",
				     victim);
			sprintf(buf, "$N removed %s's thief flag.",
				victim->name);
			wiznet(buf, ch, NULL, WIZ_GENERAL, 0, 0);
		}
		return;
	...
	if (!str_cmp(arg2, "outcast")) {
		if (IS_SET(victim->act, PLR_OUTCAST)) {
			...
			send_to_char(AT_BLUE,
				     ...
				     victim);
			sprintf(buf, "$N removed %s's outcast flag.",
				victim->name);
			wiznet(buf, ch, NULL, WIZ_GENERAL, 0, 0);
		}
		return;
...
void do_goto(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_SET(ch->act, PLR_WIZINVIS)) {
		act(AT_RED, "$n $T.", ch, NULL,
		    ...
		    TO_ROOM);
		if (pet)
			act(AT_RED, "$n leaves in a swirling mist.", pet, NULL,
			    NULL, TO_ROOM);
	} else {
	...
	if (pet && !IS_SET(ch->act, PLR_WIZINVIS))
		act(AT_RED, "$n appears in a swirling mist.", pet, NULL, NULL,
		    TO_ROOM);
	else if (pet && IS_SET(ch->act, PLR_WIZINVIS)) {
		for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
			if (get_trust(gch) >= ch->wizinvis) {
				sprintf(buf,
					"%s appears in a swirling mist.\n\r",
					pet->name);
				send_to_char(AT_RED, buf, gch);
			}
		}
...
void do_astat(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "&WSounds:   &w%s\n\r",
		pArea->reset_sound ? pArea->reset_sound : "(default)");
	send_to_char(C_DEFAULT, buf, ch);
...
#ifdef NEW_MONEY
	sprintf(buf,
		"Gold Cost: %d.  Silver Cost: %d.  Copper Cost: %d.\n\r",
...
void do_mstat(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(victim) && victim->desc) {
		if (str_cmp(victim->desc->user, "(unknown)")) {
			sprintf(buf, "&c    Email&w: &W%s&R@&W%s\n\r",
				victim->desc->user ? victim->
				desc->user : "(none)", victim->desc->host);
			send_to_char(C_DEFAULT, buf, ch);
		} else
	...
	sprintf(buf,
		"Short description&w:&W %s\n\r&cLong  description&w:&W %s",
		victim->short_descr,
		victim->long_descr[0] !=
		'\0' ? victim->long_descr : "(none)&w.\n\r");
	send_to_char(AT_CYAN, buf, ch);
	...
	send_to_char(AT_CYAN, buf, ch);
	sprintf(buf,
		"Hp&w: &R%d&w/&R%d &c%s&w: &R%d&w/&R%d &cMove&w: &R%d&w/&R%d &cPractices&w: &R%d\n\r",
		victim->hit, MAX_HIT(victim),
		is_class(victim,
			 CLASS_VAMPIRE) ?
		"Blood" : "Mana",
		MT(victim), MT_MAX(victim),
		victim->move, MAX_MOVE(victim), victim->practice);
	send_to_char(AT_CYAN, buf, ch);
	...
	if (!IS_NPC(victim)) {
		sprintf(buf,
			"Bank Account&w: &R%d&WGold  &R%d&WSilver &R%d&WCopper\n\r",
			victim->pcdata->bankaccount.gold,
			victim->pcdata->bankaccount.silver,
		...
		send_to_char(AT_CYAN, buf, ch);
				 ...
				 */
	sprintf(buf,
		"Hitroll&w: &R%d &cDamroll&w: &R%d &cPosition&w: &R%d &cWimpy&w:&R%d\n\r",
	...
	send_to_char(AT_CYAN, buf, ch);
	sprintf(buf,
		"Summon timer&w:&R %d &cCombat timer&w:&R %d &cClan timer&w:&R %d\n\r",
		victim->summon_timer, victim->combat_timer, victim->ctimer);
	send_to_char(AT_CYAN, buf, ch);

	sprintf(buf,
		"Stunned&w:&R &cTotal&w:&R %d &cCommand&w:&R %d &cMagic&c:&R %d "
		"&cNon&w-&cMagic&w:&R %d  &cTo&w-&cStun&w:&R %d\n\r",
		victim->stunned[STUN_TOTAL], victim->stunned[STUN_COMMAND],
		victim->stunned[STUN_MAGIC],
		victim->stunned[STUN_NON_MAGIC], victim->stunned[STUN_TO_STUN]);
	send_to_char(AT_CYAN, buf, ch);
	...
	for (paf = victim->affected2; paf; paf = paf->next) {
		...
		sprintf(buf,
			...
			paf->modifier,
			paf->duration,
			affect_bit_name2(paf->bitvector), paf->level);
		send_to_char(C_DEFAULT, buf, ch);
	}
	return;
	...
	if (arg[0] == '*') {
		...
		if (str_cmp((arg + 1), "invoke")) {
			if (type == ITEM_WEAPON || type == ITEM_ARMOR) {
				...
				if (type == ITEM_WEAPON)
					sub_type =
					    flag_lookup(buf2, weapon_flags);
				else
					sub_type =
					    flag_lookup(buf2, wear_flags);
			}
			if (type == ITEM_ARMOR && sub_type < 1) {
				send_to_char(AT_WHITE,
					     "Unknow armor sub-type.\n\r", ch);
				return;
			}
			if (type == ITEM_WEAPON
			    && !str_cmp(flag_string(weapon_flags, sub_type),
					"none")) {
				send_to_char(AT_WHITE,
					     "Uknown weapon sub-type.\n\r", ch);
				return;
			}
...
void do_mwhere(CHAR_DATA * ch, char *argument)
{
	...
	for (victim = char_list; victim; victim = victim->next) {
		if (IS_NPC(victim)
		    && victim->in_room && is_name(ch, arg, victim->name)) {
			found = TRUE;
	...
	if (!found) {
		act(AT_WHITE, "You didn't find any $T.", ch, NULL, arg,
		    TO_CHAR);
		return;
	}
...
void do_return(CHAR_DATA * ch, char *argument)
{
	...
	wiznet(buf, ch->desc->original, NULL, WIZ_SWITCHES, WIZ_SECURE,
	       get_trust(ch));
...
void do_oload(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0') {
		send_to_char(AT_WHITE,
			     "&RSyntax: &Woload <vnum> [number] [level].\n\r",
			     ch);
		return;
	}
...
void do_advance(CHAR_DATA * ch, char *argument)
{
	...
	if (level < LEVEL_IMMORTAL) {
		if (level <= victim->level) {
			...
			send_to_char(AT_RED,
				     "Lowering a player's level!\n\r", ch);
			sprintf(msgbuf, "$N has demoted %s to level %d!",
				victim->name, level);
	...
	if (!str_cmp(arg, "all")) {
		for (victim = char_list; victim; victim = victim->next) {
			...
			if (victim && victim->in_room)
				act(AT_BLUE, "$n has restored you.", ch, NULL,
				    victim, TO_VICT);
		}
...
/*    BAN_DATA  *to_ban;*/
	...
	for (pban = ban_list; pban; pban = pban->next) {
		if ((!str_cmp(arg, pban->name) && arg1[0] == '\0')
		    || (arg1[0] != '\0' && pban->user
			&& !str_cmp(arg1, pban->user)
			&& !str_cmp(arg, pban->name))) {
			send_to_char(AT_RED,
				     "That site is already banned!\n\r", ch);
			return;
		}
	}
...
void do_allow(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		do_banlist(ch, "");
		send_to_char(AT_RED,
			     "Remove which site or user from the ban list?\n\r",
			     ch);
		send_to_char(C_DEFAULT,
			     "Syntax: Allow banned.site [username]\n\r", ch);
		send_to_char(C_DEFAULT,
			     "Username being the optional banned user (if applicable).\n\r",
			     ch);
		return;
	}
...
void do_cset(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0') {
		send_to_char(AT_WHITE,
			     "Syntax: &Bcset <victim> <field>  <value>\n\r",
			     ch);
		send_to_char(AT_WHITE, "\n\r", ch);
	...
	if (!str_cmp(arg2, "clvl")
		...
		send_to_char(AT_WHITE,
			     "You are not empowered to set one's clan level.\n\r",
			     ch);
		return;
	}
	...
	if (!str_cmp(arg2, "clan")
		...
		send_to_char(AT_WHITE,
			     "You are not empowered to set one's clan.\n\r",
			     ch);
		return;
	}
...
void do_mset(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0') {
		...
		send_to_char(AT_WHITE,
			     "or:     mset <victim> class <slot> <value>\n\r",
			     ch);
		send_to_char(AT_WHITE, "\n\r", ch);
		send_to_char(AT_WHITE, "&pField being one of:\n\r", ch);
		send_to_char(AT_WHITE,
			     "  str int wis dex con level gold silver\n\r", ch);
		send_to_char(AT_WHITE,
			     "  copper hp mana blood move practice align\n\r",
			     ch);
		send_to_char(AT_WHITE, "  mhp mmana mblood mmove\n\r", ch);
		send_to_char(AT_WHITE,
			     "  thirst drunk full security affected_by2\n\r",
			     ch);
		send_to_char(AT_WHITE, "  affected_by act mstr mint mwis\n\r",
			     ch);
		send_to_char(AT_WHITE,
			     "  mdex mcon bank carryn carryw save race\n\r",
			     ch);
		...
		send_to_char(AT_WHITE,
			     "  example: mset self class 1 warrior\n\r", ch);
		return;
	}
	...
	if (!str_cmp(arg2, "sex")) {
		...
		if (!str_cmp(arg3, "male"))
			...
			send_to_char(AT_WHITE,
				     "Valid sexes are Male, Female & Neutral.\n\r",
				     ch);
			return;
		}
		victim->sex = value;
	...
	if (!str_cmp(arg2, "bank")) {
		...
		if (is_number(arg4)) {
			if (!str_cmp(arg3, "gold"))
				...
				send_to_char(AT_WHITE,
					     "&WSyntax: mset <victim> <bank> <currency_type> <amount>\n\r",
					     ch);
				return;
			}
		} else {
			...
			send_to_char(AT_WHITE,
				     "&WSyntax: mset &R<victim> <bank> <currency_type> <amount>\n\r",
				     ch);
			return;
		}
#else				/* 
	...
	if (!str_cmp(arg2, "act")) {
		if (!IS_NPC(victim))
			if (get_trust(ch) < L_DIR	/*&& !IS_SET( ch->affected_by2, CODER
				...
				send_to_char(AT_WHITE,
					     "You are too low of trust to set ones actflags.\n\r",
					     ch);
				return;
			}
		if (IS_NPC(victim)) {
			if (!str_cmp(arg3, "sentinel") || !str_cmp(arg3, "se"))
				value = 2;
			if (!str_cmp(arg3, "scavenger")
			    || !str_cmp(arg3, "sc"))
				value = 4;
			if (!str_cmp(arg3, "aggressive")
			    || !str_cmp(arg3, "ag"))
				value = 32;
		...
		if (!IS_NPC(victim)) {
			...
			if (!str_cmp(arg3, "ansi") || !str_cmp(arg3, "a"))
				value = 16777216;
			if (!str_cmp(arg3, "autocoins")
			    || !str_cmp(arg3, "aco"))
				value = 33554432;
			if (!str_cmp(arg3, "keeptitle")
			    || !str_cmp(arg3, "kt"))
				value = 67108864;
		...
		if (IS_NPC(victim))
			if ((value & ACT_PROTOTYPE) && (get_trust(ch) < L_CON)) {
				send_to_char(AT_WHITE,
					     "You cannot toggle the prototype flag at your current level.\n\r",
					     ch);
				return;
			}
		...
		if (0xFFFFFFFF & value) {
			if (p) {
				pMob->act ^= value;
				SET_BIT(victim->in_room->area->area_flags,
					AREA_CHANGED);
			} else {
				victim->act ^= value;
			}
	...
	if (!str_cmp(arg2, "affected_by")) {
		if ((get_trust(ch) < L_DIR) && (!IS_NPC(victim))) {
			send_to_char(AT_WHITE,
				     "You cannot set a player's affected_by from your current level.\n\r",
				     ch);
			return;
		}
		if (p) {
	...
	if (!str_cmp(arg2, "affected_by2")) {
		if (get_trust(ch) < L_IMP && !IS_NPC(victim)) {
			send_to_char(AT_WHITE,
				     "You are too low of level to set a player's affected_by2.\n\r",
				     ch);
			return;
		}
		if (p) {
	...
	if (!str_cmp(arg2, "class")) {
		...
		if (slot == 1 && value == -1) {
			send_to_char(AT_WHITE,
				     "Cannot turn off 1st Class.\n\r", ch);
			return;
		}
		if (slot == 2 && value == -1 && victim->class[2] != -1) {
			send_to_char(AT_WHITE,
				     "You must turn off 3rd Class before 2nd.\n\r",
				     ch);
			return;
		}
		...
		if (value != -1 && !class_table[value].races[victim->race]) {
			sprintf(buf,
				"Player's race (%s) does not allow class %s.\n\r",
	...
	if (!str_cmp(arg2, "hp")) {
		...
		if (victim->fighting && value < 0) {
			send_to_char(AT_WHITE,
				     "You cannot set a fighting person's hp below 0.\n\r",
				     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "blood")) {
		if (value < 0 || value > 30000) {
			send_to_char(AT_WHITE,
				     "Blood range is 0 to 30,000 mana points.\n\r",
				     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "practice")) {
		if (value < 0 || value > 100) {
			send_to_char(AT_WHITE,
				     "Practice range is 0 to 100 sessions.\n\r",
				     ch);
			return;
		}
...
void do_oset(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0') {
		send_to_char(AT_WHITE,
			     "&pSyntax: oset <object> <field>  <value>\n\r",
			     ch);
		send_to_char(AT_WHITE,
			     "&por:     oset <object> <string> <value>\n\r",
			     ch);
		send_to_char(AT_WHITE, "\n\r", ch);
		send_to_char(AT_WHITE, "&pField being one of:\n\r", ch);
	...
	if (!str_cmp(arg2, "delapply")) {
		...
		if (value == -1) {
			send_to_char(AT_WHITE,
				     "&pSyntax: oset object delapply [number]\n\r",
				     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "apply")) {
		...
		if (is_number(arg3) || arg4[0] == '\0' || !is_number(arg4)) {
			send_to_char(AT_WHITE,
				     "&pSyntax: oset object apply type modifier\n\r",
				     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "cost")) {
		...
		if (is_number(arg4)) {
			if (!str_cmp(arg3, "gold"))
				...
				send_to_char(AT_WHITE,
					     "Invaild currency type.\n\r", ch);
				send_to_char(AT_WHITE,
					     "&WSyntax: oset &Robject <cost> <currency_type> <amount>\n\r",
					     ch);
				return;
			}
		} else {
	...
	if (arg[0] == '\0') {
		for (d = descriptor_list; d; d = d->next) {
			if (d->character && can_see(ch, d->character)) {
				count++;
				sprintf(buf + strlen(buf),
					"[%4d %-11s] %s&W!&B%s&W@&G%s&R\n\r",
					d->descriptor,
					con_type_name(d->connected),
					d->original ? d->original->name :
					d->character ? d->
					character->name : "(none)",
					d->user ? d->user : "(none)", d->host);
			}
		}
...
void do_force(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "all") || is_number(arg)) {
		...
		for (vch = char_list; vch; vch = vch->next) {
			...
			if (!IS_NPC(vch) && get_trust(vch) < get_trust(ch)
				...
				if (!is_number(arg)
					...
					act(AT_RED, "$n forces you to '$t'.",
					    ch, argument, vch, TO_VICT);
					interpret(vch, argument);
					continue;
				}
				if (!is_number(arg1)) {
					sprintf(log_buf, "%s %s", arg1,
						argument);
					act(AT_RED, "$n forces you to '$t'.",
					    ch, log_buf, vch, TO_VICT);
		...
		act(AT_RED, "$n forces you to '$t'.", ch, argument, victim,
		    TO_VICT);
...
void do_cloak(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET(ch->act, PLR_CLOAKED)) {
		...
		act(AT_YELLOW, "$n is no longer cloaked.", ch, NULL, NULL,
		    TO_ROOM);
		send_to_char(AT_WHITE, "You are no longer cloaked.\n\r", ch);
	} else {
		SET_BIT(ch->act, PLR_CLOAKED);
		act(AT_YELLOW, "$n cloaks $s presence.", ch, NULL, NULL,
		    TO_ROOM);
...
void do_invis(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET(ch->act, PLR_WIZINVIS)) {
		...
		act(AT_YELLOW, "$n slowly fades into existence.", ch, NULL,
		    NULL, TO_ROOM);
		send_to_char(AT_WHITE,
			     "You slowly fade back into existence.\n\r", ch);
	} else {
		SET_BIT(ch->act, PLR_WIZINVIS);
		act(AT_YELLOW, "$n slowly fades into thin air.", ch, NULL,
		    NULL, TO_ROOM);
...
void do_newlock(CHAR_DATA * ch, char *argument)
{
	...
	if (numlock != 0 && get_trust(ch) < L_SEN) {
		send_to_char(AT_WHITE,
			     "You may not change the current numlock setting\n\r",
			     ch);
		sprintf(buf, "Game numlocked to levels %d and below.\n\r",
			numlock);
		send_to_char(AT_RED, buf, ch);
...
void do_sstime(CHAR_DATA * ch, char *argument)
{
	     ...
	     && str_cmp(arg1, "type")))
	{
		...
		send_to_char(AT_WHITE, "Field being one of:\n\r", ch);
		send_to_char(AT_WHITE,
			     "  downtime  1warning  2warning  type\n\r", ch);
		send_to_char(AT_WHITE, "\n\r", ch);
		send_to_char(AT_WHITE, "Value being format of:\n\r", ch);
		send_to_char(AT_WHITE,
			     "  hh:mm:ss  (military time) or  *  (for off)\n\r",
			     ch);
		send_to_char(AT_WHITE, "\n\r", ch);
		sprintf(buf,
			"1st warning:  %s\n\r2nd warning:  %s\n\r   Downtime:  %s\n\r",
...
void do_qset(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0') {
		...
		if (quest)
			send_to_char(AT_WHITE,
				     "There is currently a quest.\n\r", ch);
		else
			send_to_char(AT_WHITE,
				     "There is currently no quest.\n\r", ch);
	...
	if (!str_cmp(arg1, "quest") || !str_cmp(arg1, "QUEST")) {
		if (quest) {
			...
			qmax = 100;
			send_to_char(AT_WHITE,
				     "Ok, there is no longer a quest running.\n\r",
				     ch);
...
void do_sstat(CHAR_DATA * ch, char *argument)
{
	...
	for (sn = 1; skill_table[sn].name[0] != '\0'; sn++) {
		if (victim->pcdata->learned[sn] <= 0 && !can_use_skpell(ch, sn))
			continue;
...
void do_restrict(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0' || arg1[0] == '\0' || (!is_number(arg1)
		...
		send_to_char(AT_PURPLE,
			     "Syntax: restrict [command] log [logtype]\n\r",
			     ch);
	...
	if (!str_cmp("all", arg)) {
		...
		for (; (cmd <= lvl) && (cmd_table[cmd].name[0] != '\0'); cmd++) {
			if (get_trust(ch) >= cmd_table[cmd].level) {
				sprintf(log_buf, "%-3d &R%-12s &B(&Y%3d&B)   ",
					cmd, cmd_table[cmd].name,
					cmd_table[cmd].level);
	...
	if (((lvl < 0 || lvl > L_IMP || lvl > get_trust(ch))
	     && (arg2[0] == '\0'))
	...
	for (cmd = 1; cmd_table[cmd].name != '\0'; cmd++) {
		if (!str_prefix(arg, cmd_table[cmd].name)) {
			...
			if (cmd_table[cmd].level > get_trust(ch)) {
				if (log)
					send_to_char(AT_WHITE,
						     "You cannot change the log_type on a command which you do not have.\n\r",
						     ch);
				else
					send_to_char(AT_WHITE,
						     "You cannot restrict a command which you do not have.\n\r",
						     ch);
				return;
			}
			if (log)
			...
			if (log)
				sprintf(log_buf,
					"%s changing log_type of %s to %d.",
					ch->name, arg, lvl);
			else
				sprintf(log_buf,
					"%s restricting %s to level %d.",
					ch->name, arg, lvl);
			log_string(log_buf, CHANNEL_GOD, ch->level - 1);
			if (log)
				sprintf(log_buf,
					"You change the log_type of %s to %d.\n\r",
					arg, lvl);
			else
				sprintf(log_buf,
					"You restrict %s to level %d.\n\r",
					arg, lvl);
			send_to_char(AT_WHITE, log_buf, ch);
			break;
		}
	...
	if (cmd_table[cmd].name == '\0') {
		sprintf(log_buf, "There is no %s command.", arg);
		send_to_char(AT_WHITE, log_buf, ch);
	}
	return;
...
void do_wrlist(CHAR_DATA * ch, char *argument)
{
	...
	if (((uvnum == 0) && (lvnum == 0)) || (arg[0] == '\0')
	    || (type == -1)) {
		send_to_char(AT_PURPLE,
			     "Syntax: wrlist [type] [lvnum] [uvnum]\n\r", ch);
		return;
	}
				...
				if (mob->mobprogs)
					for (mprog = mob->mobprogs; mprog;
						...
						sprintf(log_buf,
							...
							mprog->arglist :
							"(None)");
						send_to_char(C_DEFAULT, log_buf,
							     ch);
					}
			}
		}
...
void do_vused(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char(AT_PINK,
		     "Used VNUMs (ignores vnums assigned but not created).\n\r",
		     ch);
	send_to_char(AT_PINK,
		     "Double check free vnums against assigned vnums in &Balist&P.\n\r",
		     ch);

	for (atvnum = 1; atvnum <= 32767; atvnum++) {
		if (get_room_index(atvnum) || get_mob_index(atvnum)
		    || get_obj_index(atvnum)) {
			if (bstart == -1)
				bstart = atvnum;
		} else if (bstart != -1) {
			bend = (atvnum - 1);
...
void do_hlist(CHAR_DATA * ch, char *argument)
{
	...
	if (lvnum == 0 || uvnum == 0 || lvnum > uvnum) {
		send_to_char(AT_PURPLE,
			     "Syntax: hlist [firsthelp#] [secondhelp#]\n\r",
			     ch);
		return;
	}
...
void do_astrip(CHAR_DATA * ch, char *argument)
{
	...
	if (ch != victim)
		send_to_char(C_DEFAULT,
			     "All your affects have been removed.\n\r", victim);
...
/*  CHAR_DATA *rch; */
	...
	if (arg1[0] == '\0' && arg2[0] == '\0') {
		...
		send_to_char(AT_GREY,
			     ...
			     ch);
		send_to_char(AT_GREY,
			     "Author: Bram    Email: bram@ionet.net\n\r", ch);
		return;
	}
	...
	if (victim->pcdata->corpses == 0) {
		send_to_char(AT_GREY,
			     "That player has no corpses on file.\n\r", ch);
		return;
	}
	...
	if (is_number(arg2)) {
		...
		for (c = 1; c < corpse_cont[0] + 1; c++) {
			...
			obj =
			    create_object(get_obj_index(corpse_cont[c]),
					  item_level[c]);
...
void do_wiznet(CHAR_DATA * ch, char *argument)
{
		...
		/* list of all wiznet options */
	{
		...
		send_to_char(AT_RED,
			     "Wiznet options available to you are:\n\r", ch);
		send_to_char(AT_RED, buf, ch);
		return;
	}
...
void do_seize(CHAR_DATA * ch, char *argument)
{
	...
	if (arg3[0] == '\0' || !str_cmp(arg3, "yes") || !str_cmp(arg3, "true"))
		act(AT_WHITE, "You no longer own $p.", ch, obj, victim,
		    TO_VICT);
...
void do_clanquest(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0') {
		send_to_char(AT_WHITE,
			     "&RSyntax: &Wcquest earn  <victim> <amount>\n\r",
			     ch);
		send_to_char(AT_WHITE,
			     "    &Ror: &Wcquest spend <victim> <amount>\n\r",
			     ch);
		return;
	}
	...
	if (victim == ch) {
		send_to_char(AT_WHITE,
			     "You are a deity, you cannot gain clan quest points!\n\r",
			     ch);
		return;
	}
	...
	if (victim->clan != ch->clan) {
		send_to_char(AT_WHITE,
			     "You cannot set the clan quest points of a person outside your clan.\n\r",
			     ch);
		return;
	}
	...
	if (!str_cmp("earn", arg1)) {
		...
		sprintf(buf,
			"&RYou've earned %d Clan Quest points!! Congratulations!!\n\r",
			value);
		...
		if (value > victim->cquestpnts) {
			if (victim->cquestpnts > 0)
				sprintf(buf,
					"&W%s only has %d Clan Quest points to spend.\n\r",
					victim->name, victim->cquestpnts);
			else
				sprintf(buf,
					"&W%s doesn't have any Clan Quest points to spend.\n\r",
					victim->name);
			send_to_char(AT_WHITE, buf, ch);
			return;
		}
