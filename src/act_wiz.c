...
send_to_char(AT_RED, "&WExtra description keywords:&W [&R",
				ch);
...
sprintf(buf, "&WSounds:   &w%s\n\r",
		   pArea->reset_sound ? pArea->reset_sound : 
"(default)");
...
IS_NPC(victim->hunting) ? victim->hunting->
			 short_descr 
 : victim->hunting->name,
...
victim->money.gold, victim->money.silver,
		    victim->money.copper);
...
act(AT_BLUE, "$n has restored you.", ch, NULL, victim,
		     TO_VICT);
...
if (uvnum > MR || uvnum < 1 || lvnum > MR || lvnum < 1
	       || lvnum > uvnum)
...
act(AT_GREY, "$n has created a $p!", ch, corpse, NULL,
		     TO_ROOM);
...
if (IS_SET
				     (ch->wiznet, wiznet_table[flag].flag))
...
for (c_obj = obj->contains; c_obj != NULL;
...
act(AT_WHITE, "$n has created $p.", ch, clone, NULL,
		       TO_ROOM);
