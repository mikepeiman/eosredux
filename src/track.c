			...
			if (!IS_NPC(ch)
				...
				for (vch = char_list; vch; vch = vch->next) {
					if (vch->deleted || !IS_NPC(vch)
					    || !can_see(ch, vch)
					    || !IS_SAME_AREA(ch->in_room,
							     vch->in_room)
					    || (dir =
						find_first_step(ch->in_room,
								vch->in_room)) <
					    0)
						continue;
...
void hunt_victim(CHAR_DATA * ch)
{
	...
	if ( !can_see(ch, ch->hunting)) {
		if (IS_NPC(ch))
			...
			write_to_buffer(ch->desc,
					"&rTrack1: &RYou have lost the trail.\n\r",
					0);
		ch->hunting = NULL;
		return;
	}
	...
	if (dir < 0) {
		if (IS_NPC(ch)) {
			sprintf(buf, "Damn!  Lost %s!",
				ch->hunting->sex ==
				SEX_FEMALE ? "her" : ch->hunting->sex ==
				SEX_MALE ? "him" : "it");
			do_say(ch, buf);
		} else if (ch->desc) {
			if (dir == TRACK_ALREADY_THERE)
				write_to_buffer(ch->desc,
						"\n\r&rTrack: &RAhh.. You have found your prey.",
						0);
			else
				write_to_buffer(ch->desc,
		...
		if (IS_NPC(ch)) {
			...
			sprintf(log_buf, "%s hunting %s at %d.",
				ch->short_descr,
				(IS_NPC(ch->hunting)) ? ch->hunting->
				short_descr : ch->hunting->name,
				ch->hunting->in_room->vnum);
