...
void check_gcast(CHAR_DATA * ch)
{
	...
	case TAR_GROUP_IGNORE:
		rprog_cast_sn_trigger(ch->in_room, ch, sn,
				      (ch->gspell->victim ? ch->gspell->
				       victim : ch));
		break;
	case TAR_GROUP_OBJ:
		if (ch->gspell->victim) {
			oprog_cast_sn_trigger(ch->gspell->victim, ch, sn,
					      ch->gspell->victim);
			rprog_cast_sn_trigger(ch->in_room, ch, sn,
					      ch->gspell->victim);
		}
...
void group_cast(int sn, int level, CHAR_DATA * ch, char *argument)
{
	...
	if (ch->gspell && ch->gspell->timer > 0) {
		send_to_char(AT_BLUE,
			     "You already have a group spell in progress.\n\r",
			     ch);
		return;
	}
...
void gspell_mass_shield(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (vch = ch->in_room->people; vch; vch = vch->next_in_room) {
		...
		case 1:
			spell_fireshield(skill_lookup("fireshield"), level, vch,
					 vch);
			break;
		case 2:
			spell_shockshield(skill_lookup("shockshield"), level,
					  vch, vch);
			break;
		case 3:
			spell_iceshield(skill_lookup("iceshield"), level, vch,
					vch);
			break;
		case 4:
			spell_chaosfield(skill_lookup("chaos field"), level,
					 vch, vch);
			break;
		case 5:
			...
			break;
		case 6:
			spell_sanctuary(skill_lookup("sanctuary"), level, vch,
					vch);
			break;
		}
	}
