...
/*  int tot_coins; */
	...
	if (container) {
		if (!palming) {
			act(AT_GREEN, "You get $p from $P.", ch, obj, container,
			    TO_CHAR);
			act(AT_GREEN, "$n gets $p from $P.", ch, obj, container,
			    TO_ROOM);
		}
		if (palming == 1)
			act(AT_GREEN, "You palm $p from $P.", ch, obj,
			    container, TO_CHAR);
		...
		if (!palming) {
			act(AT_GREEN, "You get $p.", ch, obj, container,
			    TO_CHAR);
			act(AT_GREEN, "$n gets $p.", ch, obj, container,
			    TO_ROOM);
		}
		if (palming == 1)
			act(AT_GREEN, "You palm $p.", ch, obj, container,
			    TO_CHAR);
...
void get(CHAR_DATA * ch, char *argument, bool palming)
{
	...
	if (arg2[0] == '\0') {
		if (str_cmp(arg1, "all") && str_prefix("all.", arg1)) {
			...
			if (!obj) {
				act(AT_WHITE, "I see no $T here.", ch, NULL,
				    arg1, TO_CHAR);
				return;
			}
			...
			if (!found) {
				if (arg1[3] == '\0')
					...
					act(AT_WHITE, "I see no $T here.", ch,
					    NULL, &arg1[4], TO_CHAR);
			}
		...
		if (!(container = get_obj_here(ch, arg2))) {
			act(AT_WHITE, "I see no $T here.", ch, NULL, arg2,
			    TO_CHAR);
			return;
		}
		...
		if (IS_SET(container->value[1], CONT_CLOSED)) {
			act(AT_GREEN, "The $d is closed.", ch, NULL,
...
void do_put(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET(container->value[1], CONT_CLOSED)) {
		act(AT_DGREEN, "The $d is closed.", ch, NULL, container->name,
		    TO_CHAR);
		return;
	}
		...
		for (obj = ch->carrying; obj; obj = obj_next) {
			...
			if ((arg1[3] == '\0'
				...
				if (!obj_next || obj->pIndexData->vnum !=
				    obj_next->pIndexData->vnum) {
					sprintf(buf, "You put %d $ps in $P.",
						ObjCount);
					act(AT_WHITE, buf, ch, obj, container,
					    TO_CHAR);
					sprintf(buf, "$n puts %d $ps in $P.",
						ObjCount);
					act(AT_WHITE, buf, ch, obj, container,
					    TO_ROOM);
					ObjCount = 0;
...
/* Change the parameters of create_money to MONEY_DATA *amount */
		...
		if (ch->gold < amount) {
			send_to_char(AT_DGREEN,
				     "You haven't got that many coins.\n\r",
				     ch);
			return;
		}
	...
	if (str_cmp(arg, "all") && str_prefix("all.", arg)) {
		...
		for (obj = ch->carrying; obj; obj = obj_next) {
			...
			if ((arg[3] == '\0' || is_name(ch, &arg[4], obj->name))
				...
				if (!obj_next || obj->pIndexData->vnum !=
				    obj_next->pIndexData->vnum) {
					sprintf(buf, "You drop %d $ps.",
						ObjCount);
					act(AT_WHITE, buf, ch, obj, NULL,
					    TO_CHAR);
					sprintf(buf, "$n drops %d $ps.",
						ObjCount);
					act(AT_WHITE, buf, ch, obj, NULL,
					    TO_ROOM);
					ObjCount = 0;
...
void do_give(CHAR_DATA * ch, char *argument)
{
	...
	if (is_number(arg1)) {
		...
		sprintf(buf, "You give %s %s", victim->name,
			money_string(howmuch));
		...
		act(AT_YELLOW, buf, ch, NULL, victim, TO_VICT);
		act(AT_YELLOW, "$n gives $N some gold.", ch, NULL, victim,
		    TO_NOTVICT);
	...
	if (victim->carry_number + get_obj_number(obj) > can_carry_n(victim)) {
		act(AT_DGREEN, "$N has $S hands full.", ch, NULL, victim,
		    TO_CHAR);
		return;
	}

	if (victim->carry_weight + get_obj_weight(obj) > can_carry_w(victim)) {
		act(AT_DGREEN, "$N can't carry that much weight.", ch, NULL,
		    victim, TO_CHAR);
		return;
	}
...
void do_transport(CHAR_DATA * ch, char *argument)
{
	...
	act(AT_DGREEN, "$p appears magically in your hands from $n.", ch, obj,
	    victim, TO_VICT);
	act(AT_DGREEN, "Something appears magically in $n's hands from $N.",
...
*/
	...
	case ITEM_BLOOD:
		if (!is_class(ch, CLASS_VAMPIRE)) {
			send_to_char(AT_WHITE,
				     "It is not in your nature to do such things.\n\r",
				     ch);
			return;
		}
		if ((ch->bp + 1) > MAX_BP(ch)) {
			send_to_char(AT_RED,
				     "Your hunger for blood has been abated.\n\r",
				     ch);
			return;
		}
		if ((obj->item_type == ITEM_BLOOD) && (obj->value[0] == -1)) {
			ch->bp = MAX_BP(ch);
			send_to_char(AT_RED,
				     "You gorge from the fountain of blood,",
				     ch);
			send_to_char(AT_RED, " abating your thirst.\n\r", ch);
			act(AT_RED, "$n gorges from a fountain of blood.", ch,
			    obj, NULL, TO_ROOM);
			break;
		}
		...
		send_to_char(AT_RED, "You gorge the entire blood puddle.\n\r",
			     ch);
		act(AT_RED, "$n gorges a blood puddle.", ch, obj, NULL,
		    TO_ROOM);
...
void do_drink(CHAR_DATA * ch, char *argument)
{
	...
	case ITEM_BLOOD:
		...
		if ((obj->value[1] <= 0) && (obj->value[0] != -1)) {
			act(AT_RED, "$n laps up the last of the blood.", ch,
			    NULL, NULL, TO_ROOM);
			send_to_char(AT_RED,
				     "You lap up the last of the blood.\n\r",
				     ch);
			extract_obj(obj);
		}
		break;
	...
	case ITEM_DRINK_CON:
		...
		if (!IS_NPC(ch)
		    && ch->pcdata->condition[COND_THIRST] > MAX_THIRST)
		...
		if (obj->value[3] != 0) {
			...
			act(AT_GREEN, "$n chokes and gags.", ch, NULL, NULL,
			    TO_ROOM);
			af.type = gsn_poison;
...
void do_eat(CHAR_DATA * ch, char *argument)
{
	...
	case ITEM_PILL:
		if (ch->level >= obj->level) {
			...
			act(AT_ORANGE, "$p is too high level for you.", ch, obj,
			    NULL, TO_CHAR);
			return;
		}
		break;
...
void wear_obj(CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace)
{
	...
	if (obj->item_type == ITEM_LIGHT) {
		if (!remove_obj(ch, WEAR_LIGHT, fReplace))
			return;
		act(AT_WHITE, "You light $p and hold it.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_WHITE, "$n lights $p and holds it.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_FINGER)) {
		...
		if (!get_eq_char(ch, WEAR_FINGER_L)) {
			act(AT_BLUE, "You wear $p on your left finger.", ch,
			    obj, NULL, TO_CHAR);
			act(AT_BLUE, "$n wears $p on $s left finger.", ch, obj,
			    NULL, TO_ROOM);
		...
		if (!get_eq_char(ch, WEAR_FINGER_R)) {
			act(AT_BLUE, "You wear $p on your right finger.", ch,
			    obj, NULL, TO_CHAR);
			act(AT_BLUE, "$n wears $p on $s right finger.", ch, obj,
			    NULL, TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_NECK)) {
		...
		if (!get_eq_char(ch, WEAR_NECK_1)) {
			act(AT_BLUE, "You wear $p around your neck.", ch, obj,
			    NULL, TO_CHAR);
			act(AT_BLUE, "$n wears $p around $s neck.", ch, obj,
			    NULL, TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_BODY)) {
		if (!remove_obj(ch, WEAR_BODY, fReplace))
			return;
		act(AT_BLUE, "You wear $p on your body.", ch, obj, NULL,
		    TO_CHAR);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_HEAD)) {
		if (!remove_obj(ch, WEAR_HEAD, fReplace))
			return;
		act(AT_BLUE, "You wear $p on your head.", ch, obj, NULL,
		    TO_CHAR);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_ORBIT)) {
		...
		send_to_char(AT_BLUE,
			     "There are already two things spinning about your head.\n\r",
			     ch);
		return;
	}
	...
	if (CAN_WEAR(obj, ITEM_WEAR_FACE)) {
		if (!remove_obj(ch, WEAR_ON_FACE, fReplace))
			return;
		act(AT_BLUE, "You place $p on your face.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n places $p on $s face.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_CONTACT)) {
		if (!remove_obj(ch, WEAR_IN_EYES, fReplace))
			return;
		act(AT_BLUE, "You stick $p into your eyes.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n sticks $p into $s eyes.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_LEGS)) {
		if (!remove_obj(ch, WEAR_LEGS, fReplace))
			return;
		act(AT_BLUE, "You wear $p on your legs.", ch, obj, NULL,
		    TO_CHAR);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_FEET)) {
		if (!remove_obj(ch, WEAR_FEET, fReplace))
			return;
		act(AT_BLUE, "You wear $p on your feet.", ch, obj, NULL,
		    TO_CHAR);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_HANDS)) {
		if (!remove_obj(ch, WEAR_HANDS, fReplace))
			return;
		act(AT_BLUE, "You wear $p on your hands.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n wears $p on $s hands.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_ARMS)) {
		if (!remove_obj(ch, WEAR_ARMS, fReplace))
			return;
		act(AT_BLUE, "You wear $p on your arms.", ch, obj, NULL,
		    TO_CHAR);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_ABOUT)) {
		if (!remove_obj(ch, WEAR_ABOUT, fReplace))
			return;
		act(AT_BLUE, "You wear $p about your body.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n wears $p about $s body.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_WAIST)) {
		if (!remove_obj(ch, WEAR_WAIST, fReplace))
			return;
		act(AT_BLUE, "You wear $p about your waist.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n wears $p about $s waist.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_SHIELD)) {
		if (get_eq_char(ch, WEAR_WIELD_2)) {
			send_to_char(AT_BLUE,
				     "You cannot use a shield while dual wielding!\n\r",
				     ch);
			return;
		}
		...
		act(AT_BLUE, "You wear $p as a shield.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n wears $p as a shield.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WIELD)) {
		...
		if ((strcmp(flag_string(weapon_flags, weapon_type), "none"))) {
			if (!
			    (class_table[prime_class(ch)].
			     objtype[weapon_type])) {
				sprintf(buf,
					"%s's cannot use weapons that %s.\n\r",
	...
	if (CAN_WEAR(obj, ITEM_HOLD)) {
		if (get_eq_char(ch, WEAR_WIELD_2)) {
			send_to_char(AT_BLUE,
				     "You cannot hold something while dual wielding!\n\r",
				     ch);
			return;
		}
		...
		if (!remove_obj(ch, WEAR_HOLD, fReplace))
			return;
		act(AT_BLUE, "You hold $p in your hands.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n holds $p in $s hands.", ch, obj, NULL,
		    TO_ROOM);
	...
	if (CAN_WEAR(obj, ITEM_WEAR_EARS)) {
		...
		if (!get_eq_char(ch, WEAR_EAR_L)) {
			act(AT_BLUE, "You place $p on your left ear.", ch, obj,
			    NULL, TO_CHAR);
			act(AT_BLUE, "$n places $p on $s left ear.", ch, obj,
			    NULL, TO_ROOM);
		...
		if (!get_eq_char(ch, WEAR_EAR_R)) {
			act(AT_BLUE, "You place $p on your right ear.", ch, obj,
			    NULL, TO_CHAR);
			act(AT_BLUE, "$n placess $p on $s right ear.", ch, obj,
			    NULL, TO_ROOM);
		...
		send_to_char(AT_BLUE,
			     "You already have items on both ears.\n\r", ch);
		return;
	}
	...
	if (CAN_WEAR(obj, ITEM_WEAR_ANKLE)) {
		...
		if (!get_eq_char(ch, WEAR_ANKLE_L)) {
			act(AT_BLUE, "You wear $p on your left ankle.", ch, obj,
			    NULL, TO_CHAR);
			act(AT_BLUE, "$n wears $p on $s left ankle.", ch, obj,
			    NULL, TO_ROOM);
		...
		if (!get_eq_char(ch, WEAR_ANKLE_R)) {
			act(AT_BLUE, "You wear $p on your right ankle.", ch,
			    obj, NULL, TO_CHAR);
			act(AT_BLUE, "$n wears $p on $s right ankle.", ch, obj,
			    NULL, TO_ROOM);
		...
		send_to_char(AT_BLUE,
			     "You already have items on both ankles.\n\r", ch);
		return;
	}
...
void do_dual(CHAR_DATA * ch, char *argument)
{
	...
	if ((get_eq_char(ch, WEAR_SHIELD)) || (get_eq_char(ch, WEAR_HOLD))) {
		send_to_char(AT_BLUE,
			     "You cannot dual wield while you hold something in your hands.\n\r",
			     ch);
		return;
	}
	...
	if (CAN_WEAR(obj, ITEM_WIELD)) {
		...
		if (get_obj_weight(obj) > str_app[get_curr_str(ch)].wield) {
			send_to_char(AT_BLUE,
				     "It is too heavy for you to dual wield.\n\r",
				     ch);
			return;
		}
...
void do_sacrifice(CHAR_DATA * ch, char *argument)
{
	...
	char *msgbuf[] = {
		"Thalador", "Angi", "Sherf", "Altrag", "Decklarean", "Reklar",
		    "Cluff", "Flint"
	};
	...
	if (arg[0] == '\0' || !str_cmp(arg, ch->name)) {
		send_to_char(AT_WHITE,
			     ...
			     ch);
		act(AT_WHITE,
		    "$n offers $mself to the gods, who graciously decline.", ch,
		    NULL, NULL, TO_ROOM);
		return;
	}
	if (!str_cmp(arg, "all")) {
	...
	case 1:
		...
		sprintf(buf1, "%d copper coin%s", award,
			(award != 1) ? "s" : "");
#ifdef NEW_MONEY
	...
	case 4:
		if (ch->move >= MAX_MOVE(ch))
			...
			diff = 6;
		award = number_range(0, diff);
		sprintf(buf1, "%d movement point%s", award,
			(award != 1) ? "s" : "");
		ch->move += award;
		break;
	default:
...
void do_quaff(CHAR_DATA * ch, char *argument)
{
	...
	if (obj->level > ch->level)
		act(AT_BLUE, "$p is too high level for you.", ch, obj, NULL,
		    TO_CHAR);
	else {
...
void do_recite(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch)
		...
		case 6:
			send_to_char(AT_CYAN,
				     "You must have said something incorrectly.\n\r",
				     ch);
			act(AT_CYAN, "$n must have said something incorrectly.",
			    ch, NULL, NULL, TO_ROOM);
			act(AT_CYAN, "$p blazes brightly, then is gone.", ch,
			    scroll, NULL, TO_CHAR);
			act(AT_CYAN, "$p blazes brightly and disappears.", ch,
			    scroll, NULL, TO_ROOM);
			extract_obj(scroll);
			return;
		case 7:
	...
	if (scroll->level > ch->level)
		act(AT_BLUE, "$p is too high level for you.", ch, scroll, NULL,
		    TO_CHAR);
	else {
		/* scroll->value[0] is not used for scrolls */
		obj_cast_spell(scroll->value[1], scroll->level, ch, victim,
			       obj);
		obj_cast_spell(scroll->value[2], scroll->level, ch, victim,
			       obj);
		obj_cast_spell(scroll->value[3], scroll->level, ch, victim,
			       obj);
		if (victim)
...
void do_brandish(CHAR_DATA * ch, char *argument)
{
	...
	if ((sn = staff->value[3]) < 0 || !is_sn(sn)
	...
	if ((staff->value[2] > 0) || (staff->value[1] == -1)) {
		...
		if (!IS_NPC(ch)
			...
			case 3:
				act(AT_CYAN,
				    "You are unable to invoke the power of $p.",
				    ch, staff, NULL, TO_CHAR);
				act(AT_CYAN,
				    "$n is unable to invoke the power of $p.",
			...
			case 6:
				act(AT_CYAN,
				    "You summon the power of $p, but it fizzles away.",
				    ch, staff, NULL, TO_CHAR);
				act(AT_CYAN,
				    "$n summons the power of $p, but it fizzles away.",
				    ch, staff, NULL, TO_ROOM);
				if (--staff->value[2] <= 0) {
					act(AT_CYAN,
					    "$p blazes bright and is gone.", ch,
					    staff, NULL, TO_CHAR);
					act(AT_CYAN,
					    "$p blazes bright and is gone.", ch,
					    staff, NULL, TO_ROOM);
			...
			case 7:
				act(AT_CYAN,
				    "You can't control the power of $p, and it shatters!",
				    ch, staff, NULL, TO_CHAR);
		...
		for (vch = ch->in_room->people; vch; vch = vch_next) {
			...
			obj_cast_spell(staff->value[3], staff->level, ch, vch,
				       NULL);
	...
	if (!(staff->value[1] == -1))
		if (--staff->value[2] <= 0) {
			act(AT_WHITE, "Your $p blazes bright and is gone.", ch,
			    staff, NULL, TO_CHAR);
			act(AT_WHITE, "$n's $p blazes bright and is gone.", ch,
			    staff, NULL, TO_ROOM);
			extract_obj(staff);
		}
...
void do_stare(CHAR_DATA * ch, char *argument)
{
	...
	if (wand->item_type != ITEM_LENSE) {
		send_to_char(AT_BLUE,
			     "You can only stare with magical lenses.\n\r", ch);
		return;
	}
	...
	if ((wand->value[2] > 0) || (wand->value[1] == -1)) {
		if (victim) {
			act(AT_BLUE, "You stare at $N with $p.", ch, wand,
			    victim, TO_CHAR);
			act(AT_BLUE, "$n stares at $N with $p.", ch, wand,
			    victim, TO_ROOM);
		} else {
			act(AT_BLUE, "You stare at $P with $p.", ch, wand, obj,
			    TO_CHAR);
			act(AT_BLUE, "$n stares at $P with $p.", ch, wand, obj,
			    TO_ROOM);
		}
	...
	if (!(wand->value[1] == -1))
		if (--wand->value[2] <= 0) {
			act(AT_WHITE, "Your $p melts in your eyes.", ch, wand,
			    NULL, TO_CHAR);
			act(AT_WHITE, "$n's $p melts in $s eyes.", ch, wand,
			    NULL, TO_ROOM);
			extract_obj(wand);
		}
...
void do_zap(CHAR_DATA * ch, char *argument)
{
	...
	if ((wand->value[2] > 0) || (wand->value[1] == -1)) {
		if (victim) {
			act(AT_BLUE, "You zap $N with $p.", ch, wand, victim,
			    TO_CHAR);
			act(AT_BLUE, "$n zaps $N with $p.", ch, wand, victim,
			    TO_ROOM);
		} else {
			act(AT_BLUE, "You zap $P with $p.", ch, wand, obj,
			    TO_CHAR);
			act(AT_BLUE, "$n zaps $P with $p.", ch, wand, obj,
			    TO_ROOM);
		}
		...
		if (!IS_NPC(ch)
			...
			case 7:
				act(AT_CYAN,
				    "You can't control the power of $p, and it explodes!",
				    ch, wand, NULL, TO_CHAR);
				act(AT_CYAN, "$p explodes into fragments!", ch,
				    wand, NULL, TO_ROOM);
				extract_obj(wand);
	...
	if (!(wand->value[1] == -1))
		if (--wand->value[2] <= 0) {
			act(AT_WHITE, "Your $p explodes into fragments.", ch,
			    wand, NULL, TO_CHAR);
			act(AT_WHITE, "$n's $p explodes into fragments.", ch,
			    wand, NULL, TO_ROOM);
			extract_obj(wand);
		}
...
void do_steal(CHAR_DATA * ch, char *argument)
{
	...
	if (abs(ch->level - victim->level) > 5 && (!IS_NPC(victim))) {
		send_to_char(AT_WHITE,
			     "That is not in the pkill range... valid range is +/- 5 levels.\n\r",
			     ch);
		return;
	}
	if (victim == ch) {
	...
	if (ch->level + 5 < victim->level
		...
		if (ch->guild && !strcmp(ch->guild->name, "MERCENARY")) {
			send_to_char(AT_RED,
				     "You are kicked out of &rMERCENARY&R for treason!\n\r",
				     ch);
			ch->guild = NULL;
			ch->guild_rank = 0;
		}
		...
		if (!IS_NPC(ch)) {
			if (IS_NPC(victim)) {
				...
				sprintf(buf, "$N tried to steal from %s.",
					victim->name);
...
/*	|| !str_cmp   ( arg1, "gold"  ) )  */
	{
		...
		if (amount.gold <= 0) {
			amount.silver =
			    victim->money.silver * number_range(1, 10) / 100;
			if (amount.silver <= 0)
				amount.copper =
				    victim->money.copper * number_range(1,
									10) /
				    100;
			if (amount.copper <= 0) {
	...
	if (fBuy) {
		...
		for (itype = 0; itype < MAX_TRADE; itype++) {
			if (obj->item_type == pShop->buy_type[itype]) {
				new_cost.gold =
				    obj->cost.gold * pShop->profit_sell / 100;
				new_cost.silver =
				    obj->cost.silver * pShop->profit_sell / 100;
				new_cost.copper =
				    obj->cost.copper * pShop->profit_sell / 100;
				break;
			}
		}
		...
		for (obj2 = keeper->carrying; obj2; obj2 = obj2->next_content) {
			if (obj->pIndexData == obj2->pIndexData) {
				new_cost.gold =
				    (new_cost.gold > 0) ? new_cost.gold / 2 : 0;
				new_cost.silver =
				    (new_cost.silver >
				     0) ? new_cost.silver / 2 : 0;
				new_cost.copper =
				    (new_cost.copper >
				     0) ? new_cost.copper / 2 : 0;
			}
		}
	}
...
void do_buy(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP)) {
		...
		if (noi > 1) {
			send_to_char(AT_CYAN,
				     "You can only buy one pet at a time.\n\r",
				     ch);
			return;
		}
...
/* Convert ch coins and pet cost to copper to compare */
		...
		if (!cost
		    || (cost->gold == 0 && cost->silver == 0
			&& cost->copper == 0) || !can_see_obj(ch, obj)) {
			act(AT_CYAN,
		...
		if (!IS_SET(obj->extra_flags, ITEM_INVENTORY) && noi > 1) {
			send_to_char(AT_WHITE,
				     "You can only buy one of those at a time.\n\r",
				     ch);
			return;
		}
		...
		if ((ch->money.gold * C_PER_G + ch->money.silver * S_PER_G +
		     ch->money.copper) <
		    (cost->gold * C_PER_G + cost->silver * S_PER_G +
		     cost->copper)) {
			if (noi == 1)
				sprintf(log_buf,
				...
				sprintf(log_buf,
					"$n tells you 'You can't afford to buy %d $ps.",
					noi);
		...
		if (ch->carry_number + (get_obj_number(obj) * noi) >
		    can_carry_n(ch)) {
			send_to_char(AT_CYAN,
		...
		if (haggled) {
			sprintf(log_buf, "You haggle with $N and pay %s",
				money_string(cost));
			act(AT_GREY, log_buf, ch, NULL, keeper, TO_CHAR);
			act(AT_GREY, "$n haggles with $N.", ch, NULL, keeper,
			    TO_ROOM);
		}
		...
		if (noi == 1) {
			...
			sprintf(log_buf, "You buy %d $p%s.", noi,
				(noi > 1) ? "s" : "");
			act(AT_WHITE, log_buf, ch, obj, NULL, TO_CHAR);
			sprintf(log_buf, "$n buys %d $p%s.", noi,
				(noi > 1) ? "s" : "");
		...
		if (haggled) {
			sprintf(log_buf,
				"You haggle with $N and pay %d instead of %d.",
				cost, (int)(cost / 0.85));
			act(AT_GREY, log_buf, ch, NULL, keeper, TO_CHAR);
			act(AT_GREY, "$n haggles with $N.", ch, NULL, keeper,
			    TO_ROOM);
		...
		if (IS_SET(obj->extra_flags, ITEM_INVENTORY)) {
			for (in = 1; in <= noi; in++) {
				obj =
				    create_object(obj->pIndexData, obj->level);
				obj_to_char(obj, ch);
			}
		} else {
			obj_from_char(obj);
		...
		for (obj = keeper->carrying; obj; obj = obj->next_content) {
			...
			if (can_see_obj(ch, obj)
				...
				if (!found) {
					found = TRUE;
					strcat(buf1,
					       "[Lv Gold Silv Copp] Item\n\r");
				}
#ifdef NEW_MONEY
				sprintf(buf, "[%2d %4d %4d %4d] %s.\n\r",
					obj->level, amt->gold, amt->silver,
					amt->copper,
...
void do_sell(CHAR_DATA * ch, char *argument)
{
	...
	if ((!cost)
		...
		act(AT_CYAN, "$n looks uninterested in $p.", keeper, obj, ch,
		    TO_VICT);
		return;
	}
...
void do_value(CHAR_DATA * ch, char *argument)
{
	...
	if (!can_see_obj(keeper, obj)) {
		act(AT_CYAN,
		    "$n tells you 'You are offering me an imaginary object!?!?'.",
		    keeper, NULL, ch, TO_VICT);
		ch->reply = keeper;
		return;
	}
...
void do_poison_weapon(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch)
		...
		send_to_char(AT_DGREEN,
			     "What do you think you are, a thief?\n\r", ch);
		return;
	}
	...
	if (!IS_NPC(ch) && get_curr_wis(ch) < 19) {
		send_to_char(AT_DGREEN,
			     "You can't quite remember what to do...\n\r", ch);
		return;
	}
	...
	if (!IS_NPC(ch)
		...
		act(AT_DGREEN, "$n spills the poison all over!", ch, NULL, NULL,
		    TO_ROOM);
		return;
	}

	/* Well, I'm tired of waiting.  Are you? */
	act(AT_GREEN, "You mix a deadly poison!", ch, NULL, NULL, TO_CHAR);
	act(AT_GREEN, "$n mixes a deadly poison!", ch, NULL, NULL, TO_ROOM);
...
void do_acmorph(CHAR_DATA * ch, OBJ_DATA * obj, int vnum)
{
	...
	if (!(pObjIndex = get_obj_index(vnum))) {
		act(AT_BLUE, "$p whines and sparks, but nothing happens", ch,
		    obj, NULL, TO_CHAR);
		return;
	}
	...
	act(AT_BLUE, "$p's form wavers, then solidifies as $P.", ch, obj, nObj,
	    TO_CHAR);
	act(AT_BLUE, "$n's $p wavers in form. then solidifies as $P.", ch, obj,
	    nObj, TO_ROOM);
...
void do_actrans(CHAR_DATA * ch, OBJ_DATA * obj, int vnum)
{
	...
	if (ch->in_room->vnum == 8) {
		send_to_char(AT_RED,
			     "Such items do not work for those sent to Hell.\n\r",
			     ch);
		return;
	}
	...
	if (ch->fighting) {
		act(AT_BLUE, "$p pulses lightly, but fail to function.", ch,
		    obj, NULL, TO_CHAR);
		return;
	}
	act(AT_BLUE,
	    "Everything begins to spin, when it clears you are elsewhere.", ch,
	    obj, NULL, TO_CHAR);
	act(AT_BLUE, "$n invokes $p and vanishes in a swirling red mist.", ch,
	    obj, NULL, TO_ROOM);
	char_from_room(ch);
	char_to_room(ch, location);
	act(AT_BLUE, "$n arrives in a swirling red mist.", ch, obj, NULL,
	    TO_ROOM);
	do_look(ch, "auto");
...
void do_invoke(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->level < obj->level) {
		send_to_char(AT_BLUE,
			     "You have not attained the level of mastery to use this item",
			     ch);
		act(AT_BLUE, "$n tries to use $p, but is too inexperienced.",
		    ch, obj, NULL, TO_ROOM);
		return;
...
*/
	if (gets_zapped(ch, obj)) {
		act(AT_BLUE, "You are zapped by $p and drop it.", ch, obj, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n is zapped by $p and drops it.", ch, obj, NULL,
		    TO_ROOM);
		obj_from_char(obj);
		obj_to_room(obj, ch->in_room);
		return;
	...
	if (obj->ac_charge[1] != -1)
		if (--obj->ac_charge[0] <= 0) {
			act(AT_WHITE, "Your $p sputters and sparks.", ch, obj,
			    NULL, TO_CHAR);
			act(AT_WHITE, "$n's $p sputters and sparks..", ch, obj,
			    NULL, TO_ROOM);
			obj->ac_type = 0;
...
void do_voodo(CHAR_DATA * ch, char *argument)
{
	...
	obj_to_char(obj, ch);
	act(AT_RED,
	    "You call upon the dark forces of Retribution to create $p.", ch,
	    obj, NULL, TO_CHAR);
	act(AT_RED,
	    "$n calls upon the dark forces of Retribution to create $p.", ch,
	    obj, NULL, TO_ROOM);
	return;
...
/* deposit gold, silver, copper <amount> or deposits all */
	...
	if (is_number(arg2)) {
		if (!str_cmp(arg, "gold"))
			...
			send_to_char(AT_WHITE, "Invalid amount of money.\n\r",
				     ch);
			send_to_char(AT_WHITE,
				     "&WSyntax: deposit <currency_type> <amount>\n\r",
				     ch);
			send_to_char(AT_WHITE,
				     "&W    or: deposit <currency_type> <amount> clan\n\r",
				     ch);
			return;
		}
	}
	...
	if ((!is_number(arg2) && (str_cmp(arg, "all"))) || (atoi(arg2) < 0)) {
		send_to_char(AT_WHITE, "Invalid amount of money.\n\r", ch);
		send_to_char(AT_WHITE,
			     "&WSyntax: deposit <currency_type> <amount>\n\r",
			     ch);
		...
		if (!str_cmp(arg3, "clan")) {
			if ((IS_SET
			     (ch->in_room->area->area_flags, AREA_CLAN_HQ))
			    && (pClan != 0)) {
		...
		if (clan_bank) {
			sprintf(buf,
				"You deposit into your clan bankaccount %s\n\r",
				money_string(&amount));
			sprintf(buf + strlen(buf),
				"Your current clan balance is %s\n\r",
		...
		if (clan_bank)
			sprintf(buf,
				"&w$n deposits into the clan bankaccount %s",
				money_string(&amount));
		else
	...
	if ((!is_number(arg) && (str_cmp(arg, "all"))) || (money > ch->gold)
		...
		ch->gold -= money;
		sprintf(arg,
			"You deposit %d coin%s, bringing your total bank balance to %d.\n\r",
...
/* Withdraw gold, silver, copper <amount> or withdraw all */
	...
	if (is_number(arg2)) {
		if (!str_cmp(arg, "gold"))
			...
			send_to_char(AT_WHITE, "Invalid amount of money.\n\r",
				     ch);
			send_to_char(AT_WHITE,
				     "&WSyntax: withdraw <currency_type> <amount>\n\r",
				     ch);
			send_to_char(AT_WHITE,
				     "&W    or: withdraw <currency_type> <amount> clan\n\r",
				     ch);
			return;
		}
	}
	...
	if (!str_cmp(arg3, "clan")) {
		if ((IS_SET(ch->in_room->area->area_flags, AREA_CLAN_HQ)) &&
			...
			if ((amount.gold > pClan->bankaccount.gold) ||
				...
				send_to_char(AT_WHITE,
					     "You clan bankaccount doesn't hold that much money.\n\r",
					     ch);
				return;
			} else
				clan_bank = TRUE;
		...
		if ((amount.gold > ch->pcdata->bankaccount.gold) ||
			...
			send_to_char(AT_WHITE,
				     "Your bank account doesn't hold that much money.\n\r",
				     ch);
			return;
		}
	}
	...
	if ((!is_number(arg2) && (str_cmp(arg, "all"))) || (atoi(arg2) < 0)) {
		...
		if (clan_bank) {
			...
			sprintf(buf,
				"You withdraw from the clan bankaccount %s\n\r",
				money_string(&amount));
			send_to_char(AT_WHITE, buf, ch);
			...
			send_to_char(AT_WHITE, buf, ch);
			sprintf(buf,
				"&w$n withdraws from the clan bankaccount %s\n\r",
				money_string(&amount));
			act(AT_WHITE, buf, ch, NULL, NULL, TO_ROOM);
			save_clans();
		} else {
	...
	if ((!is_number(arg) && str_cmp(arg, "all"))
	    || money > ch->pcdata->bankaccount || money < 0) {
		send_to_char(AT_WHITE, "Invalid ammount of money.\n\r", ch);
		return;
	} else {
		if (!str_cmp(arg, "all"))
		...
		ch->gold += money;
		sprintf(arg,
			"You withdraw %d coin%s from your account, leaving the balance at %d.\n\r",
			money, money > 1 ? "s" : "", ch->pcdata->bankaccount);
		send_to_char(AT_WHITE, arg, ch);
...
void do_repair(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "all")) {
		...
		for (pObj = ch->carrying; pObj; pObj = pObj->next_content) {
			...
			if (((pObj->cost.gold * 100) +
			     (pObj->cost.silver * 10) + (pObj->cost.copper)) >=
			    ((pObj->pIndexData->cost.gold * 100) +
			     (pObj->pIndexData->cost.silver * 10) +
			     (pObj->pIndexData->cost.copper)))
				continue;
			amount.gold =
			    (pObj->pIndexData->cost.gold - pObj->cost.gold);
			amount.silver =
			    (pObj->pIndexData->cost.silver - pObj->cost.silver);
			amount.copper =
			    (pObj->pIndexData->cost.copper - pObj->cost.copper);
			switch (pObj->item_type) {
			case ITEM_WEAPON:
				amount.gold =
				    (amount.gold * pObj->value[2]) /
				    (!pObj->value[1] ? 1 : pObj->value[1]);
				amount.silver =
				    (amount.silver * pObj->value[2]) /
				    (!pObj->value[1] ? 1 : pObj->value[1]);
				amount.copper =
				    (amount.copper * pObj->value[2]) /
				    (!pObj->value[1] ? 1 : pObj->value[1]);
				break;
			case ITEM_ARMOR:
				amount.gold =
				    (amount.gold * pObj->level) /
				    (!pObj->value[0] ? 1 : pObj->value[0]);
				amount.silver =
				    (amount.silver * pObj->level) /
				    (!pObj->value[0] ? 1 : pObj->value[0]);
				amount.copper =
				    (amount.copper * pObj->level) /
				    (!pObj->value[0] ? 1 : pObj->value[0]);
				break;
			default:
			...
			if (((amount.gold * 100) + (amount.silver * 10) +
			     (amount.copper)) >
			    ((ch->money.gold * 100) + (ch->money.silver * 10) +
			     (ch->money.copper))) {
				sprintf(buf,
					"$p will cost %s You lack the funds.",
					money_string(&amount));
			...
			sprintf(buf, "To repair $p, you are charged %s",
				money_string(&amount));
			act(AT_WHITE, buf, ch, pObj, NULL, TO_CHAR);
	...
	if (((ch->money.gold * 100) + (ch->money.silver * 10) +
	     (ch->money.copper)) <
	...
	if (!str_cmp(arg, "all")) {
		...
		for (pObj = ch->carrying; pObj; pObj = pObj->next_content) {
			...
			if (cost > ch->gold) {
				sprintf(buf,
					"$p will cost %d, you lack the funds.",
					cost);
				act(AT_WHITE, buf, ch, pObj, NULL, TO_CHAR);
				continue;
			}
			...
			REMOVE_BIT(pObj->extra_flags, ITEM_PATCHED);
			sprintf(buf, "You are charged %d for repairing $p.",
				cost);
			act(AT_WHITE, buf, ch, pObj, NULL, TO_CHAR);
		}
		return;
...
/*       return; */
			...
			len = strlen(ch->name);
			send_to_char(AT_WHITE,
				     "You have nothing in your account!\n\r",
				     ch);
			sprintf(buf,
...
void do_store(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_SET(ch->in_room->room_flags, ROOM_BANK)) {
		send_to_char(AT_WHITE,
			     "You must be in a bank to store items.\n\r", ch);
		return;
	}
	if (IS_SET(ch->act, PLR_THIEF)) {
		send_to_char(AT_WHITE,
			     "This Bank refuses to transact with thieves!\n\r",
			     ch);
		return;
	}
	if (argument[0] == '\0') {
...
#ifdef NEW_MONEY
	if (((ch->pcdata->bankaccount.gold * 100) +
	     (ch->pcdata->bankaccount.silver * 10) +
	...
	send_to_char(AT_WHITE,
		     "The bank deducts 1000 gold coins from your account.\n\r",
		     ch);
}
...
void do_retrieve(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_SET(ch->in_room->room_flags, ROOM_BANK)) {
		send_to_char(AT_WHITE,
			     "You must be in a bank to retrieve items.\n\r",
			     ch);
		return;
	}
	...
	if (!(obj = get_obj_storage(ch, argument))) {
		send_to_char(AT_WHITE,
			     "You do not have that object in storage.\n\r", ch);
		return;
	}
	if (IS_SET(ch->act, PLR_THIEF)) {
		send_to_char(AT_WHITE,
...
void do_patch(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->pcdata->learned[gsn_patch] <= 0) {
		send_to_char(C_DEFAULT,
			     "You don't know how to patch equipment.\n\r", ch);
		return;
	}
	...
	if (!str_cmp(arg, "all")) {
		...
		for (obj = ch->carrying; obj; obj = obj->next_content) {
...
#ifdef NEW_MONEY
			if (((obj->cost.gold * 100) + (obj->cost.silver * 10) +
			     (obj->cost.copper)) >=
			    ((obj->pIndexData->cost.gold * 100) +
			     (obj->pIndexData->cost.silver * 10) +
			...
			if (obj->item_type != ITEM_WEAPON
				...
				act(AT_WHITE,
				    "You can't repair $p it's not a weapon or armor.",
				    ch, obj, NULL, TO_CHAR);
				continue;
			}
			...
			if (IS_SET(obj->extra_flags, ITEM_PATCHED)) {
				act(AT_WHITE,
				    "You can't do anything more for $p.", ch,
				    obj, NULL, TO_CHAR);
				continue;
			}
			...
			amt.gold =
			    (ammount *
			     (obj->pIndexData->cost.gold -
			      obj->cost.gold)) / 100;
			amt.silver =
			    (ammount *
			     (obj->pIndexData->cost.silver -
			      obj->cost.silver)) / 100;
			amt.copper =
			    (ammount *
			     (obj->pIndexData->cost.copper -
...
#else
			ammount =
			    (ammount * (obj->pIndexData->cost - obj->cost)) /
			    100;
			obj->cost += ammount;
#endif
			SET_BIT(obj->extra_flags, ITEM_PATCHED);

			act(AT_WHITE, "You do your best to repair $p.", ch, obj,
			    NULL, TO_CHAR);
		}
		return;
	}
	...
	if (obj->item_type != ITEM_WEAPON && obj->item_type != ITEM_ARMOR) {
		send_to_char(C_DEFAULT,
			     "You may only repair weapons and armor.\n\r", ch);
		return;
	}
...
#ifdef NEW_MONEY
	amt.gold =
	    (ammount * (obj->pIndexData->cost.gold - obj->cost.gold)) / 100;
	amt.silver =
	    (ammount * (obj->pIndexData->cost.silver - obj->cost.silver)) / 100;
	amt.copper =
	    (ammount * (obj->pIndexData->cost.copper - obj->cost.copper)) / 100;
	obj->cost.gold += amt.gold;
	obj->cost.silver += amt.silver;
	obj->cost.copper += amt.copper;
#else
	...
	act(AT_WHITE, "You do your best to repair your $p.", ch, obj, NULL,
	    TO_CHAR);
...
void do_alchemy(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch)
		...
		send_to_char(AT_WHITE,
			     "What do you think you are, a cleric?\n\r", ch);
		return;
	}
	...
	if (ch->mana < mana) {
		send_to_char(AT_WHITE,
			     "You lack the mana to bind the elixer.\n\r", ch);
		return;
	}
	ch->mana -= mana;
	...
	if (number_percent() > chance) {
		...
		extract_obj(fobj);
		send_to_char(AT_RED,
			     "All your alchemy equipment is ruined!\n\r", ch);
...
#ifdef NEW_MONEY
	pobj->cost.gold =
	    ch->level * skill_table[sn].skill_level[prime_class(ch)];
#else
	...
	sprintf(buf, "A potion of %s has been left here.",
		skill_table[sn].name);
...
void do_scribe(CHAR_DATA * ch, char *argument)
{
	...
	if (!pobj) {
		send_to_char(AT_WHITE,
			     "You do not have the blank parchment.\n\r", ch);
		return;
	}
	for (cobj = ch->carrying; cobj; cobj = cobj->next_content) {
	...
	if (ch->mana < mana) {
		send_to_char(AT_WHITE,
			     "You lack the mana to bind the words to the parchment.\n\r",
			     ch);
		return;
	}
	ch->mana -= mana;
	...
	if (number_percent() > chance) {
		...
		extract_obj(fobj);
		send_to_char(AT_RED,
			     "All your scribery equipment is ruined!\n\r", ch);
		damage(ch, ch, dam, gsn_incinerate);
		af.type = gsn_incinerate;
		af.level = skill_table[sn].skill_level[prime_class(ch)];
		af.duration = 5;
	...
	free_string(pobj->short_descr);
	pobj->short_descr = str_dup(buf);
	sprintf(buf, "A scroll of %s has been left here.",
		skill_table[sn].name);
	free_string(pobj->description);
	pobj->description = str_dup(buf);
	if (!is_name(NULL, pobj->name, skill_table[sn].name)) {
...
void do_gravebind(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->pcdata->learned[gsn_gravebind] < number_percent()) {
		send_to_char(AT_DGREY,
			     ...
			     ch);
		act(AT_DGREY,
		    "$n's gravebind of the $p fails and it is destroyed.", ch,
		    obj, NULL, TO_ROOM);
		extract_obj(obj);
		return;
	}
...
void do_indestructable(CHAR_DATA * ch, char *argument)
{
	...
	if (number_percent() < (85 - (obj->level * .35))) {
		...
		act(AT_WHITE,
		    ...
		    ch, obj, NULL, TO_CHAR);
		act(AT_WHITE,
		    "The artifactor hands $n back $p.", ch, obj, NULL, TO_ROOM);
...
void do_flamehand(CHAR_DATA * ch, char *argument)
{
	...
	if (is_affected(ch, gsn_flamehand))
		return;
	if (IS_NPC(ch) || !can_use_skpell(ch, gsn_flamehand)) {
	...
	if (ch->pcdata->learned[gsn_flamehand] < number_percent()) {
		send_to_char(AT_GREY,
			     "You cannot seem to control your chi.\n\r", ch);
		return;
	}
...
void do_frosthand(CHAR_DATA * ch, char *argument)
{
	...
	if (is_affected(ch, gsn_frosthand))
		return;
	if (IS_NPC(ch) || !can_use_skpell(ch, gsn_frosthand)) {
		send_to_char(AT_GREY, "Huh?\n\r", ch);
		return;
	}
	...
	act(AT_GREY, "$n goes into a trance, $s palms facing to the sky.",
	    ch, NULL, NULL, TO_ROOM);
	send_to_char(AT_GREY,
		     "Your hands become freezing and are covered with &Cfrost &wand &Cice&w.\n\r",
		     ch);
	act(AT_GREY, "$n's hands slowly seem to turn to &Cice&w.", ch, NULL,
	    NULL, TO_ROOM);
...
void do_chaoshand(CHAR_DATA * ch, char *argument)
{
	...
	if (is_affected(ch, gsn_chaoshand))
		return;
	if (IS_NPC(ch) || !can_use_skpell(ch, gsn_chaoshand)) {
		send_to_char(AT_GREY, "Huh?\n\r", ch);
		return;
	}
...
void do_ironfist(CHAR_DATA * ch, char *argument)
{
	...
	if (is_affected(ch, gsn_ironfist))
		return;
	if (IS_NPC(ch) || !can_use_skpell(ch, gsn_ironfist)) {
		send_to_char(AT_GREY, "Huh?\n\r", ch);
		return;
	}
...
void do_antidote(CHAR_DATA * ch, char *argument)
{
	...
	if (!flask) {
		send_to_char(AT_GREY,
			     "You need a flask to put your antidote in.\n\r",
			     ch);
		return;
	}
	...
	flask->description =
	    str_dup
	    ("A small flask filled with a greenish liquid sets upon the floor.");
