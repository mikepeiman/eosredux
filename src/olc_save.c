...
void save_clans()
{
	...
	if ((fp = fopen("clan.dat", "w")) == NULL) {
		...
		for (pClan = clan_first; pClan; pClan = pClan->next) {
			...
			fprintf(fp, "%d %d %d\n", pClan->obj_vnum_1,
				pClan->obj_vnum_2, pClan->obj_vnum_3);
			fprintf(fp, "%d\n", pClan->settings);
		}
	...
	for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++) {
		if ((pObjIndex = get_obj_index(vnum))) {
			if (pObjIndex->area == pArea) {
				...
				fprintf(fp, "%d %d %d 0\n",
					pObjIndex->cost.gold,
					pObjIndex->cost.silver,
					pObjIndex->cost.copper);
#else
				fprintf(fp, "%d 0\n", pObjIndex->cost);
#endif
		...
		if ((pMobIndex = get_mob_index(vnum))) {
			if (pMobIndex->area == pArea && pMobIndex->pShop) {
				...
				fprintf(fp, "%3d %3d ", pShopIndex->profit_buy,
					pShopIndex->profit_sell);
				fprintf(fp, "%2d %2d ", pShopIndex->open_hour,
					pShopIndex->close_hour);
				fprintf(fp, "\t; %s\n",
					get_mob_index(pShopIndex->keeper)->
					short_descr);
			}
		}
	}
...
void do_asave(CHAR_DATA * ch, char *argument)
{
	...
	if (is_number(arg1)) {
		...
		save_area(pArea);
		REMOVE_BIT(pArea->area_flags,
			   AREA_VERBOSE | AREA_CHANGED | AREA_ADDED);
