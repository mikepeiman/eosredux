...
bool gets_zapped(CHAR_DATA * ch, OBJ_DATA * obj)
{
	if (!IS_NPC(ch))
		if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch))
		    ...
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_MAGE)
			&& is_class(ch, CLASS_MAGE))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_CLERIC)
			&& is_class(ch, CLASS_CLERIC))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_THIEF)
			&& is_class(ch, CLASS_THIEF))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_WARRIOR)
			&& is_class(ch, CLASS_WARRIOR))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_PSI)
			&& is_class(ch, CLASS_PSIONICIST))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_DRUID)
			&& is_class(ch, CLASS_DRUID))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_RANGER)
			&& is_class(ch, CLASS_RANGER))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_PALADIN)
			&& is_class(ch, CLASS_PALADIN))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_BARD)
			&& is_class(ch, CLASS_BARD))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_VAMP)
			&& is_class(ch, CLASS_VAMPIRE))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_NECRO)
			&& is_class(ch, CLASS_NECROMANCER))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_WWF)
			&& is_class(ch, CLASS_WEREWOLF))
		    || (IS_ANTI_CLASS(obj, ITEM_ANTI_MONK)
			&& is_class(ch, CLASS_MONK))
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_HUMAN)
			&& ch->race == RACE_HUMAN)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_ELF)
			&& ch->race == RACE_ELF)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_DWARF)
			&& ch->race == RACE_DWARF)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_PIXIE)
			&& ch->race == RACE_PIXIE)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_HALFLING)
			&& ch->race == RACE_HALFLING)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_DROW)
			&& ch->race == RACE_DROW)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_ELDER)
			&& ch->race == RACE_ELDER)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_OGRE)
			&& ch->race == RACE_OGRE)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_LIZARDMAN)
			&& ch->race == RACE_LIZARDMAN)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_DEMON)
			&& ch->race == RACE_DEMON)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_GHOUL)
			&& ch->race == RACE_GHOUL)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_ILLITHID)
			&& ch->race == RACE_ILLITHID)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_MINOTAUR)
			&& ch->race == RACE_MINOTAUR)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_SHADOW)
			&& ch->race == RACE_SHADOW)
		    || (IS_ANTI_RACE(obj, ITEM_ANTI_TABAXI)
			&& ch->race == RACE_TABAXI))
			return TRUE;
	return FALSE;
}
...
/* Angi: Make max carry weight less than the weight of your coins. */
	...
	max_weight = ((str_app[get_curr_str(ch)].carry) -
		      (ch->money.gold / 1000 + ch->money.silver / 1000 +
		       ch->money.copper / 1000));
	if (max_weight < 0)
		max_weight = 0;
...
/* XORPHOX perm spells */
	...
	case APPLY_HEIGHTEN_SENSES:
		...
		if (fAdd) {
			...
			if (skill_table[sn].room_msg_off)
				act(AT_BLUE, skill_table[sn].room_msg_off, ch,
				    NULL, NULL, TO_ROOM);
		}
		break;
	case APPLY_SNEAK:
	...
	case APPLY_SHOCKSHIELD:
		...
		strcpy(buf,
		       "The electricity about $n's body flee's into the ground.");
		break;
	...
	if (psn != -1) {
		if (fAdd && !is_affected(ch, psn)) {
			...
			if (skill_table[psn].msg_off) {
				send_to_char(AT_BLUE, skill_table[psn].msg_off,
					     ch);
				send_to_char(AT_BLUE, "\n\r", ch);
				if (skill_table[psn].room_msg_off) {
					act(AT_BLUE,
					    skill_table[psn].room_msg_off, ch,
					    NULL, NULL, TO_ROOM);
				}
			}
		}
...
{
	if (IS_NPC(ch)) {
		bug("obj_to_storage: NPC storage from %d",
		    ch->pIndexData->vnum);
		obj_to_char(obj, ch);
		return;
	}
...
void obj_from_storage(OBJ_DATA * obj)
{
	...
	if (IS_NPC(ch)) {
		bug("obj_from_storage: NPC storage by %d.",
		    ch->pIndexData->vnum);
		return;
	}
	...
	if (ch->pcdata->storage == obj) {
		...
		for (prev = ch->pcdata->storage; prev;
...
void extract_char(CHAR_DATA * ch, bool fPull)
{
	...
	if (!fPull) {
		...
		if (is_arena
		    && (location = get_room_index(ROOM_ARENA_HALL_SHAME)))
			...
			sprintf(buf, "You awaken in the morgue%s",
				ch->level <=
				20 ? ", your battered corpse next to you.\n\r" :
				".\n\r");
...
OBJ_DATA *get_obj_storage(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch)) {
		bug("get_obj_storage: NPC storage from %d",
		    ch->pIndexData->vnum);
		return NULL;
	}
...
OBJ_DATA *create_money(MONEY_DATA * amount)
{
	...
	if (((amount->gold <= 0) && (amount->silver <= 0)
	     && (amount->copper <= 0)) || (amount->gold < 0)
	    || (amount->silver < 0) || (amount->copper < 0)) {
		char buf[MAX_STRING_LENGTH];
		sprintf(buf, "Create_money: zero or negative money %d %d %d.",
			amount->gold, amount->silver, amount->copper);
		bug(buf, 0);
		amount->copper = 1;
		amount->gold = 0;
		amount->silver = 0;
	}
	if ((amount->gold == 1) && (amount->silver <= 0)
	    && (amount->copper <= 0))
		obj = create_object(get_obj_index(OBJ_VNUM_MONEY_ONE), 0);
	else if ((amount->silver == 1) && (amount->gold <= 0)
		 && (amount->copper <= 0))
		obj = create_object(get_obj_index(OBJ_VNUM_MONEY_ONE), 0);
	else if ((amount->copper == 1) && (amount->gold <= 0)
		 && (amount->silver <= 0))
		obj = create_object(get_obj_index(OBJ_VNUM_MONEY_ONE), 0);
	else
...
bool can_see(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (IS_AFFECTED(victim, AFF_HIDE)
	    ...
	    && !IS_AFFECTED2(ch, AFF_TRUESIGHT)
	    && !victim->fighting
	    && ch->race != RACE_DWARF && ch->race != RACE_DEMON)
		return FALSE;
...
char *item_type_name(OBJ_DATA * obj)
{
	...
	if (in_obj->carried_by)
		sprintf(buf,
			"Item_type_name: unknown type %d from %s owned by %s.",
