...
bool chat_command(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '>') {
		...
		act(num_color(ch), "$N &Y(to you): &G$t", victim, argument, ch,
		    TO_CHAR);
		act(AT_RED, "-- &CMessage directed to &$t$N &R--", ch,
		    get_color(victim), victim, TO_CHAR);
		return TRUE;
	}
	...
	if (!str_prefix(arg + 1, "quit"))
		...
		act(num_color(ch), "$N &C(&RPriv&C)&Y: &G$t", victim, argument,
		    ch, TO_CHAR);
		act(AT_RED, "-- &CMessage sent only to &$t$N &R--", ch,
		    get_color(victim), victim, TO_CHAR);
	}
	return TRUE;
}
...
void chat_join(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		if (!str_prefix(ch->name, ch->in_room->name) ||
			...
			ToMain = TRUE;
		act(num_color(ch), "$n &Yhas left the room.", ch, NULL, NULL,
		    TO_ROOM);
		char_from_room(ch);
		if (ToMain) {
			char_to_room(ch, chat_rooms->pRoom);
			send_room_stuff(ch);
			act(num_color(ch), "$n &Yhas entered the room.", ch,
			    NULL, NULL, TO_ROOM);
			return;
		}
		for (room = chat_rooms; room; room = room->next) {
			if (is_name(NULL, ch->name, room->pRoom->name)) {
				...
				send_room_stuff(ch);
				act(num_color(ch), "$n &Yhas entered the room.",
	...
	for (room = chat_rooms; room; room = room->next)
		if (!str_prefix(arg, room->pRoom->name) ||
			...
			act(num_color(ch), "$n &Yhas left the room.", ch, NULL,
			    NULL, TO_ROOM);
			char_from_room(ch);
			char_to_room(ch, room->pRoom);
			send_room_stuff(ch);
...
void chat_show(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf + strlen(buf), "There %s %d active room%s.\n\r",
		(rcount == 1 ? "is" : "are"), rcount, (rcount == 1 ? "" : "s"));
	...
	if (arg[0] == '\0') {
		...
		if (ncount == 0)
			send_to_char(AT_PURPLE,
				     "No one is invited to your room.", ch);
		send_to_char(C_DEFAULT, "\n\r\n\r", ch);
		return;
	}
...
void chat_uninvite(CHAR_DATA * ch, char *argument)
{
	...
	if (!victim)
		...
		if (is_name(NULL, name, room->invited)) {
			act(AT_PINK,
			    "You have been uninvited from &$t$N&P's room.",
			    victim, get_color(ch), ch, TO_CHAR);
