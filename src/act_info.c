...
void show_char_to_char_0(CHAR_DATA * victim, CHAR_DATA * ch)
{
	...
	if (IS_AFFECTED(victim, AFF_SANCTUARY)) {
		...
		strcat(buf2,
		       "&W's body is veiled in a glowing white mist.\n\r");
		buf2[4] = UPPER(buf2[4]);
	...
	if (IS_AFFECTED2(victim, AFF_BLADE)) {
		...
		strcat(buf2,
		       "&w is surrounded by thousands of spinning blades.\n\r");
		buf2[4] = UPPER(buf2[4]);
	...
	if (IS_AFFECTED2(victim, AFF_DANCING)) {
		...
		strcat(buf2,
		       " &.is s&.urrounde&.d by &.thou&.sand&.s of &.danci&.ng l&.igh&.ts&w.\n\r");
		buf2[4] = UPPER(buf2[4]);
		send_to_char(AT_GREY, buf2, ch);
		buf2[0] = '\0';
	}
...
/* Put the check here for argument = storage.. display their storage 
   and then return w/o showing char stuff   --Angi */
	...
	if ((!str_cmp(argument, "storage")) && (get_trust(ch) > L_DIR)) {
		if (IS_NPC(victim)) {
			send_to_char(AT_WHITE,
				     "NPC's do not have items in storage.\n\r",
				     ch);
		} else {
			sprintf(buf, "%s's storage box contains:\n\r",
				victim->name);
			send_to_char(AT_WHITE, buf, ch);
			show_list_to_char(victim->pcdata->storage, ch, TRUE,
					  TRUE);
		}
		return;
	}
	...
	if ((argument[0] != '\0') && (get_trust(ch) > L_DIR)) {
		if (!(obj = get_obj_here(victim, argument))) {
			...
			case ITEM_DRINK_CON:
				...
				sprintf(buf,
					"%s %s has is %s full of a %s liquid.\n\r",
					capitalize(obj->short_descr),
					victim->name,
					obj->value[1] <
					obj->value[0] /
					4 ? "less than" : obj->value[1] <
					3 * obj->value[0] /
					4 ? "about" : "more than",
	...
	if (victim->description[0] != '\0') {
		...
		act(AT_GREY, "You see nothing special about $M.", ch, NULL,
		    victim, TO_CHAR);
	}
	...
	for (iWear = 0; iWear < MAX_WEAR; iWear++) {
		if ((obj = get_eq_char(victim, iWear))
			...
			if (!found) {
				...
				act(AT_WHITE, "$N is using:", ch, NULL, victim,
				    TO_CHAR);
				found = TRUE;
			}
			send_to_char(AT_BLUE, where_name[iWear], ch);
			send_to_char(AT_CYAN, format_obj_to_char(obj, ch, TRUE),
				     ch);
			...
			sprintf(buf, "%s%s%s&ca Werewolf Claw",
				is_affected(victim,
					    gsn_flamehand) ? "&r(Burning) " :
				"", is_affected(victim,
						gsn_chaoshand) ? "&Y(Chaotic) "
				: "", is_affected(victim,
						  gsn_frosthand) ? "&B(Frosty) "
				: "");
	...
	if (!str_prefix(arg1, "in")) {
		...
		case ITEM_PORTAL:
			if (!(portroom = get_room_index(obj->value[0]))) {
				act(AT_WHITE,
				    "You cannot see anything through $p", ch,
				    obj, NULL, TO_CHAR);
				break;
			}
			act(AT_GREEN, "You look into $p and see...", ch, obj,
			    NULL, TO_CHAR);
			act(AT_GREEN, "$n looks into $p.", ch, obj, NULL,
			    TO_ROOM);
			...
			if (arg1[0] == '\0'
			    || (!IS_NPC(ch) && !IS_SET(ch->act, PLR_BRIEF)))
				send_to_char(AT_CYAN, portroom->description,
					     ch);
...
/*	show_char_to_char_1( victim, ch );  Changed arguments so you
	...
	if (!str_prefix(arg1, "north"))
		...
		sprintf(buf, "Value: %s  Price: %s\n\r",
			money_string(&auc_obj->cost), money_string(&auc_cost));
#else
		...
		if (arena.cch) {
			...
			if (fp > sp)
				sprintf(buf,
					"%s appears to be winning at the moment.\n\r",
					arena.fch->name);
			else if (sp > fp)
				sprintf(buf,
					"%s appears to be winning at the moment.\n\r",
				...
				strcpy(buf,
				       "They appear to be evenly matched at the moment.\n\r");
		}
		send_to_char(AT_WHITE, buf, ch);
		return;
	} else {
...
/*	send_to_char(AT_WHITE, pexit->to_room->name, ch );
	...
	if (pexit->keyword
		...
		if (IS_SET(pexit->exit_info, EX_BASHED))
			...
			act(AT_GREY, "The $d is closed.", ch, NULL,
			    pexit->keyword, TO_CHAR);
		else if (IS_SET(pexit->exit_info, EX_ISDOOR))
			act(AT_GREY, "The $d is open.", ch, NULL,
			    pexit->keyword, TO_CHAR);
	}
	...
	if ((obj = get_obj_here(ch, arg))) {
		...
		case ITEM_ARMOR:
			...
			if ((obj->pIndexData->cost.gold > 0) ||
				...
				brk =
				    (((obj->cost.gold * C_PER_G +
				       obj->cost.silver * S_PER_G +
				       obj->cost.copper) * 100) /
...
*/
			...
			act(AT_WHITE, "Looking closer, you see that $p $T", ch,
			    obj, msg, TO_CHAR);
			break;
...
void do_score(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(argument, "msg") && get_trust(ch) >= LEVEL_IMMORTAL) {
		send_to_char(AT_WHITE,
			     "Your personalized messages are as follows:\n\r",
			     ch);
	...
	sprintf(buf,
		...
		ch->level, get_age(ch), (get_age(ch) - 17) * 4);
	send_to_char(AT_CYAN, buf, ch);
	sprintf(buf,
	...
	if (ch->clan) {
		...
		if (ch->cquestpnts > 0) {
			sprintf(buf,
				"You currently have %d Clan Quest Points.\n\r",
				ch->cquestpnts);
			send_to_char(AT_WHITE, buf, ch);
		}
	}
	...
	sprintf(buf,
		...
		IS_NPC(ch) ? 13 : ch->pcdata->perm_str,
		IS_NPC(ch) ? 13 : get_curr_str(ch),
		IS_NPC(ch) ? 13 : ch->pcdata->perm_int,
		IS_NPC(ch) ? 13 : get_curr_int(ch),
		IS_NPC(ch) ? 13 : ch->pcdata->perm_wis,
		IS_NPC(ch) ? 13 : get_curr_wis(ch),
		IS_NPC(ch) ? 13 : ch->pcdata->perm_dex,
		IS_NPC(ch) ? 13 : get_curr_dex(ch),
		IS_NPC(ch) ? 13 : ch->pcdata->perm_con,
		IS_NPC(ch) ? 13 : get_curr_con(ch),
...
void do_time(CHAR_DATA * ch, char *argument)
{
	...
	if IS_IMMORTAL
		...
		sprintf(buf, "Running copy compiled at %s\r",
			(char *)ctime(&exe_comp_time));
		send_to_char(AT_RED, buf, ch);

		if (!stat(exe_file, &statis)) {
			sprintf(buf, "Storm compiled at %s\r",
				(char *)ctime(&statis.st_mtime));
			send_to_char(AT_RED, buf, ch);
		} else {
			if IS_IMMORTAL
				(ch)
				    send_to_char(AT_RED,
						 "No executable avable!!\r\n",
						 ch);
		}
		}
	return;
...
/*    fGuildRestrict = FALSE;  */
	...
	for (;;) {
		...
		if (is_number(arg)) {
			...
			if (strlen(arg) < 3) {
				send_to_char(AT_DGREEN,
					     "Classes must be longer than that.\n\r",
					     ch);
				return;
			}
			...
			if (!str_cmp(arg, "imm")) {
				...
				for (iClass = 0; iClass < MAX_CLASS; iClass++) {
					if ((!str_cmp
					     (arg, class_table[iClass].who_name)
					     && !lng)
					    ||
					    (!str_cmp
					     (arg1,
...
guild_table[iGuild].name ) )   */
		...
		if (wch->clan != 0) {
			...
			if (IS_SET(pClan->settings, CLAN_PKILL))
				...
				case 0:
					...
					break;
				case 1:
					sprintf(clan, "-<Centurion of %s>-",
						pClan->name);
					break;
				case 2:
					sprintf(clan, "-<Council of %s>-",
						pClan->name);
					break;
				case 3:
					sprintf(clan, "-<Leader of %s>-",
						pClan->name);
					break;
				case 4:
					sprintf(clan, "-<Champion of %s>-",
						pClan->name);
					break;
				case 5:
					sprintf(clan, "-<Deity of %s>-",
						pClan->name);
					break;
			} else
				switch (wch->clev) {
				...
				case 0:
					...
					break;
				case 1:
					sprintf(clan, "(Centurion of %s&C)",
						pClan->name);
					break;
				case 2:
					sprintf(clan, "(Council of %s&C)",
						pClan->name);
					break;
				case 3:
					sprintf(clan, "(Leader of %s&C)",
						pClan->name);
					break;
				case 4:
					sprintf(clan, "(Champion of %s&C)",
						pClan->name);
					break;
				case 5:
					sprintf(clan, "(Deity of %s&C)",
						pClan->name);
					break;
				}
		}
		 ...
		 */
		if (wch->level < LEVEL_HERO) {
			sprintf(buf + strlen(buf), "[%-7s %s] ", class, race);
	...
	if (nMatch > 0) {
		if (fNameRestrict)
			send_to_char(C_DEFAULT,
				     ...
				     ch);
		sprintf(buf2,
			"Mag&g[&G%d&g]&G Cle&g[&G%d&g]&G Thi&g[&G%d&g]&G "
			"War&g[&G%d&g]&G Psi&g[&G%d&g]&G Dru&g[&G%d&g]&G Rng&g[&G%d&g]&G "
			"Pal&g[&G%d&g]&G Brd&g[&G%d&g]&G Vam&g[&G%d&g]&G Nec&g[&G%d&g]&G\n\r",
			noclass[0], noclass[1], noclass[2], noclass[3],
			noclass[4], noclass[5], noclass[6], noclass[7],
			noclass[8], noclass[9], noclass[10]);
		strcat(buf, buf2);
		send_to_char(AT_GREEN, buf, ch);
		buf[0] = buf2[0] = '\0';
		sprintf(buf, "Wwf&g[&G%d&g] &GMnk&g[&G%d&g] ", noclass[11],
			noclass[12]);
		send_to_char(AT_GREEN, buf, ch);
		buf[0] = buf2[0] = '\0';
		...
		send_to_char(AT_GREEN,
			     "There is no class/race/clan/person by that name in the game.\n\r",
			     ch);
	return;
}
		...
		if (!(obj = get_eq_char(ch, iWear))) {
			if (is_class(ch, CLASS_WEREWOLF)
				...
				sprintf(buf, "%s%s%s&ca Werewolf Claw",
					is_affected(ch,
						    gsn_flamehand) ?
					"&r(Burning) " : "", is_affected(ch,
									 gsn_chaoshand)
					? "&Y(Chaotic) " : "", is_affected(ch,
									   gsn_frosthand)
					? "&B(Frosty) " : "");
				send_to_char(AT_CYAN, buf, ch);
				send_to_char(AT_CYAN, "\n\r", ch);
				found = TRUE;
				   ...
				   ((iWear == WEAR_WIELD
					...
					&& !get_eq_char(ch, WEAR_HOLD)
					&& !get_eq_char(ch, WEAR_SHIELD)
					&& ch->pcdata->learned[gsn_blackbelt] >
					0))) {
				send_to_char(AT_BLUE, where_name[iWear], ch);
...
void do_where(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		if (!found)
			act(AT_PINK, "You didn't find any $T.", ch, NULL, arg,
			    TO_CHAR);
	}
...
void do_consider(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(victim)) {
		send_to_char(C_DEFAULT,
			     "The gods do not accept this type of sacrifice.\n\r",
			     ch);
		return;
	}
...
void do_title(CHAR_DATA * ch, char *argument)
{
	...
	if (strlen_wo_col(argument) > 50) {
		send_to_char(C_DEFAULT,
			     "Max title length is 50 excluding color codes.",
			     ch);
		return;
	}
...
void do_report(CHAR_DATA * ch, char *argument)
{
	...
	if (!is_class(ch, CLASS_VAMPIRE))
		sprintf(buf,
			...
			ch->hit, MAX_HIT(ch),
			ch->mana, MAX_MANA(ch),
			ch->move, MAX_MOVE(ch), ch->exp);
	else
		sprintf(buf,
			"You report: %d/%d hp %d/%d bp %d/%d mv %d xp.\n\r",
			ch->hit, MAX_HIT(ch),
			ch->bp, MAX_BP(ch), ch->move, MAX_MOVE(ch), ch->exp);
...
/* Practice to approx 50% then use to learn */
		if (IS_NPC(ch) || ch->pcdata->learned[sn] > 35) {
			sprintf(buf,
				"&W%s tells you &c'&CI've already trained you all I can in %s&c'&w.\n\r",
				capitalize(mob->name), skill_table[sn].name);
			send_to_char(C_DEFAULT, buf, ch);
			sprintf(buf,
				"&W%s tells you &c'&CPractice in the real world for more experience&c'&w.\n\r",
				capitalize(mob->name));
			send_to_char(C_DEFAULT, buf, ch);
			return;
		}
...
*/
		...
		if (get_trust(ch) >= L_IMP) {
			send_to_char(AT_DGREY,
				     !IS_SET(ch->deaf, CHANNEL_GUARDIAN)
				     ? " +GUARD" : " -guard", ch);
		}
		...
		if (get_trust(ch) >= L_IMP) {
			send_to_char(AT_DGREY,
				     !IS_SET(ch->deaf, CHANNEL_CLASS_MASTER)
				     ? " +CLASSMASTER" : " -classmaster", ch);
		}

		if (get_trust(ch) >= L_IMP) {
			send_to_char(AT_DGREY,
				     !IS_SET(ch->deaf, CHANNEL_CLAN_MASTER)
				     ? " +CLANMASTER" : " -clanmaster", ch);
		}

		if (get_trust(ch) >= L_IMP) {
			send_to_char(AT_DGREY,
				     !IS_SET(ch->deaf, CHANNEL_GUILD_MASTER)
				     ? " +GUILDMASTER" : " -guildmaster", ch);
		}
	...
	if (arg[0] == '\0') {
		...
		if (arg[0] == '+')
			...
			send_to_char(AT_BLOOD,
				     "&WConfig &Y-&Woption or &Y+&Woption?\n\r",
				     ch);
			return;
		}
...
void do_slist(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch)) {
		send_to_char(AT_BLUE,
			     "You do not need any stinking spells!\n\r", ch);
		return;
	}
	...
	for (level = 1; level <= L_CHAMP3; level++) {
		...
		for (sn = 0; skill_table[sn].name[0] != '\0'; sn++) {
			...
			if (classes == 2
			    && skill_table[sn].skill_level[profession] != level
			    && skill_table[sn].skill_level[ch->class[1]] !=
			    level)
				continue;
...
void do_induct(CHAR_DATA * ch, char *argument)
{
	...
	if (((ch->clev == 1) && (!IS_SET(pClan->settings, CLAN_SECOND_INDUCT)))
	    || ((ch->clev == 2)
		&& (!IS_SET(pClan->settings, CLAN_FIRST_INDUCT)))
	    || ((ch->clev == 3)
		&& (!IS_SET(pClan->settings, CLAN_LEADER_INDUCT)))
	    || ((ch->clev == 4)
		&& (!IS_SET(pClan->settings, CLAN_CHAMP_INDUCT)))
	    ) {
		send_to_char(AT_WHITE,
			     "You can't induct in your current clan position.",
			     ch);
		return;
	}
	if (!(victim = get_char_room(ch, arg))) {
	...
	act(AT_RED, "$n has been inducted into $T.", victim, NULL, buf,
	    TO_ROOM);
...
void do_outcast(CHAR_DATA * ch, char *argument)
{
	...
	clname = pClan->name;
	sprintf(buf + strlen(buf), "<%s>", clname);
	act(AT_RED, "$n has been outcasted from $T.", victim, NULL, buf,
	    TO_ROOM);
	act(AT_RED, "You are no longer a member of $T.", victim, NULL, buf,
	    TO_CHAR);
...
void do_setlev(CHAR_DATA * ch, char *argument)
{
	...
	case 1:
		if (pClan->issecond) {
			send_to_char(AT_WHITE,
				     "Clan already has a Centurion, defaulting to regular member",
				     ch);
			level = 0;
			break;
		} else {
	...
	case 2:
		if (pClan->isfirst) {
			send_to_char(AT_WHITE,
				     "Clan already has a Council, defaulting to regular member",
				     ch);
			level = 0;
			break;
		} else {
	...
	case 3:
		if (pClan->isleader) {
			send_to_char(AT_WHITE,
				     "Clan already has a Leader, defaulting to regular member",
				     ch);
			level = 0;
			break;
		} else {
	...
	case 4:
		if (pClan->ischamp) {
			send_to_char(AT_WHITE,
				     "Clan already has a Champion, defaulting to regular member",
				     ch);
			level = 0;
			break;
		} else {
	...
	if (level <= victim->clev) {
		...
		act(AT_BLUE, "You have been lowered to $T.", victim, NULL, buf,
		    TO_CHAR);
		act(AT_BLUE, "Lowering a players clan level.", ch, NULL, NULL,
		    TO_CHAR);
		...
		sprintf(buf + strlen(buf), "%s %s>", cltitle, pClan->name);
		act(AT_BLUE, "You have been raised to $T.", victim, NULL, buf,
		    TO_CHAR);
		act(AT_BLUE, "Raising a players clan level.", ch, NULL, NULL,
		    TO_CHAR);
		act(AT_BLUE, "$n is now $T", victim, NULL, buf, TO_ROOM);
		victim->clev = level;
		return;
...
void do_smash(CHAR_DATA * ch, char *argument)
{
	...
	if (!(victim = get_char_world(ch, name))
	    || victim->in_room->area != ch->in_room->area) {
		send_to_char(AT_WHITE,
			     "That person's life cannot be sensed.\n\r", ch);
		return;
	}
	...
	act(AT_RED, "You call down the Dark forces of Retribution on $N.", ch,
	    NULL, victim, TO_CHAR);
	act(AT_RED, "$n smashes $p.", ch, obj, NULL, TO_ROOM);
	if (!victim->wait)
		act(AT_RED, "You feel a wave of nausia come over you.", victim,
		    NULL, NULL, TO_CHAR);
	extract_obj(obj);
	ch->ctimer = 5;
	if (victim->wait)
...
void do_guild(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->guild == NULL) {
		send_to_char(AT_BLUE,
			     "You must be in a guild to induct someone.\n\r",
			     ch);
		return;
	}
	if (!is_name(NULL, ch->name, ch->guild->deity)) {
		send_to_char(AT_BLUE,
			     "You are not the deity of this guild.\n\r", ch);
		return;
	}
...
void do_unguild(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->guild == NULL) {
		send_to_char(AT_BLUE,
			     "You must be in a guild to unguild someone.\n\r",
			     ch);
		return;
	}
	if (!is_name(NULL, ch->name, ch->guild->deity)) {
...
void do_clans(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(result,
		...
		"Num", "Clan Name", "Deity", "Members", "Pkills", "Pkilled",
		"Mkills");
...
void do_cinfo(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0') {
		...
		send_to_char(AT_WHITE,
			     "Use the command clans to find a clan name or number.\n\r",
			     ch);
		return;
	}
	...
	if (num == -1) {
		send_to_char(AT_WHITE,
			     "No such clan exists, please try again.\n\r", ch);
		send_to_char(AT_WHITE,
			     "Type clans to see available clan names.\n\r", ch);
		return;
	}

	if (!(pClan = get_clan_index(num))) {
		send_to_char(AT_WHITE,
			     "Illegal clan number, please try again.\n\r", ch);
		send_to_char(AT_WHITE,
			     "Type clans to see available clan numbers.\n\r",
			     ch);
		return;
	}
	...
	sprintf(buf, "&cChampion&w:    &z[&W%12s&z]&c%s&R%s\n\r",
		pClan->champ,
		IS_SET(pClan->settings,
		       CLAN_LEADER_INDUCT) ? " Can induct." : "",
...
void do_questflag(CHAR_DATA * ch, char *argument)
{
	...
	if (quest) {
		if (ch->level < qmin) {
			send_to_char(AT_WHITE,
				     "You are too low of level for this quest.\n\r",
				     ch);
			return;
		}
		if (ch->level > qmax) {
			send_to_char(AT_WHITE,
				     "You are too high of level for this quest.\n\r",
				     ch);
			return;
		}
	}
	...
	if (IS_SET(ch->act, PLR_QUEST) && (quest)) {
		send_to_char(AT_WHITE,
			     "You must wait for an immortal to remove you from the quest.\n\r",
			     ch);
		return;
...
void do_worth(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->level < L_CHAMP3) {
		sprintf(log_buf,
			"You have scored a total of %dxp, and are %dxp off from your"
			" next level.\n\r", ch->exp, xp_tolvl(ch) - ch->exp);
		send_to_char(AT_RED, log_buf, ch);
	} else {
		send_to_char(AT_RED,
			     "You perceive that you can level no higher by natural means.\n\r",
			     ch);
	}
	sprintf(log_buf,
...
#ifdef NEW_MONEY
	sprintf(log_buf,
		"You are carrying %d gold, %d silver, and %d copper coins.\n\r",
		ch->money.gold, ch->money.silver, ch->money.copper);
#else
