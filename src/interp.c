...
void interpret(CHAR_DATA * ch, char *argument)
{
	...
	if ((!IS_NPC(ch) && IS_SET(ch->act, PLR_LOG))
		...
		sprintf(log_buf, "%s: %s", ch->name, logline);
		log_string(log_buf,
			   (ch->level == L_IMP ? CHANNEL_NONE : CHANNEL_GOD),
			...
			if (IS_AFFECTED(ch, AFF_SNEAK) &&
				...
				if (is_affected(ch, gsn_chameleon)) {
					send_to_char(AT_BLUE,
						     "You return to your normal coloration.\n\r",
						     ch);
	...
	if (IS_AFFECTED2(ch, AFF_BERSERK) && trust < LEVEL_IMMORTAL) {
		if (!found || (str_cmp(cmd_table[cmd].name, "flee") &&
			...
			send_to_char(AT_BLOOD,
				     "You cannot do that in such a rage!\n\r",
				     ch);
			return;
		}
	}
...
*/
	...
	if (!strcmp(cmd_table[cmd].name, "sing")
		...
		send_to_char(C_DEFAULT,
			     "What do you think you are, a BARD???\n\r", ch);
		return;
...
bool can_use_cmd(int cmd, CHAR_DATA * ch, int trust)
{
	...
	if (!IS_NPC(ch)) {
		if (is_name
		    (NULL, cmd_table[cmd].name, ch->pcdata->empowerments))
			return TRUE;
		if (is_name
		    (NULL, cmd_table[cmd].name, ch->pcdata->detractments))
			return FALSE;
	}
	...
	if (arg[0] == '\0') {
		...
		if (!(victim = get_char_world(ch, arg)))
			...
			if ((ch->in_room->vnum == ROOM_VNUM_HELL) &&
				...
				sprintf(buf, "From a great distance, %s",
					pSocial->char_found);
		...
		act(AT_PINK, pSocial->others_found, ch, NULL, victim,
		    TO_NOTVICT);
		    ...
		    && (!victim->pIndexData->mobprogs))
		{
			...
			case 11:
			case 12:
				act(AT_PINK, "You slap $N.", victim, NULL, ch,
				    TO_CHAR);
				act(AT_PINK, "$n slaps you.", victim, NULL, ch,
				    TO_VICT);
				act(AT_PINK, "$n slaps $N.", victim, NULL, ch,
				    TO_NOTVICT);
				break;
			}
		}
...
void do_alias(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' && (!(ch->pcdata->alias_list))) {
		send_to_char(AT_WHITE,
			     "You have no command alias' to remove!\n\r", ch);
		return;
	}

	if (arg1[0] != '\0') {
		for (oAl = ch->pcdata->alias_list; oAl; oAl = oAl->next) {
			if (is_name(NULL, arg1, oAl->old)
			    || !str_prefix(arg, arg1)) {
				send_to_char(AT_WHITE,
					     "You cannot alias other alias' into your [new] string.\n\r",
					     ch);
				return;
			}
		}
...
void do_countcommands(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "Command table size: %d\n\r"
		...
		(int)(sizeof(cmd_table) / sizeof(cmd_table[0]) - 1), top_social, (int)(sizeof(gskill_table) / sizeof(gskill_table[0])), scnt);	/* Someone wanna explain why theres an error with
																		   sizeof(skill_table)?? */
	send_to_char(AT_PINK, buf, ch);
	for (iclass = 0; iclass < MAX_CLASS; iclass++) {
		sprintf(buf, "%-15s: Skills %2d + Spells %2d = Total %2d\n\r",
			class_table[iclass].who_long,
			skill[iclass], spell[iclass],
			skill[iclass] + spell[iclass]);
		send_to_char(AT_PINK, buf, ch);
	}
	return;
...
void do_pkill(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		send_to_char(AT_WHITE,
			     ...
			     ch);
		send_to_char(AT_WHITE,
			     "This is NOT a reversible choice. Type HELP PK.\n\r",
			     ch);
		return;
	}
	if (!str_cmp(argument, "yes")) {
		if (ch->level < 20) {
			send_to_char(AT_WHITE,
				     "You must be at least level 20 to choose the path of PKILL.\n\r",
				     ch);
			return;
		}
		if (IS_SET(ch->act, PLR_PKILLER)) {
			send_to_char(AT_WHITE,
				     "You have already chosen the path of pkilling.\n\r",
				     ch);
			return;
		}
		send_to_char(AT_RED,
