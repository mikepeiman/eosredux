...
void one_dual args((CHAR_DATA * ch, CHAR_DATA * victim, int dt));
...
void multi_hit(CHAR_DATA * ch, CHAR_DATA * victim, int dt)
{
	...
	if ((IS_AFFECTED2(ch, AFF_CONFUSED))
	    && number_percent() < 10) {
		act(AT_YELLOW, "$n looks around confused at what's going on.",
	...
	chance2 = IS_NPC(ch) ? ch->level / 8 : 0;
	if (number_percent() < chance)
		one_hit(ch, victim, dt);
	if (number_percent() < chance2)
		one_dual(ch, victim, dt);
...
*/
	...
	if (IS_AFFECTED(victim, AFF_FIRESHIELD)) {
		if (number_percent() < 50) {
			if (!IS_NPC(victim))
				spell_fireball(skill_lookup("fireball"), 15,
					       victim, ch);
			else
				spell_fireball(skill_lookup("fireball"), 25,
					       victim, ch);
		}
	}
	...
	if (IS_AFFECTED(victim, AFF_ICESHIELD)) {
		if ((number_percent() < 50)
		    || (number_percent() < 17)) {
			if (!IS_NPC(victim))
				spell_icestorm(skill_lookup("icestorm"), 20,
					       victim, ch);
			else
				spell_fireball(skill_lookup("icestorm"), 40,
					       victim, ch);
		}
	}
	if (!victim || victim->position == POS_DEAD
	...
	if (IS_AFFECTED(victim, AFF_SHOCKSHIELD)) {
		if ((number_percent() < 50)
		    || (number_percent() < 17)) {
			if (!IS_NPC(victim))
				spell_lightning_bolt(skill_lookup
						     ("lightning bolt"), 15,
						     victim, ch);
			else
				spell_lightning_bolt(skill_lookup
	...
	if (IS_AFFECTED2(victim, AFF_BLADE)) {
		if (number_percent() < 40)
			spell_holy_fires(skill_lookup("holy fires"), 45, victim,
					 ch);
	}
	if (!victim || victim->position == POS_DEAD
	    || ch->in_room != victim->in_room)
...
void damage(CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
		...
		if (dt != 91 && ch->level <= LEVEL_HERO && dt != 40) {
			if (IS_NPC(ch) && ch->desc && ch->desc->original)
				sprintf(buf,
					...
					dam, ch->name, ch->desc->original->name,
					dt);
			else
				sprintf(buf,
	...
	if (victim != ch) {
		...
		if (IS_AFFECTED2(ch, AFF_PHASED)) {
			...
			act(AT_GREY, "$n returns from an alternate plane.", ch,
			    NULL, NULL, TO_ROOM);
		}
		/*
		 * Damage modifiers.
...
*/
		...
		if (dt >= TYPE_HIT || dt == gsn_feed
			...
			if (IS_NPC(ch) && number_percent()
			    < (leveldiff <
			       -5 ? ch->level / 2 : UMAX(10, leveldiff))
			    && dam == 0 && number_bits(4) == 0)
				disarm(ch, victim);
			if (IS_NPC(ch) && number_percent()
			    < (leveldiff <
			       -5 ? ch->level / 2 : UMAX(20, leveldiff))
			    && dam == 0 && number_bits(4) == 0)
				trip(ch, victim);
		...
		if (!IS_NPC(ch)
		    ...
		    && number_percent() <=
		    ch->pcdata->learned[gsn_anatomyknow] / 9) {
			update_skpell(ch, gsn_anatomyknow);
			send_to_char(AT_RED, "You hit a pressure point!\n\r",
				     ch);
			act(AT_RED, "$n hit one of $N's pressure points!", ch,
			    NULL, victim, TO_NOTVICT);
			act(AT_RED, "$n hit you with a precise shot.", ch, NULL,
			    victim, TO_VICT);
			if (number_percent() < 2) {
				victim->hit = 1;
	...
	if (dam > 0 && dt > TYPE_HIT && ((is_wielding_poisoned(ch)
	...
	if (dam > 0 && dt > TYPE_HIT && is_wielding_flaming(ch)
	    && number_percent() < 10)
	...
	if (dam > 0 && (dt == 1014
			|| dt == 1015
			|| (dt == 1005 && is_class(ch, CLASS_WEREWOLF)
			    && is_class(ch, CLASS_MONK)
			    && is_bare_hand(ch)))
	...
	if (dam > 0 && dt > TYPE_HIT && is_wielding_icy(ch)
	    && number_percent() < 20)
	...
	if (dam > 0 && dt > TYPE_HIT && is_wielding_chaos(ch)
	    && number_percent() < 20)
		spell_energy_drain(skill_lookup("energy drain"), 45, ch,
				   victim);
	if (victim->position == POS_DEAD || ch->in_room != victim->in_room)
		return;
	if (dam > 0 && (dt == 1014
	...
	if (victim->position == POS_DEAD) {
		if (!IS_ARENA(ch)) {
			...
			if (((ch->guild !=
			      NULL) ? ch->guild->type & GUILD_CHAOS : 0)
			...
			if ((!IS_NPC(ch)) && (!IS_NPC(victim))) {
				...
				if (ch->clan != victim->clan) {
					if ((pClan =
					     get_clan_index(ch->clan)) != NULL)
						pClan->pkills++;
					if ((Cland =
					     get_clan_index(victim->clan)) !=
					    NULL)
						Cland->pdeaths++;
				}
/*            REMOVE_BIT(victim->act, PLR_THIEF);*/
			...
			if ((IS_NPC(ch)) && (!IS_NPC(victim))) {
				CLAN_DATA *pClan;
				if ((pClan =
				     get_clan_index(victim->clan)) != NULL)
					pClan->mdeaths++;
			}
			...
			if (!IS_NPC(victim)) {
				...
				if (!IS_NPC(ch)
					...
					REMOVE_BIT(victim->act, PLR_THIEF);
					info("%s the puny thief gets destroyed by the &rMERCENARY&C %s!", (int)victim->name, (int)ch->name);
				} else if (!IS_NPC(ch)
					   && IS_SET(ch->act, PLR_THIEF)
					   && victim->guild
					   && !strcmp(victim->guild->name,
						      "MERCENARY"))
					info("%s, the sly thief, has killed the &rMERCENARY&C %s.", (int)ch->name, (int)victim->name);
				else
					info("%s gets slaughtered by %s!",
					     (int)victim->name,
					     (int)(IS_NPC(ch) ? ch->
						   short_descr : ch->name));
				save_clans();
			...
			sprintf(log_buf,
				"&C%s &chas defeated &C%s &cin the arena!",
				ch->name, victim->name);
			wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0);
			log_string(log_buf, CHANNEL_LOG, -1);
...
*/
			if (((obj_lose->cost.gold * 100) +
				...
				obj_lose->cost.gold =
				    (obj_lose->cost.gold >
				     0) ? (obj_lose->cost.gold - dam / 6) : 0;
				obj_lose->cost.silver =
				    (obj_lose->cost.silver >
				     0) ? (obj_lose->cost.silver - dam / 6) : 0;
				obj_lose->cost.copper =
				    (obj_lose->cost.copper >
				     0) ? (obj_lose->cost.copper - dam / 6) : 0;
			}
/*		 if ( (obj_lose->cost.gold +
 		      (obj_lose->cost.silver/SILVER_PER_GOLD) +
		      (obj_lose->cost.copper/COPPER_PER_GOLD) ) < 0 )
...
/* Err.. we might not want pkill guilds attacking unguilded.. */
	...
	if ((victim->clan == 0) && (!IS_SET(pClan->settings, CLAN_PKILL))) {
		send_to_char(AT_WHITE,
			     "You can only murder clanned players.\n\r", ch);
		return TRUE;
	}
...
/* can murder self for testing =) */
	if (ch->clan == victim->clan && ch != victim && ch->clan != 0) {
		send_to_char(AT_WHITE,
			     "You cannot murder your own clan member.\n\r", ch);
		return TRUE;
	}

	if (!IS_SET(pClan->settings, CLAN_PKILL)) {
		send_to_char(AT_WHITE,
			     "Peaceful clan members cannot murder.\n\r", ch);
		return TRUE;
	}
	...
	if (!IS_SET(pClan->settings, CLAN_PKILL)) {
		send_to_char(AT_WHITE,
			     "You may not murder peaceful clan members.\n\r",
			     ch);
		return TRUE;
	}
...
void check_killer(CHAR_DATA * ch, CHAR_DATA * victim)
{
	 ...
	 */
	if (IS_NPC(ch)
	    || ch == victim || IS_SET(ch->act, PLR_KILLER)
	    || IS_SET(ch->act, PLR_THIEF))
		return;
	pClan = get_clan_index(ch->clan);
	if ( /*ch->clan != 0 || */ (IS_SET(pClan->settings, CLAN_PKILL)) ||
...
bool check_parry(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (IS_SET(ch->act, PLR_COMBAT))
		act(AT_GREEN, "$N parries your attack.", ch, NULL, victim,
		    TO_CHAR);
	if (IS_SET(victim->act, PLR_COMBAT))
		act(AT_GREEN, "You parry $n's attack.", ch, NULL, victim,
		    TO_VICT);
	return TRUE;
}
...
bool check_dodge(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (IS_SET(ch->act, PLR_COMBAT))
		act(AT_GREEN, "$N dodges your attack.", ch, NULL, victim,
		    TO_CHAR);
	if (IS_SET(victim->act, PLR_COMBAT))
		act(AT_GREEN, "You dodge $n's attack.", ch, NULL, victim,
		    TO_VICT);
	return TRUE;
}
...
bool check_sidestep(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (IS_SET(ch->act, PLR_COMBAT))
		act(AT_GREEN, "$N sidesteps your attack.", ch, NULL, victim,
		    TO_CHAR);
	if (IS_SET(victim->act, PLR_COMBAT))
		act(AT_GREEN, "You sidestep $n's attack.", ch, NULL, victim,
		    TO_VICT);
	return TRUE;
}
		...
		if (fch == ch || (fBoth && fch->fighting == ch)) {
			...
			if (is_affected(fch, gsn_berserk)) {
				...
				send_to_char(C_DEFAULT,
					     skill_table[gsn_berserk].msg_off,
					     fch);
				send_to_char(C_DEFAULT, "\n\r", fch);

				act(C_DEFAULT,
				    skill_table[gsn_berserk].room_msg_off, fch,
				    NULL, NULL, TO_ROOM);
			} else if (is_affected(fch, gsn_frenzy)) {
				affect_strip(fch, gsn_frenzy);
				send_to_char(C_DEFAULT,
					     skill_table[gsn_frenzy].msg_off,
					     fch);
				send_to_char(C_DEFAULT, "\n\r", fch);
				act(C_DEFAULT,
				    skill_table[gsn_frenzy].room_msg_off, fch,
				    NULL, NULL, TO_ROOM);
			}
...
/*    ROOM_INDEX_DATA *location;*/
	...
	if (IS_ARENA(ch)) {
		...
		sprintf(log_buf,
			"&C%s &chas been awarded &W%d &ccoins for %s victory.",
			gch->name, award,
			(gch->sex ==
			 SEX_NEUTRAL ? "its" : (gch->sex ==
						SEX_MALE ? "his" : "her")));
		/* wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0); */
		log_string(log_buf, CHANNEL_LOG, -1);
		challenge(log_buf, 0, 0);
...
#endif
		sprintf(log_buf,
			"You have been awarded %d gold coins for your victory."
			"\n\r", award);
...
void death_cry(CHAR_DATA * ch)
{
	...
	for (door = 0; door <= 5; door++) {
		...
		if ((pexit = was_in_room->exit[door])
		    && pexit->to_room && pexit->to_room != was_in_room) {
	...
	for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
		...
		if (gch->level - lch->level >= 6) {
			send_to_char(AT_BLUE,
				     "You are too high level for this group.\n\r",
				     gch);
			continue;
		}
		...
		if (gch->level - lch->level <= -6) {
			send_to_char(AT_BLUE,
				     "You are too low level for this group.\n\r",
				     gch);
			continue;
		}
		...
		for (obj = ch->carrying; obj; obj = obj_next) {
			...
			if (!IS_NPC(ch))
				if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL)
					...
					&& IS_NEUTRAL(ch))) {
					act(AT_BLUE, "You are zapped by $p.",
					    ch, obj, NULL, TO_CHAR);
					act(AT_BLUE, "$n is zapped by $p.", ch,
					    obj, NULL, TO_ROOM);
					obj_from_char(obj);
					obj_to_room(obj, ch->in_room);
				}
...
int xp_compute(CHAR_DATA * gch, CHAR_DATA * victim)
{
	...
	if (align > 500)
		...
		gch->alignment =
		    UMAX(gch->alignment + (align + 500) / 4, -1000);
	else
...
void dam_message(CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
	...
	if (dt == TYPE_HIT) {
		...
		if (dt > TYPE_HIT && is_wielding_poisoned(ch)) {
			sprintf(buf1, "Your &gpoisoned&X %s &r%s&X $N%c",
				attack, vp, punct);
			sprintf(buf2, "$n's &gpoisoned&X %s &G%s&X you%c",
				attack, vp, punct);
			sprintf(buf3, "$n's &gpoisoned&X %s &z%s&X $N%c",
				attack, vp, punct);
			sprintf(buf4, "Your &gpoisoned&X %s &G%s&X you%c",
				attack, vp, punct);
			sprintf(buf5, "$n's &gpoisoned&X %s &z%s&X $n%c",
				attack, vp, punct);
		} else {
			sprintf(buf1, "Your %s &r%s&X $N%c", attack, vp, punct);
			sprintf(buf2, "$n's %s &G%s&X you%c", attack, vp,
				punct);
			sprintf(buf3, "$n's %s &z%s&X $N%c", attack, vp, punct);
			sprintf(buf4, "Your %s &G%s&X you%c", attack, vp,
				punct);
...
void trip(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (!IS_STUNNED(victim, STUN_COMMAND) && !IS_STUNNED(ch, STUN_TO_STUN)) {
		act(AT_CYAN, "You trip $N and $N goes down!", ch, NULL, victim,
		    TO_CHAR);
		act(AT_CYAN, "$n trips you and you go down!", ch, NULL, victim,
		    TO_VICT);
		act(AT_GREY, "$n trips $N and $N goes down!", ch, NULL, victim,
		    TO_NOTVICT);
...
void do_kill(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(victim) && !IS_ARENA(victim)) {
		...
		if (IS_AFFECTED(victim, AFF_CHARM) && victim->master) {
			send_to_char(AT_WHITE,
				     "You must MURDER a charmed creature.\n\r",
				     ch);
			return;
		}
	}
	...
	if (IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim) {
		act(AT_BLUE, "$N is your beloved master!", ch, NULL, victim,
		    TO_CHAR);
		return;
	}
...
void do_murder(CHAR_DATA * ch, char *argument)
{
	...
	if (is_safe(ch, victim))
		return;
	if (IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim) {
		act(C_DEFAULT, "$N is your beloved master!", ch, NULL, victim,
		    TO_CHAR);
		return;
	}

	if (!IS_SET(ch->act, PLR_PKILLER) && !IS_ARENA(ch) && !IS_NPC(victim)) {	/* chars can murder mobs */
		send_to_char(C_DEFAULT,
	...
	if (!IS_SET(victim->act, PLR_PKILLER) && !IS_ARENA(victim)
		...
		send_to_char(C_DEFAULT,
			     "You can only pkill other Pkillers.\n\r", ch);
		return;
	}
...
void do_backstab(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->fighting) {
		send_to_char(C_DEFAULT,
			     "You can't backstab a fighting person.\n\r", ch);
		return;
	}
...
void do_assasinate(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->fighting) {
		send_to_char(C_DEFAULT,
			     "You can't assasinate a fighting person.\n\r", ch);
		return;
	}
...
void do_flee(CHAR_DATA * ch, char *argument)
{
	...
	for (attempt = 0; attempt < 6; attempt++) {
		...
		if (IS_ARENA(ch))
			act(AT_RED, "$n has escaped!  After $m!", ch, NULL,
			    victim, TO_VICT);
		ch->in_room = now_in;
		...
		if (!IS_NPC(ch)) {
			if (!IS_ARENA(ch)) {
				send_to_char(C_DEFAULT,
					     "You flee from combat!  You lose 25 exps.\n\r",
					     ch);
				gain_exp(ch, -25);
			} else
				send_to_char(C_DEFAULT,
...
void do_gouge(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_gouge]) {
		...
		if (number_percent() < 10) {
			...
			affect_join(victim, &af);
			act(AT_GREY, "$N is blinded!", ch, NULL, victim,
			    TO_CHAR);
		}
...
void do_circle(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_circle]) {
		...
		damage(ch, victim, number_range(400, ch->level * 9),
		       gsn_circle);
		update_pos(victim);
	} else
...
void do_high_kick(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_high_kick]) {
		damage(ch, victim, number_range(150, ch->level * 4),
		       gsn_high_kick);
	...
	if (!IS_NPC(ch))
		chance =
		    ch->pcdata->mod_str + (ch->level - victim->level) +
		    ch->pcdata->perm_str;

	if ((number_percent() < chance) || IS_NPC(ch)) {
		act(AT_RED, "You hear a crunch as you connect with $N's head.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_RED, "$n's high kick connects firmly with your head!",
		    ch, NULL, victim, TO_VICT);
		act(C_DEFAULT, "$n's high kick hit $N's head!", ch, NULL,
		    victim, TO_ROOM);
...
void do_jump_kick(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_jump_kick]) {
		damage(ch, victim, number_range(200, ch->level * 4),
		       gsn_jump_kick);
	...
	if ((number_percent() < chance) || IS_NPC(ch)) {
		act(AT_RED, "You hear a crunch as you connect with $N's head.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_RED, "$n's jump kick connects firmly with your head!",
		    ch, NULL, victim, TO_VICT);
		act(C_DEFAULT, "$n's jump kick hit $N's head!", ch, NULL,
		    victim, TO_ROOM);
...
void do_spin_kick(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_spin_kick]) {
		damage(ch, victim, number_range(250, ch->level * 5),
		       gsn_spin_kick);
	...
	if ((number_percent() < chance) || IS_NPC(ch)) {
		act(AT_RED, "You hear a crunch as you connect with $N's head.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_RED, "$n's spin kick connects firmly with your head!",
		    ch, NULL, victim, TO_VICT);
		act(C_DEFAULT, "$n's spin kick hit $N's head!", ch, NULL,
		    victim, TO_ROOM);
...
void do_punch(CHAR_DATA * ch, char *argument)
{
	...
	if ((number_percent() < chance) || IS_NPC(ch)) {
		act(AT_RED, "You hear a crunch as you connect with $N's head.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_RED, "$n's punch connects firmly with your head!", ch,
		    NULL, victim, TO_VICT);
		act(C_DEFAULT, "$n's punch hit's home!", ch, NULL, victim,
		    TO_ROOM);
...
void do_jab_punch(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_jab_punch]) {
		damage(ch, victim, number_range(125, ch->level * 3),
		       gsn_jab_punch);
	...
	if ((number_percent() < chance) || IS_NPC(ch)) {
		act(AT_RED, "You hear a crunch as you connect with $N's head.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_RED, "$n's jab connects firmly with your head!", ch,
		    NULL, victim, TO_VICT);
		act(C_DEFAULT, "$n's jab hit $N's head!", ch, NULL, victim,
		    TO_ROOM);
...
void do_kidney_punch(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch)
	    || number_percent() < ch->pcdata->learned[gsn_kidney_punch]) {
		damage(ch, victim, number_range(150, ch->level * 3),
		       gsn_kidney_punch);
...
void do_cross_punch(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch)
	    || number_percent() < ch->pcdata->learned[gsn_cross_punch]) {
		damage(ch, victim, number_range(175, ch->level * 3),
		       gsn_cross_punch);
	...
	if ((number_percent() < chance) || IS_NPC(ch)) {
		act(AT_RED, "You hear a crunch as you connect with $N's head.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_RED, "$n's cross connects firmly with your head!", ch,
		    NULL, victim, TO_VICT);
...
void do_roundhouse_punch(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch)
	    || number_percent() < ch->pcdata->learned[gsn_roundhouse_punch]) {
		damage(ch, victim, number_range(200, ch->level * 3),
	...
	if ((number_percent() < chance) || IS_NPC(ch)) {
		act(AT_RED, "You hear a crunch as you connect with $N's head.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_RED, "$n's roundhouse connects firmly with your head!",
		    ch, NULL, victim, TO_VICT);
		act(C_DEFAULT, "$n's roundhouse hit $N's head!", ch, NULL,
		    victim, TO_ROOM);
...
void do_uppercut_punch(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(ch)
	    || number_percent() < ch->pcdata->learned[gsn_uppercut_punch]) {
		damage(ch, victim, number_range(225, ch->level * 3),
		       gsn_uppercut_punch);
...
void do_flury(CHAR_DATA * ch, char *argument)
{
	...
	if ((get_eq_char(ch, WEAR_WIELD)) || (get_eq_char(ch, WEAR_WIELD_2)) ||
	    (get_eq_char(ch, WEAR_HOLD))) {
		send_to_char(C_DEFAULT,
			     "You cannot begin such a flury while your hands are full!\n\r",
			     ch);
		return;
	}
	...
	act(AT_RED, "You throw a flury of punches and kicks at $N!", ch, NULL,
	    victim, TO_CHAR);
	act(AT_RED, "$n throws a flury of punches and kicks at you!", ch, NULL,
	    victim, TO_VICT);
	act(C_DEFAULT, "$n throws a flury of punches and kicks at $N!", ch,
	    NULL, victim, TO_ROOM);
	...
	/* nothing hit */
	if (count == 0) {
		act(AT_RED, "But none of your attacks hit $N!", ch, NULL,
		    victim, TO_CHAR);
		act(AT_RED, "But none of $n attacks hit you!", ch, NULL, victim,
		    TO_VICT);
		act(C_DEFAULT, "But none of $n attacks hit $N!", ch, NULL,
		    victim, TO_ROOM);
	}
	return;
}
...
void do_disarm(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->fighting != ch && ch->fighting != victim) {
		act(C_DEFAULT, "$E is not fighting you!", ch, NULL, victim,
		    TO_CHAR);
		return;
	}
	...
	if ((victim = ch->fighting) == NULL) {
		...
		if (arg3[0] == '\0') {
			...
			if ((pexit = to_room->exit[dir]) == NULL ||
				...
				send_to_char(C_DEFAULT,
					     "You cannot throw in that direction.\n\r",
					     ch);
				return;
			}
			...
			if (IS_SET(pexit->exit_info, EX_CLOSED)) {
				send_to_char(C_DEFAULT,
					     "You cannot throw through a door.\n\r",
					     ch);
				return;
			}
			...
			for (dist = 1; dist <= MAX_DIST; dist++) {
				...
				if ((pexit = to_room->exit[dir]) == NULL ||
					...
					sprintf(buf,
						"A $p flys in from $T and hits the %s wall.",
						dir_name[dir]);
					act(AT_WHITE, buf, ch, Obj,
					    dir_noun[rev_dir[dir]], TO_ROOM);
					sprintf(buf,
						"You throw your $p %d room%s $T, where it hits a wall.",
						dist, dist > 1 ? "s" : "");
					act(AT_WHITE, buf, ch, Obj,
			...
			if (victim == NULL) {
				...
				sprintf(buf,
					...
					dist, dist > 1 ? "s" : "");
				act(AT_WHITE, buf, ch, Obj, dir_name[dir],
				    TO_CHAR);
				char_from_room(ch);
				char_to_room(ch, in_room);
				oprog_throw_trigger(Obj, ch);
		...
		if (dist > 0) {
			char_from_room(ch);
			char_to_room(ch, in_room);
			act(AT_WHITE, "A $p flys in from $T and hits $n!",
			    victim, Obj, dir_noun[rev_dir[dir]], TO_NOTVICT);
			act(AT_WHITE, "A $p flys in from $T and hits you!",
			    victim, Obj, dir_noun[rev_dir[dir]], TO_CHAR);
			sprintf(buf, "Your $p flew %d rooms %s and hit $N!",
				dist, dir_name[dir]);
...
void do_drain_life(CHAR_DATA * ch, char *argument)
{
	...
	/* 3xlevel + 1dlevel */
	if (IS_NPC(ch)
	    || number_percent() < ch->pcdata->learned[gsn_drain_life]) {
...
void do_soulstrike(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch)
	    && ch->pcdata->learned[gsn_soulstrike] < number_percent()) {
...
/* Don't need a check for update_pos, because of the previous check. */
	...
	act(AT_BLUE, "Your soul strikes deep into $N.", ch, NULL, victim,
	    TO_CHAR);
	act(AT_BLUE, "$n's soul strikes deep into you.", ch, NULL, victim,
	    TO_VICT);
	act(AT_BLUE, "$n's soul strikes deep into $N.", ch, NULL, victim,
	    TO_NOTVICT);
...
void do_multiburst(CHAR_DATA * ch, char *argument)
{
	...
	if (!can_use_skpell(ch, gsn_multiburst)) {
		send_to_char(C_DEFAULT,
			     "You're not enough of a mage to do multibursts.\n\r",
			     ch);
		return;
	}
	...
	if (ch->mana < mana && ch->level < LEVEL_IMMORTAL) {
		WAIT_STATE(ch, skill_table[gsn_multiburst].beats);
		send_to_char(C_DEFAULT,
			     "You don't have enough mana to multiburst these spells.\n\r",
			     ch);
		return;
	}
	WAIT_STATE(ch, skill_table[gsn_multiburst].beats);
...
void do_challenge(CHAR_DATA * ch, char *argument)
{
	...
	if (rvnum == ROOM_VNUM_HELL || rvnum == ROOM_VNUM_RJAIL) {
		send_to_char(C_DEFAULT,
			     "Nice try, but get out the real way.\n\r", ch);
		return;
	}
	if (arena.fch && arena.sch) {
		send_to_char(C_DEFAULT,
			     "There are already two people fighting in the"
			     " arena.\n\r", ch);
		return;
	}
	if (arena.cch) {
		sprintf(arg1,
			"%s is offering a challenge.  Type accept to accept it.\n\r",
			arena.cch->name);
	...
	if ((is_number(arg1) && (award = atoi(arg1)) < 1000000)
		...
		send_to_char(C_DEFAULT,
			     "Syntax: challenge [player] <award>\n\r", ch);
		send_to_char(C_DEFAULT,
			     " Player is the optional name of a specific person to challenge.\n\r",
			     ch);
		send_to_char(C_DEFAULT,
			     " Award is at least 1000000 gold coins.\n\r", ch);
		return;
...
/*  if ( award > ( ch->money.gold + (ch->money.silver/SILVER_PER_GOLD) +
	...
	if (!get_room_index(ROOM_ARENA_ENTER_F) ||
		...
		send_to_char(C_DEFAULT,
			     "An error has occured.  Please inform an Immortal.\n\r",
			     ch);
		return;
	}
	arena.cch = ch;
	...
	if (arena.och)
		sprintf(log_buf,
			"&C%s &cchallenges &C%s &cto a fight in the arena for &W%d &cgold coins.",
...
void do_accept(CHAR_DATA * ch, char *argument)
{
	...
	if (!(cch = arena.cch)) {
		send_to_char(C_DEFAULT,
			     "There is no challenge being offered.\n\r", ch);
		return;
	}
	if (rvnum == ROOM_VNUM_HELL || rvnum == ROOM_VNUM_RJAIL) {
		send_to_char(C_DEFAULT,
...
void do_rush(CHAR_DATA * ch, char *argument)
{
	...
	if (!can_use_skpell(ch, gsn_rush)) {
		send_to_char(C_DEFAULT,
			     "You can't pump yourself up enough.\n\r", ch);
		return;
	}
	...
	send_to_char(AT_RED,
		     "You pump yourself up and make the adrenaline flow.\n\r",
		     ch);
...
void do_howl_of_fear(CHAR_DATA * ch, char *argument)
{
	...
	if (is_affected(victim, gsn_howlfear)) {
		act(AT_RED, "$N is already scared witless.", ch, NULL, victim,
		    TO_CHAR);
		return;
	}
...
void do_frenzy(CHAR_DATA * ch, char *argument)
{
	...
	act(AT_WHITE, "$n suddenly goes into a frenzy", ch, NULL, NULL,
	    TO_ROOM);
...
void do_bladepalm(CHAR_DATA * ch, char *argument)
{
	...
	if (get_eq_char(ch, WEAR_WIELD) && get_eq_char(ch, WEAR_WIELD_2)) {
		send_to_char(AT_GREY,
			     "You need at least one free hand to perform a blade palm.\n\r",
			     ch);
		return;
	}
	if (argument[0] != '\0') {
	...
	if (ch->pcdata->learned[gsn_bladepalm] > number_percent()) {
		...
		if (anatomy > 0
		    && number_percent() < number_range(anatomy / 9,
						       anatomy / 4.5)) {
			update_skpell(ch, gsn_anatomyknow);
			send_to_char(AT_RED, "You hit a pressure point!\n\r",
				     ch);
		...
		if (ch->pcdata->learned[gsn_enhanced_damage] > 0)
			dam +=
			    dam / 4 * ch->pcdata->learned[gsn_enhanced_damage] /
			    150;
...
void do_flyingkick(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch)
		...
		send_to_char(AT_GREY,
			     "You do not hav knowledge of martial arts.\n\r",
			     ch);
		return;
	}
	if (!ch->fighting) {
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_flykick]) {
		damage(ch, victim, number_range(200, ch->level * 6),
		       gsn_flykick);
...
void do_nerve(CHAR_DATA * ch, char *argument)
{
	...
	if (number_percent() < ch->pcdata->learned[gsn_nerve]
	    && number_percent() < (ch->level * 75) / victim->level) {
		AFFECT_DATA af;
		act(AT_YELLOW, "You pinch $N's arm nerves!", ch, NULL, victim,
		    TO_CHAR);
		act(AT_YELLOW, "$n pinched your arm nerves.", ch, NULL, victim,
		    TO_VICT);
		act(AT_YELLOW, "$n pinched $N's arm nerves.", ch, NULL, victim,
		    TO_NOTVICT);
		...
		if (wield || dual) {
			if (IS_SIMM(victim, IMM_NERVE)) {
				...
				sprintf(buf,
					"$N drops $S weapon%s to the ground.",
					(wield && dual) ? "s" : "");
				act(AT_GREY, buf, ch, NULL, victim, TO_CHAR);
				act(AT_GREY, buf, ch, NULL, victim, TO_NOTVICT);
				sprintf(buf,
					"You drop your weapon%s to the ground.\n\r",
					(wield && dual) ? "s" : "");
		...
		if ((hold = get_eq_char(victim, WEAR_HOLD))) {
			if (!IS_SIMM(victim, IMM_NERVE)) {
				act(AT_GREY, "$N drops $S $p.", ch, hold,
				    victim, TO_CHAR);
				act(AT_GREY, "$N drops $S $p.", ch, hold,
				    victim, TO_NOTVICT);
				act(AT_GREY, "You drop your $p.", ch, hold,
				    victim, TO_VICT);
				obj_from_char(hold);
				obj_to_room(hold, victim->in_room);
			} else
				act(AT_GREY, "$N grips $S $p tightly.", ch,
				    hold, victim, TO_NOTVICT);
		}
	...
	if ((obj = victim->carrying) && !saves_spell(ch->level, victim)) {
		for (obj = victim->carrying; obj; obj = obj_next) {
			...
			if (obj->item_type == ITEM_CONTAINER) {
				if (!obj->contains)
					continue;
				for (potion = obj->contains; potion;
				     potion = potion_next) {
					...
					if (number_bits(2) != 0) {
						extract_obj(potion);
						act(AT_BLUE,
						    "You feel something explode from within $p.",
			...
			if (number_bits(2) != 0) {
				act(AT_BLUE, "$p vibrates and explodes!",
				    victim, obj, NULL, TO_CHAR);
				extract_obj(obj);
				blown++;
			}
