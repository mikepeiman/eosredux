...
TRAP_DATA *get_trap_data(void *vo, int vnum, int type)
{
	...
	switch (type) {
	case ED_OPROG:
		for (pTrap = obj->traps; pTrap;
		     pTrap = pTrap->next_here, value++)
			if (value == vnum)
				return pTrap;
		return NULL;
	case ED_RPROG:
		for (pTrap = room->traps; pTrap;
		     pTrap = pTrap->next_here, value++)
			if (value == vnum)
				return pTrap;
		return NULL;
...
void tedit(CHAR_DATA * ch, char *argument)
{
	...
	case ED_RPROG:
		{
			...
			for (trap = ch->in_room->traps; trap;
			     trap = trap->next_here)
	...
	case ED_EPROG:
		if (!pExit) {
			send_to_char(C_DEFAULT,
				     "Exit not found.  Returning to REditor.\n\r",
				     ch);
...
void do_cedit(CHAR_DATA * ch, char *argument)
{
	...
	if (is_number(command)) {
		if (!(pClan = get_clan_index(atoi(command)))) {
			send_to_char(C_DEFAULT,
				     "CEdit:  That clan does not exist.\n\r",
				     ch);
			return;
		}
...
bool medit_mpedit(CHAR_DATA * ch, char *argument)
{
	...
	if (is_number(command)) {
		if (!
		    (pMProg =
		     get_mprog_data((MOB_INDEX_DATA *) ch->desc->pEdit,
				    atoi(command)))) {
			send_to_char(C_DEFAULT,
				     "MPEdit:  Mobile has no such MobProg.\n\r",
				     ch);
			return FALSE;
		}
	...
	send_to_char(C_DEFAULT,
		     "MPEdit:  There is no default MobProg to edit.\n\r", ch);
	return FALSE;
}
...
bool oedit_opedit(CHAR_DATA * ch, char *argument)
{
	...
	if (is_number(command)) {
		if (!(pTrap = get_trap_data(ch->desc->pEdit, atoi(command),
					    ED_OPROG))) {
			send_to_char(C_DEFAULT,
				     "OPEdit:  Object has no such ObjProg.\n\r",
				     ch);
			return FALSE;
		}
		ch->desc->editin = ch->desc->editor;
	...
	send_to_char(C_DEFAULT,
		     "OPEdit:  There is no default ObjProg to edit.\n\r", ch);
	return FALSE;
}
...
bool redit_rpedit(CHAR_DATA * ch, char *argument)
{
	...
	if (is_number(command)) {
		if (!
		    (pTrap =
		     get_trap_data(ch->in_room, atoi(command), ED_RPROG))) {
			send_to_char(C_DEFAULT,
				     "RPEdit:  Room has no such RoomProg.\n\r",
				     ch);
			return FALSE;
		}
		ch->desc->editin = ch->desc->editor;
	...
	send_to_char(C_DEFAULT,
		     "RPEdit:  There is no default RoomProg to edit.\n\r", ch);
	return FALSE;
}
...
bool redit_epedit(CHAR_DATA * ch, char *argument)
{
	...
	argument = one_argument(argument, command);
	for (dir = 0; dir < 6; dir++)
		if (!str_prefix(command, dir_name[dir])
		    && ch->in_room->exit[dir])
			break;
	if (dir == 6) {
	...
	if (is_number(command)) {
		if (!
		    (pTrap =
		     get_trap_data(ch->in_room->exit[dir], atoi(command),
				   ED_EPROG))) {
			send_to_char(C_DEFAULT,
				     "EPEdit:  Exit has no such ExitProg.\n\r",
				     ch);
			return FALSE;
		}
		ch->desc->editin = ch->desc->editor;
	...
	send_to_char(C_DEFAULT,
		     "EPEdit:  There is no default ExitProg to edit.\n\r", ch);
	return FALSE;
}
...
void display_resets(CHAR_DATA * ch)
{
	...
	for (pReset = pRoom->reset_first; pReset; pReset = pReset->next) {
		...
		case 'M':
			...
			pMob = pMobIndex;
			sprintf(buf,
				"&BM&R[&B%5d&R]&B %-13.13s in room             R[%5d] [%3d] %-15.15s&w\n\r",
		...
		case 'O':
			...
			sprintf(buf,
				"&WO&R[&W%5d&R]&W %-13.13s in room             "
				"R[%5d]       %-15.15s&w\n\r", pReset->arg1,
				pObj->short_descr, pReset->arg3,
				pRoomIndex->name);
			strcat(final, buf);
		...
		case 'R':
			...
			sprintf(buf,
				"R[%5d] Exits (0) to (%d) are randomized in %s\n\r",
...
/*    sprintf( result, "[%3s] [%-28s] (%-5s-%5s) [%-10s]  %3s  [%-8s]\n\r",*/
	sprintf(result,
		"&z[&W%3s&z] &z[&B%-28s&z] &z(&W%-5s-%5s&z) &z[&G%-10s&z] &z[&W%3s&z][&W%-8s&z]\n\r",
	...
	for (pArea = area_first; pArea; pArea = pArea->next) {
		...
		sprintf(buf,
			...
			pArea->uvnum, pArea->filename, pArea->security,
			pArea->builders);
		strcat(result, buf);
	}
...
void do_sedit(CHAR_DATA * ch, char *argument)
{
	...
	if (command[0] == 'c' && !str_prefix(command, "create")) {
		if (argument[0] == '\0') {
			send_to_char(C_DEFAULT,
				     "SEdit:  Syntax: sedit create <social name>\n\r",
				     ch);
			return;
		}
		...
		if (get_social(argument) != NULL) {
			send_to_char(C_DEFAULT,
				     "SEdit:  Social entry already exists.\n\r",
				     ch);
			return;
		}
...
void do_rename_obj(CHAR_DATA * ch, char *argument)
{
	...
	if (command[0] == '\0') {
		send_to_char(C_DEFAULT,
			     "RENAME_OBJ:  Syntax: rename <obj name>.\n\r", ch);
		return;
	}

	if (!(pObj = get_obj_carry(ch, command))) {
		send_to_char(C_DEFAULT,
			     "RENAME_OBJ:  You don't have that obj.\n\r", ch);
		return;
	} else {
		ch->desc->pEdit = (void *)pObj;
		...
		if (!str_prefix(command, forge_obj_table[cmd].name)) {
			if (is_number(command)
				...
				send_to_char(AT_GREY,
					     "You cannot forge so much at your level.\n\r",
					     ch);
				return;
			}
...
void forge_pay_cost(CHAR_DATA * ch)
{
	...
	if (((ch->money.gold * C_PER_G) + (ch->money.silver * S_PER_G) +
	    ...
	    ((obj->cost.gold * C_PER_G) + (obj->cost.silver * S_PER_G) +
	     (obj->cost.copper))) {
		send_to_char(AT_GREY,
			     "You cannot afford to forge this object.\n\r", ch);
		extract_obj(obj);
		return;
	}
	obj_to_char(obj, ch);
...
void do_spedit(CHAR_DATA * ch, char *argument)
{
	...
	if (command[0] == 'c' && !str_prefix(command, "create")) {
		if (argument[0] == '\0') {
			send_to_char(C_DEFAULT,
				     "SPedit:  Syntax: spedit create <spell name>\n\r",
				     ch);
			return;
		}
		...
		if (skill_lookup(argument) < 0) {
			send_to_char(C_DEFAULT,
				     "SPEdit:  Spell/Skill entry already exist.\n\r",
				     ch);
			return;
		}

		send_to_char(C_DEFAULT,
			     "SPEdit:  Spell/Skill online creation not avable.\n\r",
			     ch);
		return;
	} else {
		if ((sn = skill_lookup(arg)) < 0) {
			send_to_char(C_DEFAULT,
				     "Spell/skill entry not found.\n\r", ch);
			return;
		}
...
void do_race_edit(CHAR_DATA * ch, char *argument)
{
	...
	if (command[0] == 'c' && !str_prefix(command, "create")) {
		if (argument[0] == '\0' || strlen(argument) > 20) {
			send_to_char(C_DEFAULT,
				     "Race_Edit:  Syntax: race_edit create <race name>\n\r",
				     ch);
			send_to_char(C_DEFAULT,
				     "                    (Race name can't be longer than 20 characters.)\n\r",
				     ch);
			return;
		}
		...
		if (get_race(argument) != NULL) {
			send_to_char(C_DEFAULT,
				     "Race_Edit:  Race entry already exists.\n\r",
				     ch);
			return;
		}
