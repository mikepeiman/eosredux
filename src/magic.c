...
void magic_mob(CHAR_DATA * ch, OBJ_DATA * obj, int vnum)
{
	...
	update_pos(zombie);
	act(AT_BLUE,
	    "$n passes $s hands over $p, $E slowly rises to serve $S new master.",
	    ch, obj, zombie, TO_ROOM);
	act(AT_BLUE, "You animate $p, it rises to serve you.", ch, obj, NULL,
	    TO_CHAR);
...
void update_skpell(CHAR_DATA * ch, int sn)
{
	...
	if (!(ch->fighting)) {
		sprintf(buf,
			"You gain %d experience for your success with %s.\n\r",
...
void say_spell(CHAR_DATA * ch, int sn)
{
	...
	for (pName = skill_table[sn].name; *pName != '\0'; pName += length) {
		for (iSyl = 0;
		     (length = strlen(syl_table[iSyl].old)) != 0; iSyl++) {
	...
	for (rch = ch->in_room->people; rch; rch = rch->next_in_room) {
		if (rch != ch)
			act(AT_BLUE, is_class(rch, prime_class(ch))
			    ? buf : buf2, ch, NULL, rch, TO_VICT);
	}
...
void do_acspell(CHAR_DATA * ch, OBJ_DATA * pObj, char *argument)
{
	...
	if ((sn == spec) && (is_name(ch, arg2, ch->name))) {
		send_to_char(AT_BLUE,
			     "You are already in the same room as yourself.\n\r",
			     ch);
		return;
	}
...
*/
	if (IS_STUNNED(ch, STUN_MAGIC)) {
		if (prime_class(ch) != CLASS_BARD)
			send_to_char(AT_LBLUE,
				     "You're too stunned to cast spells.\n\r",
				     ch);
		else
			send_to_char(AT_LBLUE,
				     "You're too stunned to sing songs.\n\r",
				     ch);
		return;
	}
	...
	case TAR_CHAR_SELF:
		if (arg2[0] != '\0' && !is_name(ch, arg2, ch->name)) {
			if (prime_class(ch) != CLASS_BARD)
				...
				send_to_char(AT_BLUE,
					     "You cannot sing this song for others.\n\r",
					     ch);
			return;
		}
	...
	if (!IS_NPC(ch))
	{
		if (!is_class(ch, CLASS_VAMPIRE) && ch->mana < mana) {
			...
			send_to_char(AT_RED,
				     "You are to starved to cast, you must feed.\n\r",
				     ch);
			return;
		}
	}
	...
	if (!IS_NPC(ch))
	{
		if (number_percent() > ch->pcdata->learned[sn]) {
			...
			if ((IS_AFFECTED2(ch, AFF_CONFUSED))
			    && number_percent() < 10) {
				act(AT_YELLOW,
				    "$n looks around confused at what's going on.",
				    ch, NULL, NULL, TO_ROOM);
				send_to_char(AT_YELLOW,
					     "You become confused and botch the spell.\n\r",
					     ch);
				return;
			}
			update_skpell(ch, sn);
		    ...
		    OBJ_DATA * obj)
{
	...
	case TAR_CHAR_OFFENSIVE:
		...
		if (((ch->level - 9 > victim->level)
		    ...
		    && (!IS_NPC(victim))) {
			send_to_char(AT_WHITE,
				     "That is not in the pkill range... valid range is +/- 8 levels.\n\r",
				     ch);
			return;
		}
		vo = (void *)victim;
...
void spell_astral(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "$n vanishes in a flash of blinding light.", ch, NULL,
	    NULL, TO_ROOM);
	if (ch != victim) {
		if (pet) {
			act(AT_BLUE,
			    "$n vanishes in a flash of blinding light.", pet,
			    NULL, NULL, TO_ROOM);
			char_from_room(pet);
		}
		char_from_room(ch);
	...
	if (pet) {
		...
		act(AT_BLUE, "$n appears in a flash of blinding light.", pet,
		    NULL, NULL, TO_ROOM);
	}
	return;
}
...
void spell_pass_plant(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (IS_AFFECTED(ch, AFF_ANTI_FLEE)) {
		send_to_char(AT_WHITE,
			     "You cannot walk through the plants in your condition!\n\r",
			     ch);
		return;
	}
	...
	if (IS_SET(ch->in_room->room_flags, ROOM_NO_SHADOW)) {
		send_to_char(AT_GREEN,
			     "The plants do not respond to your call.\n\r", ch);
		return;
	}
	...
	if (ch != victim) {
		act(AT_GREEN, "$n melts into the surrounding plant life.", ch,
		    NULL, NULL, TO_ROOM);
		char_from_room(ch);
		char_to_room(ch, victim->in_room);
		act(AT_GREEN, "$n steps out of the surrounding plant life.", ch,
		    NULL, NULL, TO_ROOM);

		if (pet) {
			act(AT_BLUE,
			    "$n melts into the surrounding plant life.", pet,
			    NULL, NULL, TO_ROOM);
			char_from_room(pet);
			char_to_room(pet, victim->in_room);
			act(AT_BLUE,
			    "$n steps out of the surrounding plant life.", pet,
			    NULL, NULL, TO_ROOM);
		}
	}
...
void spell_aura(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_BLUE,
		     "You feel a wave of peace flow lightly over your body.\n\r",
		     victim);
...
void spell_darkbless(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (ch != victim)
		send_to_char(AT_BLUE,
			     "You call forth the hand of oblivion.\n\r", ch);
...
void spell_draw_strength(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_BLUE,
		     "You draw from your physical strength and increase your energy reserve.\n\r",
		     ch);
...
void spell_blood_bath(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (blood_count(ch->in_room->contents, 5) < 2) {
		send_to_char(AT_RED,
			     "There is not enough blood in the room.\n\r", ch);
		return;
	}
	for (victim = ch->in_room->people; victim;
		...
		update_pos(victim);
		act(AT_RED, "You bath $N in the life giving fluid.", ch, NULL,
		    victim, TO_CHAR);
		act(AT_RED, "$n baths $N in blood.", ch, NULL, victim, TO_ROOM);
		act(AT_RED, "$n baths you in blood.", ch, NULL, victim,
		    TO_VICT);
	}
...
void spell_call_lightning(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	dam = dice(level / 2, 8);
	send_to_char(AT_WHITE,
		     "Lightning slashes out of the sky to strike your foes!\n\r",
		     ch);
	act(AT_WHITE, "$n calls lightning from the sky to strike $s foes!", ch,
	    NULL, NULL, TO_ROOM);
	...
	for (vch = char_list; vch; vch = vch->next) {
		...
		if (vch->in_room == ch->in_room) {
			if (vch != ch
				...
				dam = sc_dam(ch, dam);
				damage(ch, vch,
				       saves_spell(level, vch) ? dam / 2 : dam,
				       sn);
			}
			continue;
		}
...
void spell_continual_light(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "You twiddle your thumbs and $p appears.", ch, light, NULL,
	    TO_CHAR);
	act(AT_BLUE, "$n twiddles $s thumbs and $p appears.", ch, light, NULL,
	    TO_ROOM);
	return;
}
...
void spell_dispel_evil(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (IS_NEUTRAL(victim)) {
		act(AT_BLUE, "$N does not seem to be affected.", ch, NULL,
		    victim, TO_CHAR);
		return;
	}
...
void spell_earthquake(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_ORANGE, "The earth trembles beneath your feet!\n\r",
		     ch);
	act(AT_ORANGE, "$n makes the earth tremble and shiver.", ch, NULL, NULL,
	    TO_ROOM);
...
void spell_chain_lightning(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_BLUE, "Bolts of electricity arc from your hands!\n\r",
		     ch);
	act(AT_BLUE, "Electrical energy bursts from $n's hands.", ch, NULL,
	    NULL, TO_ROOM);
...
void spell_meteor_swarm(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_RED,
		     "Flaming meteors fly forth from your outstreched hands!\n\r",
		     ch);
	act(AT_RED, "Hundreds of flaming meteors fly forth from $n's hands.",
	    ch, NULL, NULL, TO_ROOM);
...
void spell_enchant_weapon(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	paf->modifier =
	    1 + (level >= 18) + (level >= 25) + (level >= 45) + (level >=
								 65) + (level >=
									90);
	paf->bitvector = 0;
...
void spell_holysword(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	paf->location = APPLY_HITROLL;
	paf->modifier =
	    6 + (level >= 18) + (level >= 25) + (level >= 40) + (level >=
								 60) + (level >=
									90);
	paf->bitvector = 0;
	paf->next = obj->affected;
	...
	paf->location = APPLY_DAMROLL;
	paf->modifier =
	    6 + (level >= 18) + (level >= 25) + (level >= 45) + (level >=
								 65) + (level >=
									90);;
	paf->bitvector = 0;
	paf->next = obj->affected;
...
void spell_psychic_quake(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_YELLOW, "You let the chaos free from your mind!\n\r",
		     ch);
	act(AT_YELLOW, "$n's face becomes blank and concentrated.", ch, NULL,
	    NULL, TO_ROOM);
	...
	for (vch = char_list; vch; vch = vch->next) {
		...
		if (vch->in_room == ch->in_room) {
			if (vch != ch && (IS_NPC(ch) ? !IS_NPC(vch)
					  : IS_NPC(vch)))
				if (vch != ch)
					spell_energy_drain(sn, level / 2, ch,
							   vch);
			continue;
		}
		...
		if (vch->in_room->area == ch->in_room->area)
			send_to_char(AT_BLUE,
				     "A wave of chaos brushes your mind.\n\r",
				     vch);
	}
		...
		case ITEM_ARMOR:
			{
				...
				pObj =
				    create_object(pObjIndex, obj_lose->level);
...
void spell_fireshield(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_RED, "Your body is engulfed by unfelt flame.\n\r",
		     victim);
	act(AT_RED, "$n's body is engulfed in flames.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_faerie_fog(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_PURPLE, "$n conjures a cloud of purple smoke.", ch, NULL, NULL,
	    TO_ROOM);
...
void spell_fly(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "$n's feet rise off the ground.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_golden_aura(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_YELLOW, "You are surrounded by a golden aura.\n\r",
		     victim);
	act(AT_YELLOW, "$n is surrounded by a golden aura.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_goodberry(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "You pass your hand over $p slowly.", ch, obj, NULL,
	    TO_CHAR);
...
void spell_iceshield(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_LBLUE, "An Icy crust forms about your body.\n\r",
		     victim);
	act(AT_LBLUE, "An icy crust forms about $n's body.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_identify(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	sprintf(buf,
		...
		obj->anti_race_flags ? antirace_bit_name(obj->
							 anti_race_flags) : "",
		obj->anti_class_flags ? anticlass_bit_name(obj->
							   anti_class_flags) :
		"");
	...
	case ITEM_POTION:
		...
		if (is_sn(obj->value[1])) {
			...
			send_to_char(AT_WHITE, skill_table[obj->value[1]].name,
				     ch);
			send_to_char(AT_CYAN, "'", ch);
		}
		...
		if (is_sn(obj->value[2])) {
			send_to_char(AT_CYAN, " '", ch);
			send_to_char(AT_WHITE, skill_table[obj->value[2]].name,
				     ch);
			send_to_char(AT_CYAN, "'", ch);
		}
		...
		if (is_sn(obj->value[3])) {
			send_to_char(AT_CYAN, " '", ch);
			send_to_char(AT_WHITE, skill_table[obj->value[3]].name,
				     ch);
			send_to_char(AT_CYAN, "'", ch);
		}
	...
	case ITEM_STAFF:
		if (!(obj->value[1] == -1))
			...
			sprintf(buf, "Has unlimited charges of level %d",
				obj->value[0]);
...
void spell_vibrate(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_LBLUE,
		     "You set up a complex set of vibrations around your body.\n\r",
		     victim);
	act(AT_LBLUE, "$n's body begins to vibrate.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_mist_form(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_GREY, "$n takes on the form of a mist.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_locate_object(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (obj = object_list; obj; obj = obj->next) {
		if (!can_see_obj(ch, obj)
		    || !is_name(ch, target_name, obj->name))
			continue;
...
void spell_mass_invis(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
		...
		send_to_char(AT_GREY, "You slowly fade out of existence.\n\r",
			     gch);
		act(AT_GREY, "$n slowly fades out of existence.", gch, NULL,
		    NULL, TO_ROOM);
...
/*    AFFECT_DATA *paf;*/
	...
	if (obj->item_type != ITEM_WAND
	    && obj->item_type != ITEM_STAFF && obj->item_type != ITEM_LENSE) {
		send_to_char(AT_BLUE,
			     "You cannot make that item permenant.\n\r", ch);
		return;
	}
	obj->value[2] = -1;
	obj->value[1] = -1;
	act(AT_BLUE,
	    "You run your finger up $p, you can feel it's power growing.", ch,
	    obj, NULL, TO_CHAR);
...
void spell_polymorph(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "$n's form wavers and then resolidifies.", ch, NULL, NULL,
	    TO_ROOM);
...
void spell_portal(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "A huge shimmering gate rises from the ground.", ch, NULL,
	    NULL, TO_CHAR);
	act(AT_BLUE,
	    "$n utters a few incantations and a gate rises from the ground.",
	    ch, NULL, NULL, TO_ROOM);
	obj_to_room(gate1, ch->in_room);
	act(AT_BLUE, "A huge shimmering gate rises from the ground.", victim,
	    NULL, NULL, TO_CHAR);
	act(AT_BLUE, "A huge shimmering gate rises from the ground.", victim,
	    NULL, NULL, TO_ROOM);
...
void spell_remove_curse(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (iWear = 0; iWear < MAX_WEAR; iWear++) {
		...
		if (IS_SET(obj->extra_flags, ITEM_NOREMOVE)) {
			...
			act(AT_BLUE, "You toss $p to the ground.", victim, obj,
			    NULL, TO_CHAR);
			act(AT_BLUE, "$n tosses $p to the ground.", victim, obj,
			    NULL, TO_ROOM);
			yesno = TRUE;
		}
	}
...
void spell_sanctuary(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_WHITE, "You are surrounded by a white aura.\n\r",
		     victim);
	act(AT_WHITE, "$n is surrounded by a white aura.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_web(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_WHITE, "$n has been imobilized by a plethora of sticky webs.",
	    victim, NULL, NULL, TO_ROOM);
	return;
}
...
void spell_mind_probe(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (victim->level >= LEVEL_IMMORTAL) {
		...
		send_to_char(AT_BLUE,
			     "The mind of an immortal is beyond your understanding.",
			     ch);
		return;
	}
	...
	if (IS_NPC(victim)) {
		send_to_char(AT_WHITE,
			     "The mind is to chaotic to merge with.\n\r", ch);
		return;
	}
	sprintf(buf,
		"You are %s%s.\n\r",
	...
	sprintf(buf,
		...
		victim->level, get_age(victim), (get_age(victim) - 17) * 4);
	send_to_char(AT_CYAN, buf, ch);
	sprintf(buf,
		"You are a &Y%s&c and have chosen the vocation of a &Y%s&c.\n\r",
	...
	if (get_trust(victim) != victim->level) {
		sprintf(buf,
			"You have been granted the powers of a level &R%d&W.\n\r",
			get_trust(victim));
		send_to_char(AT_WHITE, buf, ch);
	}
	...
	if (!IS_NPC(victim))
		if (get_trust(victim) > LEVEL_IMMORTAL) {
			sprintf(buf, "Bamfin&r: &w%s.\n\r",
				victim->pcdata->bamfin);
			send_to_char(AT_RED, buf, ch);
			sprintf(buf, "Bamfout&r: &w%s.\n\r",
				victim->pcdata->bamfout);
			send_to_char(AT_RED, buf, ch);
	...
	sprintf(buf, " %d/%d(%d)", victim->hit, victim->perm_hit,
		MAX_HIT(victim));
...
*/
	sprintf(buf, "%d/%d(%d)", victim->move, victim->perm_move,
		MAX_MOVE(victim));
	...
	sprintf(buf,
		...
		IS_NPC(victim) ? 13 : get_curr_str(victim),
		IS_NPC(victim) ? 13 : victim->pcdata->perm_int,
		IS_NPC(victim) ? 13 : get_curr_int(victim),
		IS_NPC(victim) ? 13 : victim->pcdata->perm_wis,
		IS_NPC(victim) ? 13 : get_curr_wis(victim),
		IS_NPC(victim) ? 13 : victim->pcdata->perm_dex,
		IS_NPC(victim) ? 13 : get_curr_dex(victim),
		IS_NPC(victim) ? 13 : victim->pcdata->perm_con,
		IS_NPC(victim) ? 13 : get_curr_con(victim));
	...
	send_to_char(AT_WHITE, buf, ch);
	sprintf(buf,
		"&cYou have accumulated &W%d &Ygold, &W%d &wsilver, &cand &W%d &Ocopper &ccoins.\n\r",
...
void spell_entangle(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_GREEN, "Hundreds of vines reach from the ground to entangle $n.",
	    victim, NULL, NULL, TO_ROOM);
	return;
}
...
void spell_shield(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_BLUE, "You are surrounded by a force shield.\n\r",
		     victim);
	act(AT_BLUE, "$n is surrounded by a force shield.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_shockshield(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_BLUE, "Sparks of electricity flow into your body.\n\r",
		     victim);
	act(AT_BLUE,
	    "Bolts of electricity flow from the ground into $n's body.", victim,
	    NULL, NULL, TO_ROOM);
	return;
}
...
void spell_spell_bind(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (obj->item_type == ITEM_WAND
		...
		if (obj->ac_charge[0] < obj->ac_charge[1]
		    && obj->ac_charge[1] != -1) {
			obj->ac_charge[0]++;
			Charged++;
		}
	...
	act(AT_BLUE, "You slowly pass your hand over $p, it vibrates slowly.",
	    ch, obj, NULL, TO_CHAR);
	act(AT_BLUE, "$n slowly passes $s hand over $p, it vibrates slowly.",
	    ch, obj, NULL, TO_ROOM);
	return;
}
...
void spell_summon(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (!(victim = get_char_world(ch, target_name))
	    || victim == ch
	    || ch->in_room->area == arena.area || IS_ARENA(victim)
	    || !victim->in_room
	    || IS_SET(victim->in_room->room_flags, ROOM_SAFE)
	    || IS_SET(victim->in_room->room_flags, ROOM_PRIVATE)
	    || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
...
void spell_teleport(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "$n glimmers briefly, then is gone.", victim, NULL, NULL,
	    TO_ROOM);
	if (pet) {
		act(AT_BLUE, "$n glimmers briefly, then is gone.", pet, NULL,
		    NULL, TO_ROOM);
		char_from_room(pet);
	}
	char_from_room(victim);
	char_to_room(victim, pRoomIndex);
	act(AT_BLUE, "The air starts to sparkle, then $n appears from nowhere.",
	    victim, NULL, NULL, TO_ROOM);
	do_look(victim, "auto");
	if (pet) {
		char_to_room(pet, pRoomIndex);
		act(AT_BLUE,
		    "The air starts to sparkle, then $n appears from nowhere.",
...
void spell_ventriloquate(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (vch = ch->in_room->people; vch; vch = vch->next_in_room) {
		if (!is_name(NULL, speaker, vch->name))
			send_to_char(AT_CYAN,
				     saves_spell(level, vch) ? buf2 : buf1,
				     vch);
	}
...
/*		    if ( ( iWear = obj_lose->wear_loc ) != WEAR_NONE )
		    ...
		    obj_lose->value[0] -= 1;*/
					...
					obj_lose->cost.gold =
					    (obj_lose->cost.gold >
					     0) ? (obj_lose->cost.gold -
						   (obj_lose->cost.gold /
						    20)) : 0;
					obj_lose->cost.silver =
					    (obj_lose->cost.silver >
					     0) ? (obj_lose->cost.silver -
						   (obj_lose->cost.silver /
						    20)) : 0;
					obj_lose->cost.copper =
					    (obj_lose->cost.copper >
					     0) ? (obj_lose->cost.copper -
						   (obj_lose->cost.copper /
						    20)) : 0;
#else
...
void spell_awe(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (victim->fighting == ch && !saves_spell(level, victim)) {
		...
		act(AT_BLUE, "You are in AWE of $n!", ch, NULL, victim,
		    TO_VICT);
		act(AT_BLUE, "$N is in AWE of $n!", ch, NULL, victim,
		    TO_NOTVICT);
	}
	return;
}
...
void spell_chaosfield(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_YELLOW,
		     "You call forth an instance of chaos from the order around you.\n\r",
		     victim);
	act(AT_YELLOW, "$n's body is veiled in an instance or pure chaos.",
	    victim, NULL, NULL, TO_ROOM);
	return;
}
...
void spell_bladebarrier(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_GREY,
		     "You bring forth thousands of tiny spinning blades about your body.\n\r",
		     victim);
	act(AT_GREY, "$n's body is surrounded by thousands of spinning blades.",
	    victim, NULL, NULL, TO_ROOM);
	return;
}
...
void spell_dancing_lights(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_WHITE,
	    "&.Thou&.sand&.s &.of &.danci&.ng &.ligh&.ts &.surr&.ound &.you&.!&w",
	    victim, NULL, victim, TO_VICT);
	act(AT_GREY,
	    "&W$n's &.body &.is &.surr&.ounded &.by d&.anci&.ng l&.ights.",
	    victim, NULL, NULL, TO_ROOM);
	return;
}
...
void spell_combat_mind(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (is_affected(victim, sn)) {
		if (victim == ch)
			send_to_char(AT_BLUE,
				     "You already understand battle tactics.\n\r",
				     victim);
		else
		...
		if (!IS_NPC(ch) ? IS_NPC(vch) : IS_NPC(vch)) {
			...
			if (!saves_spell(level, vch)
				...
				send_to_char(AT_DGREY,
					     "The haze envelops you!\n\r", vch);
				act(AT_DGREY, "The haze envelops $N!", ch, NULL,
				    vch, TO_NOTVICT);
...
void spell_disintegrate(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (number_percent() < 2 * level && !saves_spell(level, victim))
		for (obj_lose = victim->carrying; obj_lose; obj_lose = obj_next) {
			...
			act(AT_WHITE, "$p disintegrates!", victim, obj_lose,
			    NULL, TO_CHAR);
			act(AT_WHITE, "$n's $p disintegrates!", victim,
	...
	if (!saves_spell(level, victim))
		 ...
		 */
	{
		act(AT_WHITE, "You have DISINTEGRATED $N!", ch, NULL, victim,
		    TO_CHAR);
		act(AT_WHITE, "You have been DISINTEGRATED by $n!", ch, NULL,
		    victim, TO_VICT);
		act(AT_WHITE, "$n's spell DISINTEGRATES $N!", ch, NULL, victim,
		    TO_ROOM);
...
void spell_disrupt(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (!is_affected(vch, sn) && (vch != victim ||
		...
		if (ch == vch) {
			act(AT_PINK, "You disrupt yourself!", ch, NULL, NULL,
			    TO_CHAR);
			act(AT_PINK, "$n disrupts $mself!", ch, NULL, NULL,
			    TO_ROOM);
		} else {
			act(AT_PURPLE, "You disrupt $N!", ch, NULL, victim,
			    TO_CHAR);
			act(AT_PURPLE, "$n disrupts $N!", ch, NULL, victim,
			    TO_NOTVICT);
			act(AT_PURPLE, "$n disrupts you!", ch, NULL, victim,
			    TO_VICT);
			switch (af.location) {
			case APPLY_MANA:
...
void spell_displacement(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_GREY,
		     ...
		     victim);
	act(AT_GREY, "$N shimmers and appears in a different location.", ch,
...
void spell_domination(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "Your will is dominated by $n!", ch, NULL, victim,
	    TO_VICT);
	return;
}
...
void spell_ego_whip(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "You ridicule $N about $S childhood.", ch, NULL, victim,
	    TO_CHAR);
	send_to_char(AT_BLUE, "Your ego takes a beating.\n\r", victim);
	act(AT_BLUE, "$N's ego is crushed by $n!", ch, NULL, victim,
	    TO_NOTVICT);
...
void spell_flesh_armor(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "$N's flesh turns to steel.", ch, NULL, victim,
	    TO_NOTVICT);
	return;
}
...
void spell_inertial_barrier(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
		...
		act(AT_BLUE, "An inertial barrier forms around $n.", gch, NULL,
		    NULL, TO_ROOM);
		send_to_char(AT_BLUE,
...
void spell_intellect_fortress(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (gch = ch->in_room->people; gch; gch = gch->next_in_room) {
		...
		act(AT_BLUE, "A virtual fortress forms around $N.", gch, NULL,
		    gch, TO_ROOM);
...
void spell_lend_health(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (ch == victim) {
		send_to_char(AT_BLUE,
			     "Lend health to yourself?  Easily done.\n\r", ch);
		return;
	}
	hpch = UMIN(50, MAX_HIT(victim) - victim->hit);
	if (hpch == 0) {
		act(AT_BLUE, "Nice thought, but $N doesn't need healing.", ch,
		    NULL, victim, TO_CHAR);
		return;
	}
	if (ch->hit - hpch < 50) {
	...
	act(AT_BLUE, "You lend some of your health to $N.", ch, NULL, victim,
	    TO_CHAR);
	act(AT_BLUE, "$n lends you some of $s health.", ch, NULL, victim,
	    TO_VICT);
...
void spell_psychic_drain(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "$n appears drained of strength.", victim, NULL, NULL,
	    TO_ROOM);
	return;
}
...
void spell_share_strength(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (victim == ch) {
		send_to_char(AT_BLUE,
			     "You can't share strength with yourself.\n\r", ch);
		return;
	}
	if (is_affected(victim, sn)) {
		act(AT_BLUE, "$N already shares someone's strength.", ch, NULL,
		    victim, TO_CHAR);
		return;
	}
	if (get_curr_str(ch) <= 5) {
		send_to_char(AT_BLUE,
			     "You are too weak to share your strength.\n\r",
			     ch);
		return;
	}
	...
	act(AT_BLUE, "You share your strength with $N.", ch, NULL, victim,
	    TO_CHAR);
	act(AT_BLUE, "$n shares $s strength with you.", ch, NULL, victim,
	    TO_VICT);
	return;
}
...
void spell_summon_shadow(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	ch->summon_timer = 16;
	char_to_room(mob, ch->in_room);
	act(AT_GREY, "You summon $N from the shadow plane.", ch, NULL, mob,
	    TO_CHAR);
	mana = is_class(ch, CLASS_VAMPIRE) ? level / 2 : level * 2;
	if (MT(ch) < mana) {
		sprintf(buf, "%sYou don't have enough %s to bind $N!",
	...
	adjust_mana(ch, -mana);
	act(AT_GREY, "$n calls forth $N from the shadow plane.", ch, NULL, mob,
	    TO_ROOM);
...
void spell_summon_trent(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	ch->summon_timer = 16;
	char_to_room(mob, ch->in_room);
	act(AT_ORANGE, "You summon $N from the plane of nature.", ch, NULL, mob,
	    TO_CHAR);
	mana = is_class(ch, CLASS_VAMPIRE) ? level / 2 : level * 2;
	if (MT(ch) < mana) {
		sprintf(buf, "%sYou don't have enough %s to bind $N!",
	...
	adjust_mana(ch, -mana);
	act(AT_ORANGE, "$n calls forth $N from the plane of nature.", ch, NULL,
	    mob, TO_ROOM);
...
void spell_summon_beast(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	adjust_mana(ch, -mana);
	act(AT_GREEN, "$n calls forth $N from the forests.", ch, NULL, mob,
	    TO_ROOM);
...
void check_dispel_aff(CHAR_DATA * victim, bool * found, int level,
		      const char *spell, long vector)
{
	int sn;
...
void check_dispel_aff2(CHAR_DATA * victim, bool * found, int level,
		       const char *spell, long vector)
{
	int sn;
	sn = skill_lookup(spell);
...
bool dispel_flag_only_spells(int level, CHAR_DATA * victim)
{
	...
	check_dispel_aff(victim, &found, level, "detect hidden",
			 AFF_DETECT_HIDDEN);
	check_dispel_aff(victim, &found, level, "detect invis",
			 AFF_DETECT_INVIS);
	check_dispel_aff(victim, &found, level, "detect magic",
			 AFF_DETECT_MAGIC);
	...
	check_dispel_aff2(victim, &found, level, "detect good",
			  AFF_DETECT_GOOD);
	check_dispel_aff2(victim, &found, level, "field of decay", AFF_FIELD);
	check_dispel_aff2(victim, &found, level, "protection good",
			  AFF_PROTECTION_GOOD);
...
void spell_enchanted_song(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (ch == victim) {
		act(AT_BLUE, "$n sings an enchanting song.", ch, NULL, NULL,
		    TO_ROOM);
	...
	act(AT_BLUE, "$n's song slows your reactions.", ch, NULL, victim,
	    TO_VICT);
...
/*    ||   IS_SET(victim->off_flags,OFF_FAST)*/
		...
		if (victim == ch)
			...
			act(C_DEFAULT,
			    "$N is already moving as fast as $e can.", ch, NULL,
			    victim, TO_CHAR);
		return;
	}
	af.type = sn;
	...
	act(C_DEFAULT, "$n is moving more quickly.", victim, NULL, NULL,
	    TO_ROOM);
	if (ch != victim)
		send_to_char(C_DEFAULT, "Ok.\n\r", ch);
	return;
...
void spell_hex(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (IS_AFFECTED(victim, AFF_BLIND + AFF_CURSE)
		...
		act(AT_DGREY, "$N is already hexed.", ch, NULL, victim,
		    TO_CHAR);
		return;
	}
	if (saves_spell(level, victim)) {
	...
	af.location = APPLY_STR;
	af.modifier =
	    -1 - (level >= 20) - (level >= 40) - (level >= 60) - (level >= 80);
	affect_to_char(victim, &af);
...
void spell_dark_ritual(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (found == TRUE) {
		...
		send_to_char(AT_DGREY,
			     "You extract the last of the energy from the corpse.\n\r",
			     ch);
		act(AT_DGREY,
		    "$n saps away the last of the mystical energies from the $p.",
		    ch, obj, NULL, TO_ROOM);
		extract_obj(obj);
	} else
		send_to_char(AT_DGREY,
			     "You must have a corpse to sacrifice to perform a dark ritual.\n\r",
			     ch);
	return;
}
...
void spell_field_of_decay(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_DGREY,
		     "You summon the power of the dead and a black haze envelops you.\n\r",
		     ch);
...
void spell_soul_bind(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_BLUE, "You tear out $Ns soul, binding it to form a Soulgem.", ch,
	    NULL, victim, TO_CHAR);
	act(AT_BLUE, "$n tears out $Ns soul, binding it to form a Soulgem.", ch,
	    NULL, victim, TO_ROOM);
	act(AT_BLUE,
	    "$N screams in agony as it slowly dissipates into nothingness!", ch,
	    NULL, victim, TO_CHAR);
	act(AT_BLUE,
	    "$N screams in agony as it slowly dissipates into nothingness!", ch,
	    NULL, victim, TO_ROOM);
...
void spell_chi_shield(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (is_affected(ch, sn))
		return;
	send_to_char(AT_BLUE,
		     "You tap into your chi and use it to raise a chi shield.\n\r",
		     ch);
	act(AT_BLUE, "$n is surrounded by a chi shield.", ch, NULL, NULL,
	    TO_ROOM);
	af.type = sn;
	af.level = ch->level * 2;
...
void spell_thunder_strike(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_WHITE, "$N looks confused!", ch, NULL, victim, TO_NOTVICT);
...
void spell_inspiration(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	for (victim = ch->in_room->people; victim;
		...
		if (is_affected(victim, sn)) {
			if (victim == ch)
				send_to_char(AT_WHITE,
					     "You have already been inspired.\n\r",
					     victim);
			else
				act(AT_WHITE, "$N has been inspired already.",
				    ch, NULL, victim, TO_CHAR);
/*	  return;  (still want to loop through rest of chars so just continue) */
		...
		if (victim != ch) {
			...
			send_to_char(AT_CYAN,
				     "You inspire yourself to greater heights.\n\r",
				     ch);
	}
...
void spell_war_cry(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (ch != victim)
		send_to_char(AT_BLOOD,
			     ...
			     ch);
	send_to_char(AT_BLOOD,
		     "A bloodthirsty cry sends shivers down your spine.\n\r",
		     victim);
...
void spell_chant(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_BLUE, "You begin your loud chant of destruction!\n\r",
		     ch);
	act(AT_BLUE, "$n's chant wreaks havoc everywhere!", ch, NULL, NULL,
	    TO_ROOM);
...
void spell_blur(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	act(AT_GREY,
	    "$N form becomes blurred, shifting and wavering before you.", ch,
	    NULL, victim, TO_NOTVICT);
	return;
}
...
void spell_firewall(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	send_to_char(AT_RED,
		     "A wall of fire explodes out in all direction from you!\n\r",
		     ch);
	act(AT_RED, "A wall of fire explodes out from $n!", ch, NULL, NULL,
	    TO_ROOM);
