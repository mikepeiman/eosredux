...
/* void move_char( CHAR_DATA *ch, int door ) */
{
	...
	if ((IS_AFFECTED(ch, AFF_ANTI_FLEE)) && (!Fall)) {
		if (is_affected(ch, gsn_howlfear))
			send_to_char(AT_WHITE,
				     "You are too frightened to move.\n\r", ch);
		else
	...
	if (!IS_NPC(ch) && (!Fall)) {
		...
		if (number_percent() < drunk) {
			...
			send_to_char(AT_BLUE,
				     "You're too drunk to think clearly! You wander"
	...
	if (IS_SET(pexit->exit_info, EX_CLOSED)) {
		...
		if (IS_SET(pexit->exit_info, EX_PASSPROOF)) {
			act(AT_GREY,
			    "You are unable to pass through the &W$d&w.", ch,
	...
	if (IS_AFFECTED(ch, AFF_CHARM)
	    && ch->master && in_room == ch->master->in_room && (!Fall)) {
	...
	if (to_room->vnum == ROOM_VNUM_SMITHY && ch->race != RACE_DWARF
	    && (!Fall)) {
...
/* Blood trickles from your wounds.\n\r */
			...
			send_to_char(AT_BLOOD,
				     "Blood trickles from your wounds.\n\r",
				     ch);
			act(AT_RED, "$n leaves a trail of blood.", ch, NULL,
			    NULL, TO_ROOM);
		...
		if (!Fall) {
			if ((!IS_AFFECTED(ch, AFF_FLYING))
			    && (ch->race != RACE_PIXIE)
			    && (ch->race != RACE_ELDER))
				act(AT_GREY, "&B$n&w leaves $T.", ch, NULL,
				    dir_name[door], TO_ROOM);
			else
				act(AT_GREY, "&B$n&w flies $T.", ch, NULL,
				    dir_name[door], TO_ROOM);
		}
	}
	...
	if (Fall)
		act(AT_WHITE, "$n falls down from above.", ch, NULL, NULL,
		    TO_ROOM);

	if (to_room->exit[rev_dir[door]] &&
	    to_room->exit[rev_dir[door]]->to_room == in_room)
		eprog_exit_trigger(to_room->exit[rev_dir[door]], ch->in_room,
				   ch);
	else
...
int find_door(CHAR_DATA * ch, char *arg, bool pMsg)
{
	...
	if (!str_prefix(arg, "north"))
		...
		if (pMsg)
			act(AT_GREY, "I see no $T here.", ch, NULL, arg,
			    TO_CHAR);
		return -1;
	}
	...
	if (!(pexit = ch->in_room->exit[door])) {
		if (pMsg)
			act(AT_GREY, "I see no door $T here.", ch, NULL, arg,
			    TO_CHAR);
		return -1;
	}
...
void do_open(CHAR_DATA * ch, char *argument)
{
	...
	if ((door = find_door(ch, arg, TRUE)) >= 0) {
		...
		act(C_DEFAULT, "$n opens the $d.", ch, NULL, pexit->keyword,
		    TO_ROOM);
		send_to_char(C_DEFAULT, "Ok.\n\r", ch);
		...
		if ((to_room = pexit->to_room)
			...
			for (rch = to_room->people; rch;
			     rch = rch->next_in_room) {
				if (rch->deleted)
					continue;
				act(C_DEFAULT, "The $d opens.", rch, NULL,
...
void do_close(CHAR_DATA * ch, char *argument)
{
	...
	if ((door = find_door(ch, arg, TRUE)) >= 0) {
		...
		if (IS_SET(pexit->exit_info, EX_BASHED)) {
			act(C_DEFAULT,
			    "The $d has been bashed open and cannot be closed.",
			    ch, NULL, pexit->keyword, TO_CHAR);
			return;
		}

		SET_BIT(pexit->exit_info, EX_CLOSED);
		act(C_DEFAULT, "$n closes the $d.", ch, NULL, pexit->keyword,
		    TO_ROOM);
		send_to_char(C_DEFAULT, "Ok.\n\r", ch);
		...
		if ((to_room = pexit->to_room)
			...
			for (rch = to_room->people; rch;
				...
				if (rch->deleted)
					continue;
				act(C_DEFAULT, "The $d closes.", rch, NULL,
				    pexit_rev->keyword, TO_CHAR);
			}
		}
		return;
...
void do_lock(CHAR_DATA * ch, char *argument)
{
	...
	if ((door = find_door(ch, arg, TRUE)) >= 0) {
		...
		act(C_DEFAULT, "$n locks the $d.", ch, NULL, pexit->keyword,
		    TO_ROOM);
...
void do_unlock(CHAR_DATA * ch, char *argument)
{
	...
	if ((door = find_door(ch, arg, FALSE)) >= 0) {
		...
		send_to_char(C_DEFAULT, "*Click*\n\r", ch);
		act(C_DEFAULT, "$n unlocks the $d.", ch, NULL, pexit->keyword,
		    TO_ROOM);
...
void do_pick(CHAR_DATA * ch, char *argument)
{
	...
	if ((door = find_door(ch, arg, TRUE)) >= 0) {
		...
		if (!IS_AFFECTED(ch, AFF_HIDE))
			act(C_DEFAULT, "$n picks the $d.", ch, NULL,
...
void do_stand(CHAR_DATA * ch, char *argument)
{
	...
	case POS_SLEEPING:
		...
		if (!IS_AFFECTED(ch, AFF_HIDE))
			act(AT_CYAN, "$n wakes and stands up.", ch, NULL, NULL,
			    TO_ROOM);
...
void do_recall(CHAR_DATA * ch, char *argument)
{
	...
	if (!strcmp(ch->name, "Jekka")) {
		...
		act(C_DEFAULT, "$n appears in the room.", ch, NULL, NULL,
		    TO_ROOM);
		do_look(ch, "auto");
		return;
	}
	...
	if (pet) {
		...
		act(C_DEFAULT, "$n appears in the room.", pet, NULL, NULL,
		    TO_ROOM);
	}
...
void do_train(CHAR_DATA * ch, char *argument)
{
	...
	if (*pAbility >= 18 && bone_flag == 0) {
		act(AT_CYAN, "Your $T is already at maximum.", ch, NULL,
		    pOutput, TO_CHAR);
		return;
	}
...
void do_chameleon(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char(AT_DGREY,
		     "You attempt to blend in with your surroundings.\n\r", ch);
...
void do_enter(CHAR_DATA * ch, char *argument)
{
	...
	if (!(location = get_room_index(destination))) {
		act(AT_BLUE, "You try to enter $p but can't.", ch, obj, NULL,
		    TO_CHAR);
		return;
	}
	act(AT_BLUE, "You step into the $p.", ch, obj, NULL, TO_CHAR);
	act(AT_BLUE, "$n steps into the $p and is gone.", ch, obj, NULL,
	    TO_ROOM);
	rprog_exit_trigger(ch->in_room, ch);
	char_from_room(ch);
	char_to_room(ch, location);
	send_to_char(AT_BLUE,
		     "You feel dizzy, and when finally you emerge you are elsewhere.\n\r",
		     ch);
	act(AT_BLUE, "$n steps out of the $p before you.", ch, obj, NULL,
	    TO_ROOM);
	do_look(ch, "auto");
	for (fch = in_room->people; fch; fch = fch_next) {
		fch_next = fch->next_in_room;
		...
		if (fch->master == ch && fch->position == POS_STANDING) {
			act(AT_WHITE, "You follow $N through $p.", fch, obj, ch,
			    TO_CHAR);
			do_enter(fch, arg1);
		}
	}
...
void do_bash(CHAR_DATA * ch, char *argument)
{
	...
	if ((door = find_door(ch, arg, TRUE)) >= 0) {
		...
		if ((get_curr_str(ch) >= 20)
			...
			act(C_DEFAULT, "Crash!  You bashed open the $d!", ch,
			    NULL, pexit->keyword, TO_CHAR);
			act(C_DEFAULT, "$n bashes open the $d!", ch, NULL,
			    pexit->keyword, TO_ROOM);
			...
			act(C_DEFAULT,
			    "OW!  You bash against the $d, but it doesn't budge.",
			    ch, NULL, pexit->keyword, TO_CHAR);
			act(C_DEFAULT,
			    "$n bashes against the $d, but it holds strong.",
			    ch, NULL, pexit->keyword, TO_ROOM);
...
/*  if ( is_safe(ch, victim) )
  {
	...
	if (victim->position != POS_STANDING) {
		send_to_char(AT_BLUE,
			     "Can't push someone who is not standing.\n\r", ch);
		return;
	}
	pexit = ch->in_room->exit[door];
	if (pexit == NULL || IS_SET(pexit->exit_info, EX_CLOSED)) {
		act(AT_BLUE,
		    "There is no exit, but you push $M around anyways.", ch,
		    NULL, victim, TO_CHAR);
		act(AT_BLUE, "$n pushes $N against a wall.", ch, NULL, victim,
		    TO_NOTVICT);
		act(AT_BLUE, "$n pushes you against a wall, ouch.", ch, NULL,
		    victim, TO_VICT);
		return;
	}
	if (pexit->to_room->vnum == ROOM_VNUM_SMITHY) {
		act(AT_BLUE,
		    "You slam into $N, but a force field prevents $S entry.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_BLUE,
		    "$n pushes $N, but a force field makes $M bounce back.", ch,
		    NULL, victim, TO_NOTVICT);
		act(AT_BLUE,
		    "$n slams into you, but you bounce off a force field.", ch,
		    NULL, victim, TO_VICT);
		return;
	}
...
/*  if ( is_safe( ch, victim ) )
  {
	...
	if (pexit == NULL || IS_SET(pexit->exit_info, EX_CLOSED)) {
		act(AT_BLUE,
		    "There is no exit, but you drag $M around anyways.", ch,
		    NULL, victim, TO_CHAR);
		act(AT_BLUE, "$n drags $N around the room.", ch, NULL, victim,
		    TO_NOTVICT);
		act(AT_BLUE, "$n drags you around the room.", ch, NULL, victim,
		    TO_VICT);
		return;
	}
	if (pexit->to_room->vnum == ROOM_VNUM_SMITHY) {
		act(AT_BLUE,
		    "You grap onto $N, but a force field prevents you entry into the smithy.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_BLUE,
		    "$n attempts to drag $N into the smithy, but a force field stops $m.",
		    ch, NULL, victim, TO_NOTVICT);
		act(AT_BLUE,
		    "$n attempts to drag you into the smithy, but a force field stops $m.",
	...
	char_from_room(victim);
	char_to_room(victim, pexit->to_room);
	act(AT_BLUE, "$N arrives, dragging $n with $M.", victim, NULL, ch,
	    TO_ROOM);
	char_from_room(ch);
...
/*      && ( !IS_AFFECTED( ch, AFF_FLYING ) ) )  */
	{
		act(AT_RED,
		    ...
		    NULL, NULL, TO_CHAR);
		act(C_DEFAULT, "$n falls down to the room below.", ch, NULL,
		    NULL, TO_ROOM);
		damage(ch, ch, 5, TYPE_UNDEFINED);
...
void do_shadow_walk(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_AFFECTED(ch, AFF_ANTI_FLEE)) {
		send_to_char(AT_GREY,
			     "You cannot shadow walk in your condition!\n\r",
			     ch);
		return;
	}
...
*/
	...
	if (!found) {
		send_to_char(AT_GREY,
			     "The shadows offer no path to that one.\n\r", ch);
		return;
	}

	if (!IS_AFFECTED(ch, AFF_HIDE))
		act(AT_GREY, "$n steps into the shadows and is gone.", ch, NULL,
		    NULL, TO_ROOM);
	if (ch != victim) {
		char_from_room(ch);
		char_to_room(ch, victim->in_room);
	...
	if (!IS_AFFECTED(ch, AFF_HIDE))
		act(AT_GREY, "$n steps forth from the shadows.", ch, NULL, NULL,
		    TO_ROOM);
	do_look(ch, "auto");
...
{
	static const char *dir_table[] = { "to the north",
		"to the east",
		"to the south",
		"to the west",
		"above",
	...
	static const char *dis_table[] = { "close by",
		"not far off"
	};
	CHAR_DATA *sch;
	EXIT_DATA *pexit;
	...
	if (!can_use_skpell(ch, gsn_scent)) {
		send_to_char(C_DEFAULT,
			     "Your sense of smell is not keen enough.\n\r", ch);
		return;
	}
	if (ch->pcdata->learned[gsn_scent] < number_percent()) {
		send_to_char(C_DEFAULT,
			     "You sniff around and start to sneeze!.\n\r", ch);
		return;
	}
	in_room = ch->in_room;
	for (dir = 0; dir <= 5; dir++) {
		...
		for (dis = 0; dis <= 1; dis++) {
			for (sch = next_room->people; sch;
				...
				sprintf(buf, "%s &w%s %s.\n\r",
					capitalize(PERS(sch, ch)), dis_message,
					dir_message);
				send_to_char(C_DEFAULT, buf, ch);
			}
	...
	if (!found)
		send_to_char(C_DEFAULT,
			     "You did not pick up anything's scent.\n\r", ch);
...
void do_retreat(CHAR_DATA * ch, char *argument)
{
	...
	ROOM_INDEX_DATA *in_room;
	EXIT_DATA *pexit;
	static const char *dir_verb[] = { "north", "east", "south",
		"west", "up", "down"
	};
	int dir;
	const char *sdir;
	...
	if (!ch->fighting) {
		send_to_char(AT_GREY,
			     "Retreat from what? You're not even fighting!\n\r",
			     ch);
		return;
	}
	if (argument[0] == '\0') {
		send_to_char(AT_GREY,
			     "You're retreating, not fleeing!  Provide a direction!\n\r",
			     ch);
		return;
	}
	...
	if (IS_AFFECTED(ch, AFF_ANTI_FLEE)) {
		if (is_affected(ch, gsn_howlfear))
			send_to_char(AT_WHITE,
				     "You are too frightened to plan an escape.\n\r",
				     ch);
		else
			send_to_char(AT_WHITE,
				     "You can't move, how do you expect to retreat?\n\r",
				     ch);
		return;
	}
	send_to_char(AT_DGREY,
		     "You throw your smoke bomb to the ground and...\n\r", ch);
	...
	if (number_percent() < ch->pcdata->learned[gsn_retreat]) {
		...
		if (!(pexit = in_room->exit[dir])
		    || !pexit->to_room) {
			send_to_char(AT_GREY,
				     "Wham!  Ouch! Retreated straight into a wall!\n\r",
				     ch);
			...
			if (IS_SET(pexit->exit_info, EX_CLOSED)) {
				sprintf(buf,
					"Wham!  Ouch!  Retreated straight into the closed %s.\n\r",
					pexit->keyword);
				...
				sprintf(buf,
					"You retreat %s before the smoke clears.\n\r",
					sdir);
